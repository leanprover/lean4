...
* `bv_decide` tactic. This release introduces a new tactic for proving goals involving `BitVec` and `Bool`. It reduces the goal to an SAT instance that is refuted by an external solver, and the resulting LRAT proof is  checked in Lean using. This is used to synthesize a proof of the goal by reflection. As this process uses verified algorithms, proofs generated by this tactic use `Lean.ofReduceBool`, so this tactic includes the Lean compiler as part of the trusted code base.

  For example, we can use `bv_decide` to verify that the following bit twiddling formula has at most one bit set.
  ```lean
...
* `simp` tactic
  ...
  * [#5187](https://github.com/leanprover/lean4/pull/5187) restores `reduceCtorEq` to the `norm_cast` tactic.
* `ext` tactic

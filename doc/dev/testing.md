...
- [`tests/lean`](/tests/lean/): contains tests that come equipped with a
  .lean.expected.out file. The driver script [`test_single.sh`](/tests/lean/test_single.sh) runs
  each test and checks the actual output (*.produced.out) with the
  checked in expected output.

- [`tests/lean/run`](/tests/lean/run/): contains tests that are run through the lean
  command line one file at a time. These tests only look for error
  codes and do not check the expected output even though output is
  produced, it is ignored.

- [`tests/lean/interactive`](/tests/lean/interactive/): are designed to test server requests at a
...
- [`tests/lean/server`](/tests/lean/server/): Tests more of the Lean `--server` protocol.
  There are just a few of them, and it uses .log files containing
  JSON.

- [`tests/compiler`](/tests/compiler/): contains tests that will run the Lean compiler and
  build an executable that is executed and the output is compared to
  the .lean.expected.out file. This test also contains a subfolder
  [`foreign`](/tests/compiler/foreign/) which shows how to extend Lean using C++.

- [`tests/lean/trust0`](/tests/lean/trust0): tests that run Lean in a mode that Lean doesn't
  even trust the .olean files (i.e., trust 0).

- [`tests/bench`](/tests/bench/): contains performance tests.

- [`tests/plugin`](/tests/plugin/): tests that compiled Lean code can be loaded into
...
For an example of a conforming test, see [`tests/lean/1971.lean`](/tests/lean/1971.lean).
...
Now, suppose `bad_class.lean` test is broken. We can see the problem by going to [`tests/lean`](/tests/lean) directory and
executing

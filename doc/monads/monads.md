...
The `bind` operator also has infix notation `>>=` where `x >>= f` represents the result of
executing `x`to get a value of type `f α` then passing it to function `f` of type
`α → f β` and returning the result of type `f β`.

This `bind` operation looks similar to the other ones we've seen so far, if we put
them all togther Monad has the following operations:
...
over a type `α`, and then produce a structure wrapping a type `β`. They just vary in what the function
looks like. For the functor, the function is a normal pure function. For applicatives, the function
is still pure, but wrapped in the structure. Now with monads, our function argument takes a "pure"
input but produces an output in the structure.
...
`:= match a, b with` can be collapsed away.  To make tis more clear consider the following
simpler example:
...
So `Option.bind` is using the second example.  What is important is it is using the
function in the second argument to compute the value `f a` which is then returned as the
result of type `Option β`

You can also see in `Option.bind` that if you apply a function over `none` you
just get back `none` which makes sense.
...
The `Option` monad helps us fix this. Here's what this function looks like using the `bind` operator.
...
bind operator allows us to take this `Option Nat` value and combine it with `maybeFunc2`, whose type
...
This notation allows us to write monadic operations one after another, line-by-line. It almost makes
...
At first glance, this looks more complicated than the bind example. However, it gives us a lot more
...
So, once again we see that the `do` notation lets us chain a series of monadic actions.
...
But I'm not sure how to turn that int an simple example...
...
The associativity law is difficult to parse like some of the applicative laws, but it has a similar
meaning. If we change the grouping of `bind` operations, we should still get the same result.
You can see this in action in the following rewrite of `runMaybeFuncsBind`:
...
examples of what example a "computational context" means would be useful to you. The Reader, State
and Except monads each provide a concrete and easily understood context that can be compared easily
to function parameters. So you can learn more about those in [Part 4: Reader monads](readers.md) and

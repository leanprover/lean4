...
2- The `Options` control whether tracing commands will be ignored or not.

3- The macro `trace! <cls> <msg>` adds the trace message `<msg>` if `<cls>` is activated and tracing is enabled.
...
5- `traceCtx <cls> <action>` groups all messages generated by `<action>` into a single `MessageData.node`.
    If `<cls> is not activated, then tracing is disabled while executing `<action>`.
-/

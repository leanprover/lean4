...
meta_constant trace_fmt {A : Type u} : format → (unit → A) → A
...
meta_definition to_fmt {A : Type u} [has_to_format A] : A → format :=
...
meta_definition format.when {A : Type u} [has_to_format A] : bool → A → format
| tt a := to_fmt a
| ff a := nil
...
meta_definition list.to_format_aux {A : Type u} [has_to_format A] : bool → list A → format
| b  []      := to_fmt ""
| tt (x::xs) := to_fmt x ++ list.to_format_aux ff xs
| ff (x::xs) := to_fmt "," ++ line ++ to_fmt x ++ list.to_format_aux ff xs

meta_definition list.to_format {A : Type u} [has_to_format A] : list A → format
| []      := to_fmt "[]"
| (x::xs) := to_fmt "[" ++ group (nest 1 (list.to_format_aux tt (x::xs))) ++ to_fmt "]"

attribute [instance]
meta_definition list.has_to_format {A : Type u} [has_to_format A] : has_to_format (list A) :=
...
attribute [instance]
meta_definition option.has_to_format {A : Type u} [has_to_format A] : has_to_format (option A) :=
has_to_format.mk (λ o, option.cases_on o
  (to_fmt "none")
  (λ a, to_fmt "(some " ++ nest 6 (to_fmt a) ++ to_fmt ")"))

attribute [instance]
meta_definition sum.has_to_format {A : Type u} {B : Type v} [has_to_format A] [has_to_format B] : has_to_format (sum A B) :=
...
attribute [instance]
meta_definition subtype.has_to_format {A : Type u} {P : A → Prop} [has_to_format A] : has_to_format (subtype P) :=

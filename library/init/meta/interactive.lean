...
meta instance rw_rule_has_reflect : has_reflect rw_rule
| {pos := p, symm := s, rule := r} := `(_)
...
meta instance rw_rule_t_has_reflect : has_reflect rw_rules_t
| {rules := rs, end_pos := e} := `(_)
...
meta instance simp_arg_type_has_reflect : has_reflect simp_arg_type
| (simp_arg_type.all_hyps)  := `(_)
| (simp_arg_type.except ex) := `(_)
| (simp_arg_type.expr e)    := `(_)

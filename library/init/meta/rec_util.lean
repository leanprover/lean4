...
meta definition is_type_app_of (e : expr) (I_name : name) : tactic bool :=
do t ← infer_type e,
   return $ is_constant_of (get_app_fn t) I_name

/- Auxiliary function for using brec_on "dictionary" -/
private meta definition mk_rec_inst_aux : expr → nat → tactic expr
| F 0     := do
  P ← mk_app `prod.fst [F],
  mk_app `prod.fst [P]
| F (n+1) := do
...
meta definition mk_brec_on_rec_value (F_name : name) (i : nat) : tactic expr :=
do F ← get_local F_name,
   mk_rec_inst_aux F i

meta definition constructor_num_fields (c : name) : tactic nat :=
do env     ← get_env,
...
private meta definition mk_name_list_aux : name → nat → nat → list name → list name × nat
| p i 0     l := (list.reverse l, i)
| p i (j+1) l := mk_name_list_aux p (i+1) j (mk_num_name p i :: l)

private meta definition mk_name_list (p : name) (i : nat) (n : nat) : list name × nat :=
mk_name_list_aux p i n []

/- Return a list of names of the form [p.i, ..., p.{i+n}] where n is
   the number of fields of the constructor c -/
meta definition mk_constructor_arg_names (c : name) (p : name) (i : nat) : tactic (list name × nat) :=
do nfields ← constructor_num_fields c,
   return $ mk_name_list p i nfields

private meta definition mk_constructors_arg_names_aux : list name → name → nat → list (list name) → tactic (list (list name))
...
meta definition mk_constructors_arg_names (I : name) (p : name) : tactic (list (list name)) :=
do env ← get_env,

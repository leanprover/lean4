...
let
  ...
  llvmPackages = if pkgs.stdenv.isDarwin then llvmPackages_11 else llvmPackages_14;
  ...
  stdenv = overrideCC pkgs.stdenv cc;
  lean = callPackage (import ./bootstrap.nix) (args // {
    inherit stdenv buildLeanPackage;
  });
...
in {
  inherit cc lean4-mode buildLeanPackage llvmPackages vscode-lean4 stdenv;
  lean = lean.stage1;

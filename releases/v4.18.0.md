...
* [#6744](https://github.com/leanprover/lean4/pull/6744) extend the preprocessing of well-founded recursive definitions
  to bring assumptions like `h✝ : x ∈ xs`, if a recursive call is in the argument of a higher-order function like `List.map`,
  into scope automatically. In many cases this removes the need to use functions like `List.attach`.

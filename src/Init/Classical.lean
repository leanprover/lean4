...
noncomputable def inhabitedOfExists {α : Sort u} {p : α → Prop} (h : ∃ x, p x) : Inhabited α :=
  inhabitedOfNonempty (Exists.elim h (fun w _ => ⟨w⟩))
...
noncomputable def typeDecidable (α : Sort u) : PSum α (α → False) :=
  ...
  | (isTrue hp)  => PSum.inl (@default _ (inhabitedOfNonempty hp))
...
/-- the axiom of choice -/
theorem axiom_of_choice {α : Sort u} {β : α → Sort v} {r : ∀ x, β x → Prop} (h : ∀ x, ∃ y, r x y) : ∃ (f : ∀ x, β x), ∀ x, r x (f x) :=
...
theorem by_cases {p q : Prop} (hpq : p → q) (hnpq : ¬p → q) : q :=
  Decidable.by_cases (dec := propDecidable _) hpq hnpq

-- this supercedes by_contradiction in Decidable
theorem by_contradiction {p : Prop} (h : ¬p → False) : p :=
  Decidable.by_contradiction (dec := propDecidable _) h

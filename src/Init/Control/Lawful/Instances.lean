...
@[simp, grind =] theorem run'_eq [Functor m] (x : StateT σ m α) (s : σ) : run' x s = (·.1) <$> run x s :=
  rfl

@[simp, grind =] theorem run_pure [Pure m] (a : α) (s : σ) : (pure a : StateT σ m α).run s = pure (a, s) := rfl

@[simp, grind =] theorem run_bind [Bind m] (x : StateT σ m α) (f : α → StateT σ m β) (s : σ)
    : (x >>= f).run s = x.run s >>= λ p => (f p.1).run p.2 := rfl

@[simp, grind =] theorem run_map {α β σ : Type u} [Functor m] (f : α → β) (x : StateT σ m α) (s : σ) : (f <$> x).run s = (fun (p : α × σ) => (f p.1, p.2)) <$> x.run s := rfl

@[simp, grind =] theorem run_get [Pure m] (s : σ)    : (get : StateT σ m σ).run s = pure (s, s) := rfl

@[simp, grind =] theorem run_set [Pure m] (s s' : σ) : (set s' : StateT σ m PUnit).run s = pure (⟨⟩, s') := rfl

@[simp, grind =] theorem run_modify [Pure m] (f : σ → σ) (s : σ) : (modify f : StateT σ m PUnit).run s = pure (⟨⟩, f s) := rfl

@[simp, grind =] theorem run_modifyGet [Pure m] (f : σ → α × σ) (s : σ) : (modifyGet f : StateT σ m α).run s = pure ((f s).1, (f s).2) := rfl

@[simp, grind =] theorem run_lift {α σ : Type u} [Functor m] (x : m α) (s : σ) : (StateT.lift x : StateT σ m α).run s = (·, s) <$> x := rfl
...
@[simp, grind =] theorem run_monadLift {α σ : Type u} [Functor m] [MonadLiftT n m] (x : n α) (s : σ) : (monadLift x : StateT σ m α).run s = (·, s) <$> (monadLift x : m α) := rfl

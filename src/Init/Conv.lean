...
For recursive global definitions, it uses autogenerated "equation lemmas",
which are derived from the definition's match patterns.
Only one level of unfolding is performed, unlike `simp only [id]`, which unfolds `id` recursively.

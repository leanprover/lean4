...
/-- Split an dependent if-then-else into cases. The `split` tactic is generally easier to use than this theorem. -/
def diteInduction {c} [inst : Decidable c] {motive : α → Sort _} {t : c → α} {e : ¬ c → α}
    (hpos : (h : c) → motive (t h)) (hneg : (h : ¬c) → motive (e h)) : motive (dite c t e) :=
  match inst with
  | isTrue h => hpos h
  | isFalse h => hneg h
...
def iteInduction {c} [inst : Decidable c] {motive : α → Sort _} {t e : α}
    (hpos : c → motive t) (hneg : ¬c → motive e) : motive (ite c t e) := diteInduction hpos hneg

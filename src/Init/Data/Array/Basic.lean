...
def swap (a : Array α) (i j : @& Fin a.size) : Array α :=
  let v₁ := a.get i i.isLt
  let v₂ := a.get j j.isLt
  let a'  := a.set i v₂
  a'.set j v₁ (Nat.lt_of_lt_of_eq j.isLt (size_set a i v₂ _).symm)

@[simp] theorem size_swap (a : Array α) (i j : Fin a.size) : (a.swap i j).size = a.size := by
  show ((a.set i (a.get j j.isLt)).set j (a.get i i.isLt)
    (Nat.lt_of_lt_of_eq j.isLt (size_set a i (a.get j j.isLt) _).symm)).size = a.size
...
def indexOfAux [BEq α] (a : Array α) (v : α) (i : Nat) : Option (Fin a.size) :=
  if h : i < a.size then
    ...
    if a.get idx idx.isLt == v then some idx

...
theorem getD_get? (a : Array α) (i : Nat) (d : α) :
  ...
  if h : i < a.size then
    simp [setD, h, getElem?, decidableGetElem?]
  else
...
theorem get?_push {a : Array α} : (a.push x)[i]? = if i = a.size then some x else a[i]? := by
  ...
  | Or.inl g =>
    ...
    simp [getElem?, decidableGetElem?, size_push, g, h1, h2, get_push_lt]
  | Or.inr (Or.inl heq) =>
    simp [heq, getElem?_pos, get_push_eq]
  | Or.inr (Or.inr g) =>
    simp only [getElem?, decidableGetElem?, size_push]
...
@[simp] theorem get?_size {a : Array α} : a[a.size]? = none := by
  simp only [getElem?, decidableGetElem?, Nat.lt_irrefl, dite_false]

...
-- `List.modifyNth` is in Batteries...

-- `List.mapIdx` is down in Batteries / Mathlib
-- @[simp] theorem mapIdx_toArray (l : List α) (f : Fin l.toArray.size → α → β):
--     l.toArray.mapIdx f = (l.enum.map fun ⟨i, x⟩ => f ⟨i, sorry⟩ x).toArray := by
--   sorry

...
-/
def Array.qsort (xs : Array α) (lt : α → α → Bool := by exact (· < ·)) (sllo := 0) (slhi := xs.size)
  (hslloslhi : sllo ≤ slhi := by omega) (hslhi : slhi ≤ xs.size := by omega) : Array α :=
  letI : Ord α := ordOfLt lt
  (quicksorthelper xs.toVector sllo slhi (by omega) (by omega)).toArray

def Vector.qsort {size} (xs : Vector α size) (lt : α → α → Bool := by exact (· < ·)) (sllo := 0) (slhi := size)
  (hslloslhi : sllo ≤ slhi := by omega) (hslhi : slhi ≤ size := by omega) : Vector α size :=
  letI : Ord α := ordOfLt lt
  quicksorthelper xs sllo slhi (by omega) (by omega)

def Array.qsortOrd [Ord α] (xs : Array α) (sllo := 0) (slhi := xs.size - 1)
  (hslloslhi : sllo ≤ slhi := by omega) (hslhi : slhi ≤ xs.size := by omega) : Array α :=
  (quicksorthelper xs.toVector sllo slhi (by omega) (by omega)).toArray

def Vector.qsortOrd [Ord α] {size} (xs : Vector α size) (sllo := 0) (slhi := size)
  (hslloslhi : sllo ≤ slhi := by omega) (hslhi : slhi ≤ size := by omega) : Vector α size :=
  quicksorthelper xs sllo slhi (by omega) (by omega)

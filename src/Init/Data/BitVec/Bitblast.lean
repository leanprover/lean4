...
theorem aandRec_true_of_le_toNat_mul_toNat (x y : BitVec w) (s : Nat) (hs : s < w) (hw : 1 < w):
    2 ^ (w - 1 - (s - 1)) * 2 ^ s ≤ x.toNat * y.toNat ∧ x.toNat * y.toNat < 2 ^ (w - 1 - (s - 1) + 1) * 2 ^ (s + 1) → resRec x y s hs hw := by
  sorry

...
theorem umulOverflow_eq {w : Nat} (x y : BitVec w) :
  ...
  · simp only [ge_iff_le, show 0 < w + 1 by omega, decide_true, Bool.true_and, decide_eq_decide, mul_mod_mod]
...
theorem smulOverflow_eq {w : Nat} (x y : BitVec w) :
  ...
  · have := two_mul_toInt_lt (x := y)
    have := two_mul_toInt_lt (x := x)
    simp only [Nat.add_one_sub_one, ge_iff_le, ne_eq, show ¬w + 1 = 0 by omega,
      not_false_eq_true, decide_true, BitVec.slt, intMax, ofNat_eq_ofNat, toInt_mul, intMin, Bool.true_and]
    repeat rw [BitVec.toInt_signExtend_of_lt (by omega)]
    simp [show BitVec.twoPow (w + 1) w - 1#(w + 1) = BitVec.intMax (w + 1) by simp [intMax]]
    push_cast

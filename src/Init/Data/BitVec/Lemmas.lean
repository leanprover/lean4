    ...
    rw_mod_cast [BitVec.twoPow, BitVec.toInt_sub, BitVec.toInt_shiftLeft, BitVec.toNat_ofNat,
      ...
      Nat.mod_eq_of_lt (by simp [Nat.one_lt_two_pow]), Int.bmod_eq_of_le_of_lt
        (by
          ...
          rw [← Nat.mul_two]
          rw [Nat.mul_comm]
          have h1 := Nat.two_pow_pos (w := (w + 1))
          have := Int.le_sub_one_of_lt (a := - 2 ^ (w + 1)) (b := 2 ^ (w + 1))
          have := Int.neg_neg_of_pos (a := 2 ^ (w + 1)) (by norm_cast)
          have := Int.mul_le_mul_of_nonneg_right (a := - 2 ^ (w + 1)) (b := 2 ^ (w + 1) - 1) (c := 2) (by
            have : - 2 ^ (w + 1) < 2 ^ (w + 1) := by
              sorry
            omega
            ) (by omega)
        ...
        (by
          ...
          rw [← Nat.mul_two]
          push_cast
          have := Int.mul_le_mul_of_nonneg_right (a := 2 ^ (w + 1) - 1) (b := 2 ^ (w + 1)) (c := 2)
          omega
          )]

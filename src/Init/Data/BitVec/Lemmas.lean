...
@[simp] protected theorem add_zero (x : BitVec n) : x + 0#n = x := by simp [toNat_eq, toNat_add]

@[simp] protected theorem zero_add (x : BitVec n) : 0#n + x = x := by simp [toNat_eq, toNat_add]

...
theorem dvd_eq_false (ctx : Context) (k : Int) (p : Poly) : dvdUnsatCert k p → (k ∣ p.denote' ctx) = False := by
  simp [dvdUnsatCert]
  intro h₁ h₂
...
theorem neg_le (ctx : Context) (p₁ p₂ : Poly) : negLeCert p₁ p₂ → ¬ p₁.denote' ctx ≤ 0 → p₂.denote' ctx ≤ 0 := by
  simp [negLeCert]

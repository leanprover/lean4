...
@[defeq, grind =] protected theorem min_def (n m : Int) : min n m = if n ≤ m then n else m := rfl

@[defeq, grind =] protected theorem max_def (n m : Int) : max n m = if n ≤ m then m else n := rfl

...
def IterM.filterMapM {α β γ : Type w} {m : Type w → Type w'} {n : Type w → Type w''}
    [MonadLiftT m n] [Iterator α m β] (f : β → n (Option γ))
...
def IterM.mapM {α β γ : Type w} {m : Type w → Type w'} {n : Type w → Type w''} [Iterator α m β]
  ...
  (IterM.InternalCombinators.map (n := n) (fun ⦃_⦄ => monadLift) f it : IterM n γ)
...
def IterM.filterM {α β : Type w} {m : Type w → Type w'} {n : Type w → Type w''} [Iterator α m β]
  ...
  (it.filterMapM
    (fun b => (if ·.down = true then some b else none) <$> f b) : IterM n β)

instance FilterMap.instIterator {α β γ : Type w} {m : Type w → Type w'} {n : Type w → Type w''}
    {lift : ⦃α : Type w⦄ → m α → n α} {f : β → n (Option γ)}
...
private def FilterMap.instFinitenessRelation {α β γ : Type w} {m : Type w → Type w'}
    ...
    [Iterator α m β] {lift : ⦃α : Type w⦄ → m α → n α} [Internal.LawfulMonadLiftFunction lift]
    {f : β → n (Option γ)} [Finite α m] :
  ...
  subrelation {it it'} h := by
    ...
    letI : MonadLift m n := ⟨lift (α := _)⟩
    haveI : LawfulMonadLift m n :=
      ⟨Internal.LawfulMonadLiftFunction.lift_pure, Internal.LawfulMonadLiftFunction.lift_bind⟩
    replace hs' := LawfulMonadAttach.canReturn_liftM_imp' hs'
    cases step', hs' using PlausibleIterStep.casesOn'
    · obtain ⟨fx, hfx, h⟩ := LawfulMonadAttach.canReturn_bind_imp' h
      split at h
...
instance FilterMap.instFinite {α β γ : Type w} {m : Type w → Type w'}
    ...
    [Monad n] [MonadAttach n] [LawfulMonad n] [LawfulMonadAttach n]
    [Iterator α m β] {lift : ⦃α : Type w⦄ → m α → n α} [Internal.LawfulMonadLiftFunction lift]
    {f : β → n (Option γ)} [Finite α m] : Finite (FilterMap α m n lift f) n :=
  Finite.of_finitenessRelation FilterMap.instFinitenessRelation

@[no_expose]
instance {α β γ : Type w} {m : Type w → Type w'} {n : Type w → Type w''}
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Monad n] [MonadAttach n] [LawfulMonad n] [LawfulMonadAttach n]
    [Iterator α m β] {lift : ⦃α : Type w⦄ → m α → n α} [Internal.LawfulMonadLiftFunction lift]
    {f : β → n γ} [Finite α m] :
...
private def Map.instProductivenessRelation {α β γ : Type w} {m : Type w → Type w'}
  ...
  subrelation {it it'} h := by
    ...
    replace hs := LawfulMonadAttach.canReturn_liftM_imp' hs
    cases step, hs using PlausibleIterStep.casesOn'
    · simp only [bind_map_left, bind_pure_comp] at h
      obtain ⟨fx, hfx, h⟩ := LawfulMonadAttach.canReturn_map_imp' h
      nomatch h
    · cases LawfulMonadAttach.eq_of_canReturn_pure h
      assumption
...
instance {α β γ : Type w} {m : Type w → Type w'}
    {n : Type w → Type w''} {o : Type w → Type x} [Monad n] [MonadAttach n] [Monad o] [Iterator α m β]
...
instance Map.instIteratorLoop {α β γ : Type w} {m : Type w → Type w'}
    {n : Type w → Type w''} {o : Type x → Type x'} [Monad n] [MonadAttach n] [Monad o]
    [MonadAttach o] [Iterator α m β] {lift : ⦃α : Type w⦄ → m α → n α} {f : β → n γ} :
...
def IterM.filterMap {α β γ : Type w} {m : Type w → Type w'}
  ...
  (it.filterMapM (fun b => pure (f b)) : IterM m γ)
...
def IterM.map {α β γ : Type w} {m : Type w → Type w'} [Iterator α m β] [Monad m] (f : β → γ)
    (it : IterM (α := α) m β) :=
  (it.mapM (fun b => pure (f b)) : IterM m γ)

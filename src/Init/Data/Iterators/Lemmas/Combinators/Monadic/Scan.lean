...
theorem IterM.Intermediate.step_scanM
    ...
    (IterM.Intermediate.scanM f acc yieldAcc it).step = (do
        if h : yieldAcc = true then
          ...
          | .yield it' b hp => do
            ...
            return .deflate <| .yield
              (IterM.Intermediate.scanM f newAcc false it')
              newAcc
  ...
  simp only [IterM.Intermediate.scanM, IterM.step_eq]
  cases h : yieldAcc
  case true => rfl
  case false =>
    apply bind_congr
...
private theorem IterM.toList_scanWithPostCondition_afterInit
  ...
  simp only [IterM.Intermediate.step_scanM, ↓reduceDIte, Bool.false_eq_true]
  ...
  | ⟨.yield inner' out, hp⟩ =>
    ...
    simp +singlePass only [← List.cons_tail_scanlM]
    simp
...
private theorem IterM.toList_scan_afterInit
    ...
    IterM.toList (IterM.Intermediate.scanM (pure <| f · ·) init false it) =
  ...
  | ⟨.yield inner' out, hp⟩ =>
    ...
    simp +singlePass only [← List.cons_tail_scanl]
    simp
  | ⟨.skip inner, hp⟩ => simp_all
  | ⟨.done, x⟩ => simp_all
...
theorem IterM.toList_scanWithPostCondition [Monad m] [LawfulMonad m] [Iterator α Id β] [Finite α Id]
  ...
  rw [IterM.toList_eq_match_step, IterM.Intermediate.step_scanM]
  simp only [↓reduceDIte, pure_bind, Shrink.inflate_deflate]
  rw [toList_scanWithPostCondition_afterInit, ← List.cons_tail_scanlM]
  simp

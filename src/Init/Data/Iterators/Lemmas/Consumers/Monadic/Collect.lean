...
private theorem IterM.DefaultConsumers.toArrayMapped.go_eq [Monad m] [Monad n]
    [MonadAttach m] [MonadAttach n] [Iterator α m β] [Finite α m]
    [LawfulMonad m] [LawfulMonadAttach m] [LawfulMonad n]
    [LawfulMonadLiftFunction lift] {acc : Array γ} :
    letI : MonadLift m n := ⟨lift (δ := _)⟩
    haveI : LawfulMonadLift m n := sorry
  ...
  · simp only
    conv => rhs; rw [← LawfulMonadAttach.map_attach (x := it.step)]
...
private theorem IterM.DefaultConsumers.toArrayMapped.go.aux₁ [Monad m] [Monad n]
    [MonadAttach m] [MonadAttach n] [LawfulMonad m] [LawfulMonad n]
    [LawfulMonadAttach m] [LawfulMonadLiftFunction lift]
  ...
  rw [← LawfulMonadAttach.map_attach (x := it.step)]
  simp only [liftM_map, bind_map_left]
  apply bind_congr; intro step
  cases step using PlausibleIterStep.casesOn
...
@[simp]
theorem IterM.toArray_ensureTermination
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m] [IteratorCollect α m m] {it : IterM (α := α) m β} :
...
@[simp]
theorem IterM.toList_ensureTermination
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m] [IteratorCollect α m m] {it : IterM (α := α) m β} :
...
theorem IterM.toList_toArray_ensureTermination
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m]
...
theorem IterM.toArray_toList_ensureTermination
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m]
    [IteratorCollect α m m] {it : IterM (α := α) m β} :
...
@[simp]
theorem IterM.toListRev_ensureTermination_eq_toListRev
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m]
    {it : IterM (α := α) m β} :
...
private theorem IterM.toListRev.go_eq
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m]
  ...
  · conv => rhs; simp only [← LawfulMonadAttach.map_attach (x := it.step), bind_map_left]
    apply bind_congr; intro step
    cases step using PlausibleIterStep.casesOn <;> simp [go]
  · simp only [show (IterM.finitelyManySteps! = IterM.finitelyManySteps) by rfl]
    apply InvImage.wf
    exact WellFoundedRelation.wf

private theorem IterM.toListRev.go.aux₁
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m]
...
theorem IterM.toListRev_eq_match_step
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
...
theorem IterM.toListRev_ensureTermination_eq_match_step
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m] {it : IterM (α := α) m β} :
...
@[simp]
theorem IterM.reverse_toListRev
    [Monad m] [MonadAttach m] [LawfulMonad m] [LawfulMonadAttach m]
    [Iterator α m β] [Finite α m]

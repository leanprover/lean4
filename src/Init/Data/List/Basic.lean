...
/-- `getSuffix? l₁ l₂` returns `some t` when `l₂ == l₁ ++ t`. -/
def getSuffix? [BEq α] : List α → List α → Option (List α)
  | [], l₂ => some l₂
  | _, [] => none
  | (x₁ :: l₁), (x₂ :: l₂) =>
    if x₁ == x₂ then getSuffix? l₁ l₂ else none
...
/-- `getPrefix? l₁ l₂` returns `some t` when `l₂ == t ++ l₁`.-/
def getPrefix? [BEq α] (l₁ l₂ : List α) : Option (List α) :=
  Option.map List.reverse <| getSuffix? l₁.reverse l₂.reverse

...
theorem idxOf_singleton : idxOf b [a] = if a == b then 0 else 1 := by simp [idxOf]

theorem idxOf_getElem?_eq_some (w : xs[idxOf y xs]? ≠ none) : y ∈ xs := by simp_all [idxOf, w]

theorem idxOf_getElem {w : idxOf y xs < xs.length} : xs[idxOf y xs] = y := by
  let p := fun x => x == y
  have h : p xs[findIdx p xs] = true := findIdx_getElem
  simp only [idxOf, beq_iff_eq, p] at h
  exact h

theorem List.idxOf_lt_length_of_exists (h : x ∈ xs) : idxOf x xs < xs.length := by simp [idxOf, h]

theorem idxOf_getElem?_eq_getElem_of_exists (h : x ∈ xs) :
...
theorem idxOf_eq_length1 : ¬a ∈ l ↔ idxOf a l = l.length := by
  constructor
  · intro h
    induction l with
    | nil => rfl
...
theorem idxOf_eq_length_of_not_mem (h : ¬a ∈ l) : idxOf a l = l.length := idxOf_eq_length1.mp h

/-- Elements with indices less than xs.findIdx x are not equal to x.-/
theorem List.not_of_lt_idxOf (h : i < idxOf x xs) :
    xs[i]'(Nat.le_trans h findIdx_le_length) ≠ x := by
  have h1 : (λ a => a == x) (xs[i]'(Nat.le_trans h findIdx_le_length)) = false :=
    List.not_of_lt_findIdx h
  simp_all

/-- If xs[j] ≠ x for all j < i, then i ≤ xs.idxOf x. -/
theorem List.le_idxOf_of_not (h : i < xs.length)
...
/-- If xs[j] ≠ x for all j ≤ i, then i < xs.idxOf x. -/
theorem List.lt_idxOf_of_not (h : i < xs.length)
    (h2 : ∀ (j : Nat) (hji : j ≤ i), xs[j]'(Nat.lt_of_le_of_lt hji h) ≠ x) : i < idxOf x xs := by
...
theorem List.idxOf_eq (h : i < xs.length) :
    idxOf x xs = i ↔ xs[i] = x ∧ ∀ (j : Nat) (hji : j < i), xs[j] ≠ x := by
  let p := fun y ↦ y == x
...
theorem IsPrefix.idxOf_le
    (h : l₁ <+: l₂) : idxOf x l₁ ≤ idxOf x l₂ := by
  let p := fun y ↦ y == x
  exact List.IsPrefix.findIdx_le h

theorem IsPrefix.idxOf_eq_of_idxOf_lt_length (h : l₁ <+: l₂) (lt : idxOf x l₁ < l₁.length) :
    idxOf x l₂ = idxOf x l₁ := by
  let p := fun y ↦ y == x
  exact List.IsPrefix.findIdx_eq_of_findIdx_lt_length h lt

theorem List.idxOf_subtype {p : α → Prop} {l : List {x : α // p x}} (h : p x) :

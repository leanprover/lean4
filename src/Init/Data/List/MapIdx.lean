...
theorem mapFinIdx_eq_ofFn {as : List α} {f : Fin as.length → α → β} :
    as.mapFinIdx f = List.ofFn (fun i => f ⟨i, sorry⟩ (as[i]'sorry)) := by
  induction as with
  | nil => simp [mapFinIdx]
  | cons _ _ ih => simp [mapFinIdx, ih]

-- theorem mapFinIdx_go_append  {l₁ l₂ : List α} {arr : Array β} :
--     mapFinIdx.go (l₁ ++ l₂) f arr = mapFinIdx.go l₂ f (List.toArray (mapFinIdx.go l₁ f arr)) := by
...
theorem getElem?_mapIdx_go {as : List α} {f : Fin as.length → α → β} :
    ∀ {bs : List α} {acc : Array β} {h} {i : Nat},
    (mapFinIdx.go as f bs acc h)[i]? =
      if h : i < acc.size then some acc[i] else bs[i - acc.size]?.pbind fun x m => f ⟨i, sorry⟩ x
  | [], arr, i => by

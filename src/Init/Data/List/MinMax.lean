...
theorem max?_eq_some_iff_legacy [Max α] [LE α]
    (le_refl : ∀ a : α, a ≤ a)
    (max_eq_or : ∀ a b : α, max a b = a ∨ max a b = b)
    (max_le_iff : ∀ a b c : α, max a b ≤ c ↔ a ≤ c ∧ b ≤ c) {xs : List α}
    (anti : ∀ a b, a ∈ xs → b ∈ xs → a ≤ b → b ≤ a → a = b := by
      exact fun a b _ _ => Std.Antisymm.antisymm a b) :
    xs.max? = some a ↔ a ∈ xs ∧ ∀ b, b ∈ xs → b ≤ a := by
  -- TODO: Extract the order on Subtype (· ∈ xs) into separate declarations so that they can
  ...
  haveI : LinearOrder (Subtype (· ∈ xs)) := by
    ...
    · exact fun a b hab hba => Subtype.ext <| anti a.val b.val a.property b.property hab hba
    · intro a b c hab hbc
      have : max a.val b.val = b.val := by
        apply anti _ _ (MaxEqOr.elim a.property b.property) b.property
        · refine MaxEqOr.elim (P := (· ≤ b.val)) hab (le_refl _)
        · exact ((max_le_iff _ _ (max a.val b.val)).mp (le_refl _)).2
      specialize max_le_iff a b c
      rw [this] at max_le_iff
      exact (max_le_iff.mp hbc).1
    · exact fun a b => by
        cases max_eq_or a.val b.val
        case inl h => exact Or.inr ((h ▸ max_le_iff a.val b.val a.val).mp (le_refl _)).2
        case inr h => exact Or.inl ((h ▸ max_le_iff a.val b.val b.val).mp (le_refl _)).1
  haveI : LawfulOrderSup (Subtype (· ∈ xs)) := by
    refine ⟨fun a b c => ?_⟩
    simpa [OrderData.IsLE, LawfulOrderLE.le_iff] using max_le_iff a b c
  haveI : MaxEqOr (Subtype (· ∈ xs)) := by
    refine ⟨fun a b => ?_⟩
    simpa [Max.max, Subtype.ext_iff] using max_eq_or a b
  haveI : LawfulOrderMax (Subtype (· ∈ xs)) := ⟨⟩
  exact max?_eq_some_iff_subtype (α := α) (xs := xs) (a := a)

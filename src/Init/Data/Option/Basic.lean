...
@[instance_reducible] def getM [Alternative m] : Option α → m α
  | none     => failure
  | some a   => pure a
...
@[inline, instance_reducible] def isSome : Option α → Bool
  | some _ => true
  | none   => false
...
-/
@[inline, instance_reducible] def isNone : Option α → Bool
  | some _ => false
  | none   => true
...
-/
@[inline, instance_reducible] protected def bind : Option α → (α → Option β) → Option β
  | none,   _ => none
  | some a, f => f a
...
-/
@[always_inline, inline, instance_reducible] protected def filter (p : α → Bool) : Option α → Option α
...
-/
@[always_inline, inline, instance_reducible] protected def all (p : α → Bool) : Option α → Bool
  | some a => p a
  | none   => true
...
-/
@[always_inline, inline, instance_reducible] protected def any (p : α → Bool) : Option α → Bool
  | some a => p a
  | none   => false
...
-/
@[inline, instance_reducible] def guard (p : α → Bool) (a : α) : Option α :=
...
-/
@[inline, instance_reducible] def join (x : Option (Option α)) : Option α := x.bind id

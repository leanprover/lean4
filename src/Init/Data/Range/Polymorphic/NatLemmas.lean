...
@[simp] -- Oof, this nice simp lemma causes non-confluence
theorem toList_rio_succ {n : Nat} :
    (*...(n+1)).toList = (*...n).toList ++ [n] := by
  rw [toList_rio_eq_if, if_pos (by omega)]
  simp

theorem toList_rio_succ_right_eq_cons_map {m n : Nat} (h : m ≤ n) :
    (m...(n + 1)).toList = m :: (m...n).toList.map (· + 1) := by
  rw [Rco.toList_eq_if_roo, if_pos (Nat.lt_succ_iff.mpr h), Roo.toList_eq_match_rio]
  simp [toList_rio_succ_succ]

@[simp]
theorem toList_rio_eq_nil_iff {m n : Nat} :
    (m...n).toList = [] ↔ n ≤ m := by
  simp [Rco.toList_eq_if_roo]

theorem toList_rio_ne_nil_iff {m n : Nat} :
    (m...n).toList ≠ [] ↔ m < n := by
  simp

@[simp]
theorem toList_rio_eq_nil {m n : Nat} (h : n ≤ m) :
    (m...n).toList = [] := by
  simp [h]

@[simp]
theorem toList_rio_eq_singleton_iff {m n : Nat} :
    (m...n).toList = [k] ↔ n = m + 1 ∧ m = k := by
  rw [toList_rio_eq_if]
  split <;> (simp; omega)

@[simp]
theorem toList_rio_eq_singleton {m n : Nat} (h : n = m + 1) :
    (m...n).toList = [m] := by
  simp [h]

@[simp]
theorem toList_rio_eq_cons_iff {m n a : Nat} :
    (m...n).toList = a :: xs ↔ m = a ∧ m < n ∧ xs = ((m + 1)...n).toList := by
  rw [Rco.toList_eq_if_roo]
  split <;> simp +contextual [*, Roo.toList_eq_match_rio, eq_comm]

@[simp]
theorem toList_rio_eq_cons {m n : Nat} (h : m < n) :
    (m...n).toList = m :: ((m + 1)...n).toList := by
  simp [h]
...
theorem map_add_toList_rio {m n k : Nat} :
    (m...n).toList.map (· + k) = ((m + k)...(n + k)).toList := by
  induction k
  · simp
  · rename_i k ih
    simp [← Nat.add_assoc, toList_rio_succ_succ, ← ih]

theorem map_add_toList_rio' {m n k : Nat} :
    (m...n).toList.map (k + ·) = ((k + m)...(k + n)).toList := by
  simp [Nat.add_comm, ← map_add_toList_rio]

theorem toList_rio_add_right_eq_map {m n : Nat} :
    (m...(m + n)).toList = (0...n).toList.map (· + m) := by
  rw (occs := [1]) [← Nat.zero_add m]
  simp [map_add_toList_rio', Nat.add_comm _ m]

theorem toList_rio_add_succ_right_eq_append {m n : Nat} :
    (m...(m + n + 1)).toList = (m...(m + n)).toList ++ [m + n] := by
  induction n generalizing m
  · simp
  · rename_i n ih
    rw [toList_rio_eq_cons (by omega), toList_rio_eq_cons (m := m) (by omega),
      show m + (n + 1) + 1 = (m + 1) + n + 1 by omega, ih]
    simp [show m + 1 + n = m + (n + 1) by omega]

theorem toList_rio_succ_right_eq_append {m n : Nat} (h : m ≤ n) :
    (m...(n + 1)).toList = (m...n).toList ++ [n] := by
  obtain ⟨k, rfl⟩ := Nat.exists_eq_add_of_le h
  simp [toList_rio_add_succ_right_eq_append]

theorem toList_rio_add_succ_right_eq_append' {m n : Nat} :
    (m...(m + (n + 1))).toList = (m...(m + n)).toList ++ [m + n] := by
  simp [toList_rio_add_succ_right_eq_append, ← Nat.add_assoc]

theorem toList_rio_eq_append {m n : Nat} (h : m < n) :
    (m...n).toList = (m...(n - 1)).toList ++ [n - 1] := by
  obtain ⟨k, rfl⟩ := Nat.exists_eq_add_of_lt h
  simp [toList_rio_add_succ_right_eq_append]

theorem toList_rio_add_add_eq_append {m n k : Nat} :
    (m...(m + n + k)).toList = (m...(m + n)).toList ++ ((m + n)...(m + n + k)).toList := by
  induction k
  · simp
  · rename_i k ih
    rw [toList_rio_add_succ_right_eq_append', ← List.append_assoc, ← ih, toList_rio_eq_append]
    · simp
    · omega

theorem toList_rio_append_toList_rio {l m n : Nat} (h : l ≤ m) (h' : m ≤ n) :
    (l...m).toList ++ (m...n).toList = (l...n).toList := by
  obtain ⟨_, rfl⟩ := Nat.exists_eq_add_of_le h
  obtain ⟨_, rfl⟩ := Nat.exists_eq_add_of_le h'
  simp [toList_rio_add_add_eq_append]

@[simp]
theorem getElem_toList_rio {m n i : Nat} (_h : i < (m...n).toList.length) :
    (m...n).toList[i]'_h = m + i := by
  simp [Rco.getElem_toList_eq]

theorem getElem?_toList_rio {m n i : Nat} :
    (m...n).toList[i]? = if i < n - m then some (m + i) else none := by
  simp [Rco.getElem?_toList_eq, Option.filter_some, ← Nat.lt_sub_iff_add_lt']

@[simp]
theorem getElem?_toList_rio_eq_some_iff {m n i : Nat} :
    (m...n).toList[i]? = some k ↔ i < n - m ∧ k = m + i := by
  simp [getElem?_toList_rio, eq_comm]

@[simp]
theorem getElem?_toList_rio_eq_none_iff {m n i : Nat} :
    (m...n).toList[i]? = none ↔ n ≤ i + m := by
  simp [getElem?_toList_rio]

@[simp]
theorem isSome_getElem?_toList_rio_eq {m n i : Nat} :
    (m...n).toList[i]?.isSome = (i < n - m) := by
  simp

@[simp]
theorem isNone_getElem?_toList_rio_eq {m n i : Nat} :
    (m...n).toList[i]?.isNone = (n ≤ i + m) := by
  simp

@[simp]
theorem getElem?_toList_rio_eq_some {m n i : Nat} (h : i < n - m) :
    (m...n).toList[i]? = some (m + i) := by
  simp [h]

@[simp]
theorem getElem?_toList_rio_eq_none {m n i : Nat} (h : n ≤ i + m) :
    (m...n).toList[i]? = none := by
  simp [h]

theorem getElem!_toList_rio {m n i : Nat} :
    (m...n).toList[i]! = if i < n - m then (m + i) else 0 := by
  simp only [List.getElem!_eq_getElem?_getD, getElem?_toList_rio, Nat.default_eq_zero]
  split <;> simp

@[simp]
theorem getElem!_toList_rio_eq_add {m n i : Nat} (h : i < n - m) :
    (m...n).toList[i]! = m + i := by
  simp [h]

@[simp]
theorem getElem!_toList_rio_eq_zero {m n i : Nat} (h : n ≤ i + m) :
    (m...n).toList[i]! = 0 := by
  simp [h]

theorem getElem!_toList_rio_eq_zero_iff {m n i : Nat} :
    (m...n).toList[i]! = 0 ↔ n ≤ i + m ∨ (m = 0 ∧ i = 0) := by
  simp only [List.getElem!_eq_getElem?_getD, getElem?_toList_rio, Nat.default_eq_zero]
  split <;> (simp; omega)

theorem zero_lt_getElem!_toList_rio_iff {m n i : Nat} :
    0 < (m...n).toList[i]! ↔ i < n - m ∧ 0 < i + m := by
  simp only [List.getElem!_eq_getElem?_getD, getElem?_toList_rio, Nat.default_eq_zero]
  split <;> (simp; omega)

theorem getD_toList_rio {m n i fallback : Nat} :
    (m...n).toList.getD i fallback = if i < n - m then (m + i) else fallback := by
  by_cases h : i < n - m <;> simp [h]

@[simp]
theorem getD_toList_rio_eq_add {m n i fallback : Nat} (h : i < n - m) :
    (m...n).toList.getD i fallback = m + i := by
  simp [h]

@[simp]
theorem getD_toList_rio_eq_fallback {m n i fallback : Nat} (h : n ≤ i + m) :
    (m...n).toList.getD i fallback = fallback := by
  simp [h]
...
The following is an example reproving {name}`length_toList_rio`.

```lean
example (a b : Nat) : (a...b).toList.length = b - a := by
  induction a, b using Std.PRange.Nat.induct_rio_left
  case base =>
    simp only [Std.PRange.Nat.toList_rio_eq_nil, List.length_nil, Nat.sub_eq_zero_of_le, *]
  case step =>
    simp only [Std.PRange.Nat.toList_rio_eq_cons, List.length_cons, *]; omega
```
-/
theorem induct_rio_left (motive : Nat → Nat → Prop)
    (base : ∀ a b, b ≤ a → motive a b)
    (step : ∀ a b, a < b → motive (a + 1) b → motive a b)
    (a b : Nat) : motive a b := by
  induction h : b - a generalizing a b
...
```lean
example (a b : Nat) : (a...b).toList.length = b - a := by
  induction a, b using Std.PRange.Nat.induct_rio_right
  case base =>
    simp only [Std.PRange.Nat.toList_rio_eq_nil, List.length_nil, Nat.sub_eq_zero_of_le, *]
  case step a b hle ih =>
    rw [Std.PRange.Nat.toList_rio_eq_append (by omega),
      List.length_append, List.length_singleton, Nat.add_sub_cancel, ih]
    omega
```
-/
theorem induct_rio_right (motive : Nat → Nat → Prop)
    (base : ∀ a b, b ≤ a → motive a b)
    (step : ∀ a b, a ≤ b → motive a b → motive a (b + 1))
    (a b : Nat) : motive a b := by
  induction h : b - a generalizing a b
...
@[simp]
theorem toList_ric_eq_toList_rio {m n : Nat} :
    (m...=n).toList = (m...(n + 1)).toList := by
  simp

theorem toList_ric_eq_if {m n : Nat} :
    (m...=n).toList = if m ≤ n then m :: ((m + 1)...=n).toList else [] := by
  rw [toList_ric_eq_toList_rio, toList_ric_eq_toList_rio, toList_rio_eq_if]
  split <;> (simp; omega)

theorem toList_ric_succ_succ {m n : Nat} :
    ((m+1)...=(n+1)).toList = (m...=n).toList.map (· + 1) := by
  simp [← succ_eq, Rco.toList_succ_succ_eq_map]

theorem toList_ric_succ_right_eq_cons_map {m n : Nat} (h : m ≤ n + 1) :
    (m...=(n + 1)).toList = m :: (m...=n).toList.map (· + 1) := by
  simp [toList_rio_succ_succ]; omega

@[simp]
theorem toList_ric_eq_nil_iff {m n : Nat} :
    (m...=n).toList = [] ↔ n < m := by
  simp; omega

theorem toList_ric_ne_nil_iff {m n : Nat} :
    (m...=n).toList ≠ [] ↔ m ≤ n := by
  simp; omega

@[simp]
theorem toList_ric_eq_nil {m n : Nat} (h : n < m) :
    (m...=n).toList = [] := by
  simp; omega

@[simp]
theorem toList_ric_eq_singleton_iff {m n : Nat} :
    (m...=n).toList = [k] ↔ n = m ∧ m = k := by
  simp; omega

@[simp]
theorem toList_ric_eq_singleton {m n : Nat} (h : n = m) :
    (m...=n).toList = [m] := by
  simp [h]

@[simp]
theorem toList_ric_eq_cons_iff {m n a : Nat} :
    (m...=n).toList = a :: xs ↔ m = a ∧ m ≤ n ∧ xs = ((m + 1)...=n).toList := by
  simp; omega

@[simp]
theorem toList_ric_eq_cons {m n : Nat} (h : m ≤ n) :
    (m...=n).toList = m :: ((m + 1)...=n).toList := by
  simp; omega

theorem map_add_toList_ric {m n k : Nat} :
    (m...=n).toList.map (· + k) = ((m + k)...=(n + k)).toList := by
  simp [map_add_toList_rio, show n + 1 + k = n + k + 1 by omega]

theorem map_add_toList_ric' {m n k : Nat} :
    (m...=n).toList.map (k + ·) = ((k + m)...=(k + n)).toList := by
  simp [map_add_toList_rio', ← Nat.add_assoc]

theorem toList_ric_add_right_eq_map {m n : Nat} :
    (m...=(m + n)).toList = (0...=n).toList.map (· + m) := by
  simp [show m + n + 1 = (m + 1) + n by omega, toList_rio_add_right_eq_map, toList_rio_succ_succ,
    show ∀ a, a + 1 + m = a + (m + 1) by omega]
  omega

theorem toList_ric_add_succ_right_eq_append {m n : Nat} :
    (m...=(m + n + 1)).toList = (m...=(m + n)).toList ++ [m + n + 1] := by
  simp only [Rcc.toList_eq_toList_rio, succ_eq]
  rw [toList_rio_eq_append (by omega)]
  simp

theorem toList_ric_eq_append {m n : Nat} (h : m ≤ n) (h' : 0 < n) :
    (m...=n).toList = (m...=(n - 1)).toList ++ [n] := by
  simp only [toList_ric_eq_toList_rio]
  rw [toList_rio_eq_append (by omega), Nat.sub_add_cancel (by omega)]
  simp

theorem toList_ric_succ_right_eq_append {m n : Nat} (h : m ≤ n + 1) :
    (m...=(n + 1)).toList = (m...=n).toList ++ [n + 1] := by
  rw [toList_ric_eq_append (by omega) (by omega)]
  simp
...
@[simp]
theorem getElem_toList_ric {m n i : Nat} (_h : i < (m...=n).toList.length) :
    (m...=n).toList[i]'_h = m + i := by
  simp

theorem getElem?_toList_ric {m n i : Nat} :
    (m...=n).toList[i]? = if i < n + 1 - m then some (m + i) else none := by
  simp [getElem?_toList_rio]

@[simp]
theorem getElem?_toList_ric_eq_some_iff {m n i : Nat} :
    (m...=n).toList[i]? = some k ↔ i < n + 1 - m ∧ k = m + i := by
  simp [getElem?_toList_rio, eq_comm]

@[simp]
theorem getElem?_toList_ric_eq_none_iff {m n i : Nat} :
    (m...=n).toList[i]? = none ↔ n + 1 ≤ i + m := by
  simp [getElem?_toList_rio]

@[simp]
theorem isSome_getElem?_toList_ric_eq {m n i : Nat} :
    (m...=n).toList[i]?.isSome = (i < n + 1 - m) := by
  simp

@[simp]
theorem isNone_getElem?_toList_ric_eq {m n i : Nat} :
    (m...=n).toList[i]?.isNone = (n + 1 ≤ i + m) := by
  simp

@[simp]
theorem getElem?_toList_ric_eq_some {m n i : Nat} (h : i < n + 1 - m) :
    (m...=n).toList[i]? = some (m + i) := by
  simp [h]

@[simp]
theorem getElem?_toList_ric_eq_none {m n i : Nat} (h : n + 1 ≤ i + m) :
    (m...=n).toList[i]? = none := by
  simp [h]

theorem getElem!_toList_ric {m n i : Nat} :
    (m...=n).toList[i]! = if i < n + 1 - m then (m + i) else 0 := by
  simp only [toList_ric_eq_toList_rio, getElem!_toList_rio]

@[simp]
theorem getElem!_toList_ric_eq_add {m n i : Nat} (h : i < n + 1 - m) :
    (m...=n).toList[i]! = m + i := by
  simp [h]

@[simp]
theorem getElem!_toList_ric_eq_zero {m n i : Nat} (h : n + 1 ≤ i + m) :
    (m...=n).toList[i]! = 0 := by
  simp [h]

theorem getElem!_toList_ric_eq_zero_iff {m n i : Nat} :
    (m...=n).toList[i]! = 0 ↔ n + 1 ≤ i + m ∨ (m = 0 ∧ i = 0) := by
  simp only [toList_ric_eq_toList_rio, getElem!_toList_rio_eq_zero_iff]

theorem zero_lt_getElem!_toList_ric_iff {m n i : Nat} :
    0 < (m...=n).toList[i]! ↔ i < n + 1 - m ∧ 0 < i + m := by
  simp only [toList_ric_eq_toList_rio, zero_lt_getElem!_toList_rio_iff]

theorem getD_toList_ric {m n i fallback : Nat} :
    (m...=n).toList.getD i fallback = if i < n + 1 - m then (m + i) else fallback := by
  by_cases h : i < n + 1 - m <;> simp [h]

@[simp]
theorem getD_toList_ric_eq_add {m n i fallback : Nat} (h : i < n + 1 - m) :
    (m...=n).toList.getD i fallback = m + i := by
  simp [h]

@[simp]
theorem getD_toList_ric_eq_fallback {m n i fallback : Nat} (h : n + 1 ≤ i + m) :
    (m...=n).toList.getD i fallback = fallback := by
  simp [h]
...
The following is an example reproving {name}`length_toList_ric`.

```lean
example (a b : Nat) : (a...=b).toList.length = b + 1 - a := by
  induction a, b using Std.PRange.Nat.induct_ric_left
  case base =>
    simp only [Std.PRange.Nat.toList_ric_eq_nil, List.length_nil, *]; omega
  case step =>
    simp only [Std.PRange.Nat.toList_ric_eq_cons, List.length_cons, *]; omega
```
-/
theorem induct_ric_left (motive : Nat → Nat → Prop)
    (base : ∀ a b, b < a → motive a b)
    (step : ∀ a b, a ≤ b → motive (a + 1) b → motive a b)
    (a b : Nat) : motive a b := by
  induction h : b + 1 - a generalizing a b
...
```lean
example (a b : Nat) : (a...=b).toList.length = b + 1 - a := by
  induction a, b using Std.PRange.Nat.induct_ric_right
  case base a b hge =>
    by_cases h : b < a
    · simp only [Std.PRange.Nat.toList_ric_eq_nil, List.length_nil, *]
      omega
    · have : b = a := by omega
      simp only [Std.PRange.Nat.toList_ric_eq_singleton, List.length_singleton,
        Nat.add_sub_cancel_left, *]
  case step a b hle ih =>
    rw [Std.PRange.Nat.toList_ric_eq_append (by omega),
      List.length_append, List.length_singleton, Nat.add_sub_cancel, ih] <;> omega
```
-/
theorem induct_ric_right (motive : Nat → Nat → Prop)
    (base : ∀ a b, b ≤ a → motive a b)
    (step : ∀ a b, a ≤ b → motive a b → motive a (b + 1))
    (a b : Nat) : motive a b := by
  induction h : b - a generalizing a b

...
inductive Expr.InSemiring : Expr → Prop where
  | num  (k : Int) : k ≥ 0 → InSemiring (.num k)
  | var  (x : Var) : InSemiring (.var x)
  | add  {a b : Expr} : a.InSemiring → b.InSemiring → InSemiring (.add a b)
  | mul {a b : Expr} : a.InSemiring → b.InSemiring → InSemiring (.mul a b)
  | pow {a : Expr} (k : Nat) : a.InSemiring → InSemiring (.pow a k)

def Expr.inSemiring : Expr → Bool
  | .num k => k ≥ 0
  | .var _ => true
  | .mul a b | .add a b => a.inSemiring && b.inSemiring
  | .pow a _ => a.inSemiring
  | .neg .. | .sub .. => false

theorem Expr.InSemiring_of_inSemiring (e : Expr) : e.inSemiring → e.InSemiring := by
  induction e <;> simp [inSemiring]
  next => intro h; exact InSemiring.num _ h
  next => exact InSemiring.var _
  next ih₁ ih₂ => intro h₁ h₂; exact InSemiring.add (ih₁ h₁) (ih₂ h₂)
  next ih₁ ih₂ => intro h₁ h₂; exact InSemiring.mul (ih₁ h₁) (ih₂ h₂)
  next ih => intro h; exact InSemiring.pow _ (ih h)
...
theorem Expr.toPoly_NonnegCoeffs {e : Expr}
    : e.InSemiring → e.toPoly.NonnegCoeffs := by
  fun_induction toPoly <;> intro h <;> cases h
  ...
  next ih₁ ih₂ h₁ h₂ =>
    replace ih₁ := ih₁ h₁
    replace ih₂ := ih₂ h₂
    apply Poly.mul_NonnegCoeffs <;> assumption
  next h => cases h; constructor; apply Int.pow_nonneg; assumption
  next h => cases h; constructor; decide; constructor; decide
  next ih h =>
    replace ih := ih h
    apply Poly.pow_NonnegCoeffs; assumption

theorem Expr.denoteS_toPoly {α} [CommSemiring α] (ctx : Context α) (e : Expr)
   : e.InSemiring →  e.toPoly.denoteS ctx = e.denoteS ctx := by
  fun_induction toPoly <;> intro h <;> cases h
    <;> simp [denoteS, Poly.denoteS, Poly.denoteS_ofVar]
  next ih₁ ih₂ h₁ h₂ =>
    rw [Poly.denoteS_combine, ih₁ h₁, ih₂ h₂]
    all_goals apply toPoly_NonnegCoeffs; assumption
  next ih₁ ih₂ h₁ h₂ =>
    rw [Poly.denoteS_mul, ih₁ h₁, ih₂ h₂]
    all_goals apply toPoly_NonnegCoeffs; assumption
  next h =>
    cases h; rw [Int.toNat_pow_of_nonneg, natCast_pow]; assumption
  next =>
    simp [Poly.ofMon, Poly.denoteS, denoteSInt_eq, Power.denote_eq, Mon.denote]
  next ih h =>
    rw [Poly.denoteS_pow, ih h]
    apply toPoly_NonnegCoeffs; assumption

def eq_normS_cert (lhs rhs : Expr) : Bool :=
  lhs.inSemiring && rhs.inSemiring && lhs.toPoly == rhs.toPoly

theorem eq_normS {α} [CommSemiring α] (ctx : Context α) (lhs rhs : Expr)
    : eq_normS_cert lhs rhs → lhs.denoteS ctx = rhs.denoteS ctx := by
  simp [eq_normS_cert]; intro h₁ h₂ h₃
  replace h₁ := lhs.InSemiring_of_inSemiring h₁
  replace h₂ := rhs.InSemiring_of_inSemiring h₂

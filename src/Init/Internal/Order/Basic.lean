...
This is intended to be used in the construction of `partial_fixpoint`, and not meant to be used otherwise.
-/
@[semireducible] def monotone (f : α → β) : Prop := ∀ x y, x ⊑ y → f x ⊑ f y
...
This is intended to be used in the construction of `partial_fixpoint`, and not meant to be used otherwise.
-/
@[semireducible] def FlatOrder {α : Sort u} (b : α) := α
...
@[semireducible] def ImplicationOrder := Prop
...
@[semireducible] def ReverseImplicationOrder := Prop

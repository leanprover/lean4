...
-/
@[suggest_for ℕ, override_runtime_type tobj]
inductive Nat where
  /--
...
/-- The implementation of `Nat.zero` for the compiler. -/
@[inline] def Nat.zeroImpl : Nat := 0

@[csimp]
theorem Nat.zero_eq_zeroImpl : Eq Nat.zero Nat.zeroImpl := rfl
...
/-- The implementation of `Nat.succ` for the compiler. -/
@[inline] def Nat.succImpl (n : Nat) : Nat := n.add 1

@[csimp]
theorem Nat.succ_eq_succImpl : Eq Nat.succ Nat.succImpl := rfl
...
/-- The implementation of `Nat.casesOn` for the compiler. -/
@[macro_inline] def Nat.casesOnImpl {motive : Nat → Sort u} (t : Nat)
    (zero : motive .zero) (succ : (n : Nat) → motive (.succ n)) : motive t :=
  dite (Eq t 0)
    (fun h => Eq.rec zero h.symm)
    (fun h =>
      haveI : Eq (Nat.succ (t.sub 1)) t :=
        match t, h with
        | Nat.succ _, _ => rfl
      Eq.rec (motive := fun t _ => motive t) (succ (t.sub 1)) this)

@[csimp]
theorem Nat.casesOn_eq_casesOnImpl : Eq @Nat.casesOn @Nat.casesOnImpl :=
  funext fun _motive => funext fun t => funext fun _zero => funext fun _succ =>
    match t with
    | Nat.zero => rfl
    | Nat.succ _ => rfl

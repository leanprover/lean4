...
@[csimp]
theorem _root_.Bool.and_eq_internalAnd : Eq Bool.and Bool.Internal.and :=
  funext (fun x => match x with | true | false => rfl)

@[csimp]
theorem _root_.Bool.or_eq_internalOr : Eq Bool.or Bool.Internal.or :=
  funext (fun x => match x with | true | false => rfl)

@[csimp]
theorem _root_.Bool.not_eq_internalNot : Eq Bool.not Bool.Internal.not :=
  funext (fun x => match x with | true | false => rfl)

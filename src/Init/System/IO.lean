...
-/
@[expose, irreducible] def BaseIO (α : Type) := ST IO.RealWorld α

attribute [local semireducible] BaseIO
...
/- TODO(Leo): mark it as an opaque definition. Reason: prevent
   ...
   ```
-/
@[expose, irreducible] def EIO (ε : Type) (α : Type) : Type := EST ε IO.RealWorld α

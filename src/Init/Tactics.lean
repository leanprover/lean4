...
The legacy multi-location syntax `rwa [rules] at h₁ h₂` is deprecated. Use
`rw [rules] at h₁ h₂ <;> assumption` instead.
-/
...
@[inherit_doc rwa, tactic_alt rwa]
macro (name := rwaAtMany) "rwa " rws:rwRuleSeq " at " h:term:max
    hs:(ppSpace colGt term:max)+ : tactic =>
  `(tactic| (rw $rws:rwRuleSeq at $h:term $hs:term*; assumption))

deprecated_syntax Lean.Parser.Tactic.rwaAtMany
  "use `rw [...] at h₁ h₂ <;> assumption` instead" (since := "2026-08-27")

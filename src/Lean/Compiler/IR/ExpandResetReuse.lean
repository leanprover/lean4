...
partial def computeProjCounts (bs : Array FnBody) : ProjCounts :=
  let incrementCountIfProj r b :=
    if let .vdecl _ _ (.proj i v) _ := b then
      ...
      r
  bs.foldl incrementCountIfProj Std.HashMap.empty

/-- Auxiliary function for eraseProjIncFor -/
partial def eraseProjIncForAux (y : VarId) (bs : Array FnBody) (projCounts : ProjCounts) (mask : Mask) (keep : Array FnBody) : Array FnBody × Mask :=

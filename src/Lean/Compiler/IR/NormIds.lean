...
def normExpr : Expr → M Expr
  ...
  | Expr.uproj i s x,    m => Expr.uproj i s (normVar x m)
...
partial def normFnBody : FnBody → N FnBody
  ...
  | FnBody.uset x i y s b   => return FnBody.uset (← normVar x) i (← normVar y) s (← normFnBody b)
...
def mapExpr (f : VarId → VarId) : Expr → Expr
  ...
  | Expr.uproj i s x    => Expr.uproj i s (f x)
...
partial def mapFnBody (f : VarId → VarId) : FnBody → FnBody
  ...
  | FnBody.uset x i y s b        => FnBody.uset (f x) i (f y) s (mapFnBody f b)

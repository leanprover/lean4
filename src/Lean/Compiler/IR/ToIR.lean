    ...
    if let some decl ← LCNF.getMonoDecl? name then
        ...
        if isExtern env name then
          ...
          for field in fields do
            ...
            | .var .. =>
              match field with
              | .object .. =>
                objArgs := objArgs.push irArg
              | .usize .. =>
                usizeArgs := usizeArgs.push irArg
              | .scalar _ offset argType =>
                scalarArgs := scalarArgs.push { offset, type := argType, arg := irArg }
              | .irrelevant => pure ()
            | .irrelevant => pure ()
            i := i + 1
      ...
      | some (.axiomInfo ..) =>
        if name == ``Quot.lcInv then
          ...
          throwError f!"axiom {name} unsupported by code generator"
      | some (.quotInfo ..) =>

...
/--
Return `true` if `declName` is a `.het` auxiliary declaration generated by `mkCasesOnSameCtor`.
These are identified by having name `parent ++ "het"` where `parent` is a registered matcher.
-/
def isCasesOnSameCtorHet (env : Environment) (declName : Name) : Bool :=
  match declName with
  | .str parent "het" => Meta.getMatcherInfoCore? env parent |>.isSome
  | _ => false

/--
Inline `.het` auxiliary declarations generated by `mkCasesOnSameCtor`.
These are exposed after `inlineMatchers` unfolds the parent `match_on_same_ctor` matcher.
-/
def inlineCasesOnSameCtorHet (e : Expr) : CoreM Expr :=
  Core.transform e fun e => do
    let .const declName us := e.getAppFn | return .continue
    unless isCasesOnSameCtorHet (← getEnv) declName do return .continue
    let info ← getConstInfo declName
...
def toDecl (declName : Name) : CompilerM (Decl .pure) := do
  ...
  if let some externAttrData := getExternAttrData? env declName then
    ...
    let (type, value) ← Meta.MetaM.run' do
      ...
      /- `inlineMatchers` may expose `.het` auxiliaries from `mkCasesOnSameCtor`. -/
      let value ← inlineCasesOnSameCtorHet value

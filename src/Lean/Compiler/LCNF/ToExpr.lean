...
@[inline] def mkLambdaM (params : Array (Param ph)) (e : Expr) : ToExprM Expr :=
...
@[inline] partial def withParams (params : Array (Param ph)) (k : ToExprM α) : ToExprM α :=
  go 0
where
...
private def Arg.toExprM (arg : Arg ph) : ToExprM Expr :=
  return arg.toExpr.abstract' (← read) (← get)

mutual
partial def FunDecl.toExprM (decl : FunDecl ph) : ToExprM Expr :=
  withParams decl.params do mkLambdaM decl.params (← decl.value.toExprM)

partial def Code.toExprM (code : Code ph) : ToExprM Expr := do
  match code with
  | .let decl k =>
...
public def Code.toExpr (code : Code ph) (xs : Array FVarId := #[]) : Expr :=
  run' code.toExprM xs

public def FunDecl.toExpr (decl : FunDecl ph) (xs : Array FVarId := #[]) : Expr :=
  run' decl.toExprM xs

...
private def isBodyRelevant (decl : Decl) : CompilerM Bool := do
  -- let opts := (← getOptions)
  decl.isTemplateLike <||>
    pure (`_at_ ∈ decl.name.components)
    -- <||> decl.value.isCodeAndM (pure <| ·.sizeLe (compiler.small.get opts))

def inferVisibilities (decls : Array Decl) : CompilerM Unit := do
  ...
  for decl in decls do
    if metaExt.isTagged (← getEnv) decl.name then
      ...
      if (← isBodyRelevant decl) then
        trace[Compiler.LCNF.inferVisibility] m!"Marking {decl.name} as transparent because it is already opaque and its body looks relevant"
        modifyEnv (bumpDeclVisibility · decl.name .transparent)
        decl.value.forCodeM fun code =>
          for ref in collectUsedDecls code do
            if getDeclVisibility (← getEnv) ref != .private then
              continue
            trace[Compiler.LCNF.inferVisibility] m!"Marking {ref} as opaque because it is in template closure"
            modifyEnv (bumpDeclVisibility · ref .opaque)
      else

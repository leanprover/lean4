...
public def mkModuleInitializationStem (moduleName : Name) (pkg? : Option PkgId := none) : String :=
  -- This works because of parity: Names generated by `mangleAux` always have an even number of
  -- preceding underscores
  match pkg? with
  | none => moduleName.mangle ""
  | some pkg => (Name.mkStr1 pkg |>.appendCore moduleName).mangle "_"
...
public def mkPackageSymbolName (declName : Name) (pkg? : Option PkgId := none) : String :=
  match pkg? with

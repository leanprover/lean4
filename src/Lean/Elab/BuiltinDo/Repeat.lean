...
@[builtin_doElem_elab Lean.Parser.Term.doRepeat] def elabDoRepeat : DoElab := fun stx dec => do
  let `(doElem| repeat%$tk $[$inv?:doLoopInvariant]? $[$var?:doDecreasing]? $seq) := stx
...
@[builtin_macro Lean.Parser.Term.doWhile] def expandDoWhile : Macro
  | `(doElem| while%$tk $cond:doIfCond $[$inv?:doLoopInvariant]? $[$dec?:doDecreasing]? do $seq) =>
    `(doElem| repeat%$tk $[$inv?:doLoopInvariant]? $[$dec?:doDecreasing]?
        if $cond:doIfCond then $seq else break)
  | _ => Macro.throwUnsupported

@[builtin_macro Lean.Parser.Term.doRepeatUntil] def expandDoRepeatUntil : Macro
  | `(doElem| repeat%$tk $[$inv?:doLoopInvariant]? $[$dec?:doDecreasing]? $seq until $cond) =>
    `(doElem| repeat%$tk $[$inv?:doLoopInvariant]? $[$dec?:doDecreasing]?
        do $seq:doSeq
        if $cond then break)
  | _ => Macro.throwUnsupported

...
def mkDefViewOfAbbrev (modifiers : Modifiers) (stx : Syntax) : DefView :=
  ...
  { ref := stx, headerRef := mkNullNode stx.getArgs[*...3].copy, kind := DefKind.abbrev, modifiers,
...
def mkDefViewOfDef (modifiers : Modifiers) (stx : Syntax) : DefView :=
  ...
  { ref := stx, headerRef := mkNullNode stx.getArgs[*...3].copy, kind := DefKind.def, modifiers,
...
def mkDefViewOfTheorem (modifiers : Modifiers) (stx : Syntax) : DefView :=
  ...
  { ref := stx, headerRef := mkNullNode stx.getArgs[*...3].copy, kind := DefKind.theorem, modifiers,
...
def mkDefViewOfInstance (modifiers : Modifiers) (stx : Syntax) : CommandElabM DefView := do
  ...
  return {
    ref := stx, headerRef := mkNullNode stx.getArgs[*...5].copy, kind := DefKind.instance, modifiers := modifiers,
...
def mkDefViewOfOpaque (modifiers : Modifiers) (stx : Syntax) : CommandElabM DefView := do
  ...
  return {
    ref := stx, headerRef := mkNullNode stx.getArgs[*...3].copy, kind := DefKind.opaque, modifiers := modifiers,
...
def mkDefViewOfExample (modifiers : Modifiers) (stx : Syntax) : DefView :=
  ...
  { ref := stx, headerRef := mkNullNode stx.getArgs[*...2].copy, kind := DefKind.example, modifiers := modifiers,

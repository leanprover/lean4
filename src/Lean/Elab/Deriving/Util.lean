...
/--
    Represents `ind params1 ... paramsn`, where `paramsi` is either a nested type formers, a constant name, or a free variable
    ...
    ```
    this nested type formers `Foo A (Option Bar) B Nat` is encoded as
    ```lean
      node Foo #[
        .inr (.bvar 2),
    ...
    Remark 2 : Free variables are abstracted away as bound variables in nested type formerss.
    This is useful when trying to delete duplicate occurences, since the check is now purely syntactical.
  -/
inductive IndTypeFormer : Type :=
  | node (ind : InductiveVal) (params : Array (IndTypeFormer ⊕ Expr))
  | leaf (ind : InductiveVal) (fvars : Array Expr)

namespace IndTypeFormer

instance : Inhabited IndTypeFormer := ⟨leaf default #[]⟩
instance : Inhabited (IndTypeFormer ⊕ α) := ⟨.inl default⟩

partial instance : BEq IndTypeFormer := ⟨go⟩
where go
...
partial instance : ToString IndTypeFormer := ⟨go⟩
where go
...
@[inline]
def getIndVal :  IndTypeFormer → InductiveVal
  | leaf indVal _ | node indVal _ => indVal

@[inline]
def getArr :  IndTypeFormer → Array (IndTypeFormer ⊕ Expr)
  | leaf ..    => #[]
  | node _ arr => arr

@[inline]
def isLeaf : IndTypeFormer → Bool
  | leaf .. => true
  | node .. => false

@[inline]
def isNode : IndTypeFormer → Bool := not ∘ isLeaf

partial def containsFVar (fvarId : FVarId) : IndTypeFormer → Bool
  | leaf _ e   => e.any (Expr.containsFVar · fvarId)
  | node _ arr => arr.any (Sum.elim (containsFVar fvarId) (Expr.containsFVar · fvarId))

partial def toListofTypeFormers (e : IndTypeFormer) : List IndTypeFormer :=
  match e with
  | .leaf _ _   => []
  | .node _ arr =>
    let l := flip arr.foldr [] fun indFormer l =>
      if let .inl indFormer := indFormer then
          indFormer.toListofTypeFormers ++ l
      else l
    e::l

/-- Return the inductive declaration's type applied to the arguments in `argNames`. -/
partial def mkAppTerm (indTypeFormer : IndTypeFormer) (argNames : Array Name) : TermElabM Term := do
  go indTypeFormer argNames
where
  go (indTypeFormer : IndTypeFormer) (argNames : Array Name) : TermElabM Term := do
  match indTypeFormer with
    | leaf indVal _ =>
      let f := mkCIdent indVal.name
    ...
    | node indVal arr =>
      let f := mkCIdent indVal.name
      let mut args := #[]
      for indTypeFormer? in arr do
        match indTypeFormer? with
          | .inl indFormer =>
            let arg ← go indFormer argNames
            args := args.push arg
          | .inr (.bvar i) =>
            let some argName := argNames[argNames.size-i-1]?
              | throwError s!"Cannot instantiate {indTypeFormer} : not enough arguments given"
...
/-- Return the inductive declaration's type applied to the arguments in `argNames`. -/
partial def mkAppExpr (indTypeFormer : IndTypeFormer) (argNames : Array Expr) : TermElabM Expr := do
  let res ← go indTypeFormer argNames
  return res
where
  go (indTypeFormer : IndTypeFormer) (argNames : Array Expr): TermElabM Expr := do
  match indTypeFormer with
    | leaf indVal _ =>
      let numArgs := indVal.numParams + indVal.numIndices
      unless argNames.size >= numArgs do
...
structure Result where
  indFormer : IndTypeFormer
  args : Subarray Expr
  argNames : Array Name

instance : ToString Result where
  toString res := s!"⟨{res.indFormer},{res.args},{res.argNames}⟩"

instance : BEq Result := ⟨(·.indFormer == ·.indFormer)⟩
...
abbrev IndTypeFormerM := StateT IndTypeFormer.Context TermElabM

def withIndNames (indNames : List Name) (f : IndTypeFormerM Unit) : TermElabM IndTypeFormer.Context := do
  let ⟨(),ctx⟩ ← StateT.run f ⟨indNames,[]⟩
  return ctx

def addResult (x : IndTypeFormer.Result) : IndTypeFormerM Unit := do
  let ⟨names,res⟩ ← get
  set (⟨names,x::res⟩ : IndTypeFormer.Context)

def addName (n : Name) : IndTypeFormerM Unit := do
  let ⟨names,res⟩ ← get
  set (⟨n::names,res⟩ : IndTypeFormer.Context)

partial def getIndTypeFormersOf (inds : List Name) (e: Expr) (fvars : Array Expr): MetaM (Option IndTypeFormer) := do
  let .inl occs ← go e | return none
  return occs
where
  go (e : Expr) : MetaM (IndTypeFormer ⊕ Expr) := do
    let hd := e.getAppFn
    ...
    if let some indName := inds.find? (· = name) then
      ...
      try
        ...
        let indTypeFormersArgs ← args.mapM go
        if indTypeFormersArgs.any (· matches .inl _) then
          return .inl <| .node indVal indTypeFormersArgs
        else fallback ()
      catch _ => fallback ()

partial def getIndTypeFormers (indNames : List Name) : TermElabM (List IndTypeFormer.Result) := do
  let ⟨_,l⟩ ← withIndNames indNames do
    for name in indNames do
      go name #[] #[]
  return l.eraseDups
where
  go (indName : Name) (args : Array Expr) (fvars : Array Expr): IndTypeFormerM Unit := do
    let indVal ← getConstInfoInduct indName
    ...
    for constInfo in consts do
      ...
      forallTelescope instConstInfo fun xs _ => do
        ...
        for i in [:xs.size] do
          ...
          let localDecl ← e.fvarId!.getDecl
          let paramName ← mkFreshUserName localDecl.userName.eraseMacroScopes
          let indFormers ← getIndTypeFormersOf indNames ty xs[:i]
          let l' := if let .some x := indFormers then x.toListofTypeFormers else []
          for indFormer in l' do
            let new_args := paramArgs ++ localArgs.filter (indFormer.containsFVar ⟨·⟩)
            if (← get).results.all (indFormer != ·.indFormer) then
              addResult ⟨indFormer, xs[:i], new_args⟩
              let fvars := fvars ++ xs[:i]
              let app   ← indFormer.mkAppExpr fvars
...
partial def mkInstName: IndTypeFormer → String
  | .leaf ind _ => indNameToFunName ind.name
  | .node ind arr => Id.run do
    let mut res ← indNameToFunName ind.name
    for indTypeFormer in arr do
      if let .inl indFormer := indTypeFormer then
        let nestedInstName ← mkInstName indFormer
...
partial def mkInstImplicitBinders (className : Name) (indTypeFormer : IndTypeFormer) (argNames : Array Name) : TermElabM (Array Syntax) := do
  go indTypeFormer argNames
where
  go (indTypeFormer : IndTypeFormer) (argNames : Array Name) : TermElabM (Array Syntax) :=
  let indVal := indTypeFormer.getIndVal
  let arr    := indTypeFormer.getArr
  forallBoundedTelescope indVal.type indVal.numParams fun xs _ => do
    let mut binders : Array Syntax := #[]
    for i in [:xs.size] do
      if indTypeFormer.isNode && arr[i]? matches some (.inl _) then
        let some indFormer := arr[i]!.getLeft? | unreachable!
        let nestedBinders ← go indFormer argNames
...
def mkContext (fnPrefix : String) (typeName : Name) (withNested : Bool := true): TermElabM Context := do
  ...
  let mut typeInfos' : List IndTypeFormer.Result := []
  for indName in indNames do
    let indVal ← getConstInfoInduct indName
    let args ← mkInductArgNames indVal
    typeInfos' := ⟨.leaf indVal #[], #[].toSubarray, args⟩::typeInfos'
  if withNested then
    typeInfos' := (← getIndTypeFormers indVal.all) ++ typeInfos'
  let typeArgNames := typeInfos'.map (·.argNames) |>.toArray
  let typeInfos := typeInfos'.map (·.indFormer) |>.toArray
  let auxFunNames ← typeInfos.mapM fun indFormer => do
    return ← mkFreshUserName <| Name.mkSimple <| fnPrefix ++ mkInstName indFormer
...
def mkLocalInstanceLetDecls (ctx : Context) (className : Name) (argNames : Array Name) : TermElabM (Array (TSyntax ``Parser.Term.letDecl)) := do
  ...
  for i in [:ctx.typeInfos.size] do
    let indFormer          := ctx.typeInfos[i]!
    let auxFunName   := ctx.auxFunNames[i]!
    unless indFormer.isLeaf do
      continue
    let indVal       := indFormer.getIndVal
...
def mkInstanceCmds (ctx : Context) (className : Name) (useAnonCtor := true) : TermElabM (Array Command) := do
  ...
  for i in [:ctx.typeInfos.size] do
    ...
    unless indTypeFormer.isLeaf do continue
    let auxFunName := ctx.auxFunNames[i]!
    let argNames   := ctx.typeArgNames[i]!
    let binders    ←  mkImplicitBinders argNames
    let binders    := binders ++ (← mkInstImplicitBinders className indTypeFormer argNames)
    let indType    ←  indTypeFormer.mkAppTerm argNames
    let type       ←  `($(mkCIdent className) $indType)
...
open TSyntax.Compat in
def mkHeader (className : Name) (arity : Nat) (argNames : Array Name) (indTypeFormer : IndTypeFormer) : TermElabM Header := do
  let mut binders ← mkImplicitBinders argNames
  let targetType  ← indTypeFormer.mkAppTerm argNames
  let mut targetNames := #[]
  for _ in [:arity] do
    targetNames := targetNames.push (← mkFreshUserName `x)
  binders := binders ++ (← mkInstImplicitBinders className indTypeFormer argNames)
...
def Context.getFunName? (ctx : Context) (header : Header) (ty : Expr) (xs : Array Expr): TermElabM (Option Name) := do
  let indValNum ← ctx.typeInfos.findIdxM? <|
    (return .some · == (← getIndTypeFormersOf ctx.indNames ty xs[:header.argNames.size]))

...
partial def ofElem (stx : DoElem) : TermElabM ControlInfo := do
  ...
  -- For/Repeat
  | `(doElem| for $[$[$_ :]? $_ in $_],* $[$_:doForInvariant]? $[$_:doDecreasing]? do $bodySeq) =>
  ...
  | `(doRepeat| repeat $[$_:doForInvariant]? $[$_:doDecreasing]? do $bodySeq)
  | `(doRepeat| repeat $[$_:doForInvariant]? $[$_:doDecreasing]? $bodySeq:doSeq) =>

...
Recall that
```
def declValSimple    := leading_parser " :=\n" >> termParser >> Termination.suffix >> optional Term.whereDecls >> optional Term.obligationsBy
...
Likewise, the `obligationsBy` is ignored here, and extracted in `TODO`.
-/
...
/-- Elaborates the `obligations_by` section in a `declVal` syntax. -/
private def declValToObligations (declVal : Syntax) : TermElabM Obligations :=
  if declVal.isOfKind ``Parser.Command.declValSimple then
    mkObligations ⟨declVal[4]⟩
  else if declVal.isOfKind ``Parser.Command.declValEqns then
    mkObligations ⟨declVal[0][3]⟩
  else
    return .none
...
private def dischargeObligations (ref : Syntax) (name : Name) (value : Expr) (obligations : Obligations) : TermElabM PUnit := withRef ref do
  if obligations.isNone then return
  ...
  let goals := goals.insertionSort (·.fst < ·.fst) |>.map (·.snd)
  Lean.Elab.Term.TermElabM.run' do
  Term.withDeclName name do
  withRef obligations.ref do
    unless goals.isEmpty do
      ...
      let remainingGoals ← Tactic.run goals[0]! do
        ...
        Tactic.withTacticInfoContext obligations.ref do
          Tactic.evalTactic obligations.tactic
...
where
  ...
  finishElab headers (isExporting := false) := withFunLocalDecls headers fun funFVars =>
    ...
    -- Hence we solve the remaining ones using `obligations_by`.
    for l in letRecsToLift do
      dischargeObligations l.ref l.declName l.val l.obligations
    for header in headers, value in values do
      let obligations ← declValToObligations header.value
      dischargeObligations header.ref header.declName value obligations

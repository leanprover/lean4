...
structure FieldsHintView where
  /--
  Was this syntax synthetically generated by expanding a `where` structure instance?

  Note: This uses an approximate heuristic and may give false positives.
  -/
  isExpandedWhereStructInst : Bool
...
suggestions.
-/
def StructInstView.toFieldsHintView? (view : StructInstView) : Option FieldsHintView := do
  let stx := view.ref
  ...
  -- We refer only to the "closing" token `}` rather than a flexible "trailer" because
  ...
  -- generated by macro-expanding a `where` structure instance, which adds synthetic `{` and `}`
  let isExpandedWhereStructInst := stx[0].getHeadInfo matches .synthetic ..
  some {
    isExpandedWhereStructInst
    leaderPos
    leaderTailPos
    openingPos
    closingPos
...
def mkMissingFieldsHint (missingFields : Array PendingField) (unsolvedFields : Std.HashSet Name) : StructInstM MessageData := do
  ...
  let indent :=
    if let some initFieldPos := view.initFieldPos? then
      let lineStart := fileMap.source.findLineStart initFieldPos
      col initFieldPos - col lineStart
    else if view.isExpandedWhereStructInst then
      ...
      -- `...\n{ f := v\n ... }` and `...{\n  f := v\n ... \n}` styles
      let openLineStart := fileMap.source.findLineStart view.openingPos

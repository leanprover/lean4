...
private def takeTwoLines (ss : Substring) : Substring :=
  let { str, startPos, stopPos } := ss.takeWhile (· != '\n')
  let { stopPos, .. } :=
    { str, startPos := str.next stopPos, stopPos := str.endPos : Substring }.takeWhile (· != '\n')
...
def mkMissingFieldsHint (missingFields : Array PendingField) (unsolvedFields : Std.HashSet Name) : StructInstM MessageData := do
  let individualSuggestions ← missingFields.mapM fun field =>
    do
  ...
  let singleLine ← isSingleLineStyle (← read).view
  let replacementStx :=
    if singleLine then
      -- "single line" mode can only be detected if there are existing fields, so include the leading
      -- comma to offset from the prior field
      .group (behavior := .fill) <|
        individualSuggestions.foldl (init := Std.Format.nil) (fun acc sug => acc ++ "," ++ .line ++ format sug)
    else
      Std.Format.joinSep individualSuggestions.toList .line
  let fields := (← read).view.fields
  let ref := (← read).view.ref
  -- This is an approximate heuristic for whether this structure instance syntax was actually
  -- generated by macro-expanding a `where` structure instance
  -- TODO: find a better heuristic
  let isMacroExpanded := (ref.getArg 0).getHeadInfo matches .synthetic ..
  -- TODO: we really need a `StructInstHintView` that stores all the bits of syntax we need for
  -- generating hints
  let width := Tactic.TryThis.format.inputWidth.get (← getOptions)
  let map ← getFileMap
  let indent :=
    -- TODO: don't `get!` (both clauses)
    if _ : fields.size > 0 then
      let firstFieldStart := fields[0].ref.getPos?.get!
      let lineStart := map.source.findLineStart firstFieldStart
      (firstFieldStart - lineStart).1
    else if isMacroExpanded then
      -- `whereStructInst` syntax doesn't provide any indentation clues, so use default indentation
      2
    else
      -- Take the minimum of the open and close brace indentations to account for both the
      -- `...\n{ f := v\n ... }` and `...{\n  f := v\n ... \n}` styles
      let openBraceStart := ref.getPos?.get!
      let openLineStart := map.source.findLineStart openBraceStart
      let closeBraceStart := ref[5].getPos?.get!
      let closeLineStart := map.source.findLineStart closeBraceStart
      min (openBraceStart - openLineStart).1 (closeBraceStart - closeLineStart).1 + 2
  let fileMap ← getFileMap
  let otherwise (stx₁ stx₂ : Syntax) := if stx₁ matches .missing then stx₂ else stx₁
  -- The leader is either the `with` keyword (if present) or the `{` character
  let leader := if ref[1][1] matches .missing then ref[0] else ref[1][1]
  -- Our trailer is always `}` because macro-expansion will erase the type annotation at `ref[4]`
  -- (note that it's impossible to have an ellipsis if we're generating a "fields missing" error)
  let trailer := ref[5]
  -- TODO: no `get!`
  let leaderLine := (fileMap.utf8PosToLspPos leader.getPos?.get!).line
  let leaderTrailerSeparated := leaderLine < (fileMap.utf8PosToLspPos trailer.getPos?.get!).line
  ...
  let interveningLineEndPos? :=
    if !leaderTrailerSeparated then none else
      -- TODO: no `get!`
      let startPos := leader.getTailPos?.get!
      let endPos := indent.fold (init := startPos) (fun _ _ p => map.source.next p)
      let nextTwoLines := takeTwoLines { str := map.source, startPos, stopPos := map.source.endPos }
  ...
  let replacementStx :=
    ...
    if singleLine then replacementStx
    ...
    else if isMacroExpanded then .line ++ replacementStx
    -- If there is a `with` clause without an intervening line, we need a new line to reset our indentation
    -- TODO: use a view rather than direct syntax indexing here
    else if !ref[1][0].isMissing && !leaderTrailerSeparated then .line ++ replacementStx
  ...
  -- TODO: don't use `get!` (crashes on some parse failures)
  let (insertionStart, insertionEnd) :=
    if fields.size > 0 then
      let pos := ref[2].getTailPos?.get!
      (pos, pos)
    else if leaderTrailerSeparated then
      -- If the line is clear up to `indent`, then insert at `indent`; otherwise, insert immediately
      -- after the leader
      ...
      if let some endPos := interveningLineEndPos? then
        ...
        -- TODO: don't use `get!`
        let pos := leader.getTailPos?.get!
        (pos, pos)
    else
      let pos := leader.getTailPos?.get!
      (pos, pos)
  let col := (insertionStart - map.source.findLineStart insertionStart).byteIdx
  let replacementStx := replacementStx.pretty width indent col
  let synth := Syntax.ofRange ⟨insertionStart, insertionEnd⟩
  let individualSuggestions := #[replacementStx] ++ individualSuggestions |>.map fun stx =>
    { suggestion := .string stx : Hint.Suggestion }
  MessageData.hint m!"Add missing fields:" individualSuggestions synth

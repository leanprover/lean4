...
private structure FieldsHintView where
  ...
  Note: This uses an approximate heuristic and may give false positives if the structure instance
  was generated by some other macro.
  -/
  ...
  hasWith : Bool
  /-- Does this structure instance have existing fields? -/
  hasFields : Bool
...
private def StructInstView.toFieldsHintView? (view : StructInstView) : Option FieldsHintView := do
  ...
  some { isExpandedWhereStructInst, leaderPos, leaderTailPos, openingPos, closingPos, hasWith,
         hasFields, initFieldPos?, lastFieldTailPos? }
...
private def mkMissingFieldsHint (missingFields : Array PendingField) (unsolvedFields : Std.HashSet Name)
  ...
  let (preWs, postWs) : Format × Format :=
    ...
    if useSingleLine then
      (.nil, .nil)
    -- Add a line break for consistent indentation if fields exist or there are no fields after `where`/`with`
    else if view.hasFields || view.isExpandedWhereStructInst || (view.hasWith && !leaderClosingSeparated) then
      (.line, .nil)

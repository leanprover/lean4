...
private partial def mkCoercionToCopiedParent (levelParams : List Name) (params : Array Expr) (view : StructView) (source : Expr) (parent : StructParentInfo) (parentType parentVal : Expr) : MetaM StructureParentInfo := do
  ...
  if isProp then
    ...
    addDecl <| Declaration.defnDecl { cval with
      value       := declVal
...
/-- Compiles the declarations generated by `mkRemainingProjections`. -/
private def compileRemainingProjections : StructElabM Unit := do
  for parent in (← get).parents do
    unless parent.subobject do
      compileDecls #[parent.declName]

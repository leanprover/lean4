...
def ofExpr? (e : Expr) : MetaM (Option Argument) := ofExpr?Aux e |>.run where
  ofExpr?Aux (e : Expr) : OptionT MetaM Argument := do
...
-/
def replaceCallWithFVar (g : MVarId) (fn : Function) (args : ArgumentArray) : AckM (CallInfo × MVarId) := do
  g.withContext do

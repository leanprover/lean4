...
where
  ...
  goEven stx := do
    ...
    * `next` is used as the context when invoking `goOdd` and thus eventually used for the next
    ...
    Term.withNarrowedTacticReuse (stx := stx) (fun stx => (stx[0], mkNullNode stx.getArgs[1...*].copy)) fun stxs => do
  ...
  goOdd stx := do
    ...
    Term.withNarrowedTacticReuse (fun stx => (stx[0], mkNullNode stx.getArgs[1...*].copy)) goEven stx

    ...
    let runTac? : TermElabM (Option Expr) :=
      try
        /- Recall that `simp` uses temporary metavariables (`withNewMCtxDepth`).
           ...
           We also need to revert any error messages generated by the discharger.
           (Previously we used `withoutModifyingStateWithInfoAndMessages` here,
           but then discharger errors from failed invocations propogated to the user.
           See leanprover/lean4#2634.)
        -/

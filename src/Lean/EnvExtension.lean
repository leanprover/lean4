...
def mkTagDeclarationExtension (name : Name := by exact decl_name%)
  (asyncMode : EnvExtension.AsyncMode := .mainOnly)
  (declCovered : Bool := false) : IO TagDeclarationExtension :=
...
def tag (ext : TagDeclarationExtension) (env : Environment) (declName : Name) : Environment :=
  if declName.isAnonymous then
    ...
    if ext.getState (asyncMode := ext.toEnvExtension.asyncMode) (asyncDecl := declName) env
      ...
      let env := if ext.toEnvExtension.declCovered then env.logDeclChange declName else env
...
def insert (ext : MapDeclarationExtension α) (env : Environment) (declName : Name) (val : α)
  ...
  -- Write-once guard: dependency-recording coverage claims for declaration-keyed

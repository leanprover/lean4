...
/--
Checks whether the final name component is one generated by the code generator (specialization
and closure-lifting artifacts, closed terms, boxed wrappers). Such names live in the
extra-constants view, so looking them up extras-first avoids a pointless walk of (and a negative
cache entry in) the much larger constants view.
-/
private def isCodegenName : Name → Bool
  | .str _ s =>
    s == "_boxed" || s == "_redArg" || "_closed_".isPrefixOf s || "_lam_".isPrefixOf s
      || "_elambda".isPrefixOf s || "_cstage".isPrefixOf s
  | _ => false
...
def getModuleIdxFor? (env : Environment) (declName : Name) : Option ModuleIdx :=
  if isCodegenName declName then
    match env.importedExtraConsts.findExtraModIdxCached? declName with
    | some modIdx => some modIdx
    | none        => env.allImportedConsts.findConstModIdxCached? declName
  else

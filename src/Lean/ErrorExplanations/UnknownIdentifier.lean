...
message "Unknown identifier \`x\`" indicates that none of `x`, `A.x`, and `B.x` could be found, or
`A.x` or `B.x` is a protected or private declaration.
...
suggests constant names similar to the one provided. This includes constants in the environment as
...
accessed in the outer `do` block. Moving this binding to the outer `do` block—where it remains in
scope in the inner block as well—resolves the issue.
...
example—can be used to allow a `protected` constant to be referred to by its unqualified name; see
the manual section on [Namespaces and Sections](lean-manual://section/namespaces-sections) for
details.

## Invalid name inferred by dotted-identifier notation
...
Note: Inferred this identifier from the expected type of `.toNat`:
  Nat
```
...
In this example, the dotted-identifier notation `.toNat` causes Lean to infer a nonexistent
identifier (`Nat.toNat`). The namespace used by dotted-identifier notation is always inferred from
the expected type of the expression in which it occurs, which—due to the type annotation on
`disjoinToNat`—is `Nat` in this example. To use the namespace of an argument—as the author of this
code intended—use *generalized field notation* as shown in the first corrected example.
Alternatively, the correct namespace can be explicitly specified by writing the fully qualified
function name.
-/

...
private def withAppBuilderTrace [ToMessageData α] [ToMessageData β]
  ...
  withTraceNode `appBuilder (return m!"{emoji ·} f: {f}, xs: {xs}") do
    try
      let res ← k
      trace[appBuilder.result] res
      pure res
    catch ex =>
      trace[appBuilder.error] ex.toMessageData
      throw ex
...
builtin_initialize do
  registerTraceClass `appBuilder (descr := "the `Expr` application builder helper functions")
  registerTraceClass `appBuilder.result (inherited := true) (descr := "the result of `Expr` application builder helper functions")
  registerTraceClass `appBuilder.error (inherited := true) (descr := "errors generated by `Expr` application builder helper functions")

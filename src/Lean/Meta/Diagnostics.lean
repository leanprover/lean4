...
def mkDiagSummary (cls : Name) (counters : PHashMap Name Nat) (p : Name → Bool := fun _ => true) : MetaM DiagSummary := do
  ...
  if entries.isEmpty then
    ...
    for (declName, counter) in entries do
      data := data.push <| .trace { cls } m!"{.ofConstName declName} ↦ {counter}" #[]
...
def reportDiag : MetaM Unit := do
  if (← isDiagnosticsEnabled) then
    ...
    let unfoldCtxKernel ← mkDiagSummary `kernel (Kernel.getDiagnostics (← getEnv)).unfoldCtxCounter
    let m := #[]
    ...
    let m := appendSection m `kernel "context of unfoldings" unfoldCtxKernel
    unless m.isEmpty do

...
/--
Forward reference to `Lean.Elab.Eqns.mkEqnsImpl` (which involves tactics like simp
that want use equational theorems)
-/
@[extern "lean_mk_eqns"]
opaque mkEqns (declName : Name) (declNames : Array Name) : MetaM (Array Name)
...
private def getEqnsFor?Core (declName : Name) : MetaM (Option (Array Name)) := withLCtx {} {} do
  ...
  if (← isRecursiveDefinition declName) then
    ...
    registerEqnThms declName n
    return some n
  if (← getEnv).contains declName then
    if backward.eqns.nonrecursive.get (← getOptions) then
      let n ← mkEqns declName #[]
      registerEqnThms declName n
      return some n
    let name := mkEqLikeNameFor (← getEnv) declName eqn1ThmSuffix
    if let some n ← mkSimpleEqThm declName name then
      trace[Elab.definition.eqns] "creating simple equation for {declName} "
      registerEqnThms declName #[n]
      return #[n]
  return none

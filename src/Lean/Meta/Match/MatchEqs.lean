...
def getEquationsForImpl (matchDeclName : Name) : MetaM MatchEqns := do
  ...
  if matchInfo.numAlts = 0 then
    -- Special case of no alternatives, do not bother creating anythings
    let result := { eqnNames := #[], splitterName := matchDeclName, splitterAltNumParams := #[]}
...
where go baseName := withConfig (fun c => { c with etaStruct := .none }) do
  ...
  forallTelescopeReducing constInfo.type fun xs matchResultType => do
    ...
    if (← needsSplitter matchDeclName) then
      ...
      unless splitterAltNumParams = matchInfo.altNumParams do
        panic s!"internal error: altNumParams for {matchDeclName} changed from {matchInfo.altNumParams} to {splitterAltNumParams} even though no splitter is needed"
      let result := { eqnNames, splitterName := matchDeclName, splitterAltNumParams }
      registerMatchEqns matchDeclName result

...
/--
  Construct a proof for the elimination principle generated by `mkEquationsfor`.
...
private partial def mkElimProof (template : Expr) (alts altsNew : Array Expr) : MetaM Expr := do
  trace[Meta.Match.matchEqs] "proof template: {template}"
  let map := mkMap
  let (proof, mvarIds) ← convertTemplate map |>.run #[]
  trace[Meta.Match.matchEqs] "elim proof: {proof}"
  for mvarId in mvarIds do
    trace[Meta.Match.matchEqs] "elim subgoal:\n{MessageData.ofGoal mvarId}"
    -- TODO: close goal `mvarId`
  check proof -- TODO remove
  return proof
where
  ...
  addExtraParams (e : Expr) : StateRefT (Array MVarId) MetaM Expr := do
    ...
    let (mvars, _, _) ← forallMetaTelescope (← inferType e) (kind := MetavarKind.syntheticOpaque)
...
partial def mkEquationsFor (matchDeclName : Name) :  MetaM Unit := do
  ...
  forallTelescopeReducing constInfo.type fun xs matchResultType => do
    ...
    withElimAlts elimAltTypes fun altsNew => do
      ...
      let elimVal ← mkElimProof template alts altsNew

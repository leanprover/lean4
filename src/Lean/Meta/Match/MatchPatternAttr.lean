...
builtin_initialize matchPatternAttr : TagAttribute ←
  ...
  -- covered: declaration-keyed; post-hoc applications are recorded in the declaration change log
  registerTagAttribute (synthCovered := true) `match_pattern "mark that a definition can be used in a pattern (remark: the dependent pattern matching compiler will unfold the definition)"

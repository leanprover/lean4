...
`environment::add_inductive` certifies each declaration as it makes it, so the check covers whatever
is being compiled, `Init.Prelude` included. There is no way to turn it off: a nested inductive whose
certificate does not answer for what the kernel declared is rejected. Core's own nested inductives
are therefore certified by the stage 2 build, where a kernel carrying this check compiles them.

The model is built from the environment as it stood before the declaration was added, so the rules
are proved somewhere the declaration itself is not assumed. Were it built afterwards, a rule could
be discharged by the very recursor it is meant to justify.

Occurrence discovery is purely syntactic, with no `whnf`, and matches an already-created auxiliary
type by structural equality. It is a second implementation of the rule the kernel eliminates by, and
the two are required to agree on whether a declaration is nested at all.
-/
...
The kernel declares its own; these are what it is measured against. `checkRestore` does the
comparison where the declaration is already present, and `certify_nested` does it in the kernel,
where the recursor's index count is read off these rather than off the declaration.
...
-/
def restoreRecursors (iv : InductiveVal) (types : Array InductiveType)
...
The target types come from `restoreRecursors`, so the realisation is built against a derivation
of the recursor rather than against the one the kernel declared. That the two agree is what
`realises` asks in the kernel, and it asks it of these definitions.
-/
...
Nothing depends on the result. It is here so that the derivation is measured against the kernel's
own answer, in full, for every nested inductive reachable after the fact. The kernel makes the
narrower comparison itself, of the index count and of the definitions realising each constant.
-/
...
requires itself. The declaration's constants name definitions over the model there, so restating a
rule in `env` restates it over the model, where it has content, rather than over the declaration,
where it holds by construction and so says nothing. Looking a proof up is not trusting it, since the
kernel checks the term it finds. What the kernel does take on faith is that the declarations in `env`
were checked when they were added, which is the same thing it already assumes of any environment the
elaborator hands it.
-/
structure Certificate where
  ...
  kernel to measure the ones it declared against.
  -/
  recs   : Array RestoredRec
  /--
  Whether the model makes each declared type reflexive, in the order the declaration lists them.
  Compared in one direction only: an occurrence under a redex hides the binder that makes the type
  reflexive, and the model has already reduced it away, so the model can be reflexive where the
  declaration is not.
  -/
  reflexive : Array Bool

/--
What `buildModelFor` found: how many rules already held definitionally, how many it proved, which it left
...
number left open.
-/
def buildModelFor (iv : InductiveVal) (types : Array InductiveType) (pre : Name)
  ...
  withElimNested iv.levelParams iv.numParams types.toList fun model aux params => do
    -- no occurrence to unfold, so there is no model to build and nothing to check against; the
    -- kernel takes this as disagreeing with its own elimination
    ...
    -- reached from `add_inductive` the declaration is not in the environment yet, so there is
    -- nothing to compare against there; this is the after-the-fact comparison
...
`ofKernelEnv` is not enough for that: the caches behind queries such as `Meta.getFunInfo` realize
values for imported declarations, which needs a realization context it leaves empty.

The same caveat applies as for `ofKernelEnv`: the result is temporary and must not leak into
elaboration. Its realization context is its own, so anything realized on it is discarded along with
it rather than shared with the environment being elaborated.
-/
def ofKernelEnvForElab (kenv : Kernel.Environment) : BaseIO Environment :=
  Environment.enableRealizationsForImports (Environment.ofKernelEnv kenv) {}

/--
The constants the generator needs whatever the declaration looks like. The kernel certifies a
declaration at the point it is made, so a nested inductive declared before these exist could not be
certified at all; `certifyDecl` reports that rather than passing it silently.
...
/--
Build the model for `iv` over `env`, realise the declaration's constants as definitions over it, and
prove every computation rule that does not already hold there. Returns a message describing the
failure, the certificate, or `none` if the declaration has no nested occurrence.

`sur` names the realisations. The kernel wants them under the declaration's own names, so that the
rules it states resolve to them; a caller certifying a declaration that is already in `env` has to
name them out of its way instead.

Nothing here can license a declaration: the model, the bridges and the rule proofs all go into a
scratch environment, and the kernel takes nothing from it but answers to questions it asks itself.
-/
def certifyCore (env : Environment) (iv : InductiveVal) (types : Array InductiveType)
...
which is how `add_inductive` reaches it. `kenv` is the environment as it stood before the declaration
was added, so the realisations can carry its own names, and so the rules are proved somewhere the
declaration is not itself assumed.

`ok none` means the declaration has no nested occurrence. The kernel asks only about declarations its
own elimination found one in, so it treats that answer as the two disagreeing.
-/
@[export lean_certify_nested_inductive]
def certifyDecl (kenv : Kernel.Environment) (d : Declaration) :
  ...
  let env ← ofKernelEnvForElab kenv

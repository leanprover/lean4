...
whatever is being compiled, `Init.Prelude` included. There is no way to turn it off: a nested
inductive whose certificate does not answer for what the kernel declared is rejected.

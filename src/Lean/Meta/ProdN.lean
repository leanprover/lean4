...
For `n = 0`, return `PUnit` with the given level.
-/
def mkProdN (ts : Array Expr) (u : Level) : MetaM Expr := do
  if h : ts.size > 0 then
...
For `n = 0`, return `PUnit.unit` with the given level.
-/
def mkProdMkN (es : Array Expr) (u : Level) : MetaM (Expr × Expr) := do
  if h : es.size > 0 then

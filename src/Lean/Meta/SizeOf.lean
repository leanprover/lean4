...
partial def mkSizeOfFn (recName : Name) (declName : Name): MetaM Unit := do
  ...
  forallTelescopeReducing recInfo.type fun xs _ =>
    ...
    let params := xs[*...recInfo.numParams].copy
    let motiveFVars := xs[recInfo.numParams...(recInfo.numParams + recInfo.numMotives)].copy
    let minorFVars := xs[recInfo.getFirstMinorIdx...(recInfo.getFirstMinorIdx + recInfo.numMinors)].copy
    let indices := xs[recInfo.getFirstIndexIdx...(recInfo.getFirstIndexIdx + recInfo.numIndices)].copy
...
private def recToSizeOf (e : Expr) : M Expr := do
  matchConstRec e.getAppFn (fun _ => throwFailed) fun info us => do
    ...
    | some sizeOfName =>
      ...
      let indices := args[info.getFirstIndexIdx...(info.getFirstIndexIdx + info.numIndices)].copy
...
mutual
  ...
  private partial def mkSizeOfAuxLemmaProof (info : InductiveVal) (lhs : Expr) : M Expr := do
    ...
    let sizeOfBaseArgs := lhsArgs[*...(lhsArgs.size - info.numIndices - 1)].copy
    let indicesMajor := lhsArgs[(lhsArgs.size - info.numIndices - 1)...*].copy
    ...
    | some (_, us) =>
      ...
      let r := mkAppN r majorTypeArgs[*...info.numParams].copy
    ...
    | some (fName, us) =>
      ...
      if (← getEnv).contains thmName then
        ...
        matchConstInduct xType.getAppFn (fun _ => throwFailed) fun info _ => do
          let params := xType.getAppArgs[*...info.numParams].copy
          forallTelescopeReducing (← inferType (mkAppN xType.getAppFn params)) fun indices _ => do
            ...
            withLocalDeclD `x majorType fun major => do
              let lhsArgs := lhs.getAppArgs
              let lhsArgsNew := lhsArgs[*...(lhsArgs.size - 1 - indices.size)].copy ++ indices ++ #[major]
...
private def mkSizeOfSpecTheorem (indInfo : InductiveVal) (sizeOfFns : Array Name) (recMap : NameMap Name) (ctorName : Name) : MetaM Unit := do
  ...
  forallTelescopeReducing ctorInfo.type fun xs _ => do
    let params := xs[*...ctorInfo.numParams].copy
    let fields := xs[ctorInfo.numParams...*].copy
        ...
        unless indInfo.isUnsafe do
          ...
          for indTypeName in indInfo.all, fn in fns do
            ...
            forallTelescopeReducing indInfo.type fun xs _ =>
              ...
              withInstImplicitAsImplicit params do
                let indices := xs[indInfo.numParams...*].copy
                mkLocalInstances params fun localInsts => do

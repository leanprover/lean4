    ...
    if let some entry := (← get).tableEntries.find? gNode.key then
      if let some answer := entry.answers[0]? then
        ...
        if answer.numMVars == 0 then
          ...
          let cacheKey := (← getLocalInstances, gNode.mvarType)
          modify fun s => { s with cacheEntries := s.cacheEntries.push (cacheKey, inst)}
        if entry.answers.size ≥ 2 then
          throwError "too many anwers to {gNode.key}"
  ...
  withReader (fun ctx => { ctx with inTypeClassResolution := true }) do
    ...
    let uncached := do
      let result? ← withNewMCtxDepth (allowLevelAssignments := true) do
        ...
        let result₂ ← SynthInstance.UseGlobalCache.main normType maxResultSize
        if result₁ != result₂ then
          throwError m! "For instance {type}, the old implementation gives {result₁.map fun ⟨a,b,c⟩ => (a,b,c,repr c)}, but the new implementation gives {result₂.map fun ⟨a,b,c⟩ => (a,b,c,repr c)}"
        return result₁

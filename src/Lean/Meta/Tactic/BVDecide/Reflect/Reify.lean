...
-/
public partial def ReifiedBVExpr.of (origExpr : Expr) : LemmaM (Option ReifiedBVExpr) := do
  goOrAtom origExpr
where
  /--
...
-/
public partial def ReifiedBVPred.of (origExpr : Expr) : LemmaM (Option ReifiedBVPred) := do
...
Unless this function is called on something that is not a `Bool` it is always going to return `some`.
-/
public partial def ReifiedBVLogical.of (origExpr : Expr) : LemmaM (Option ReifiedBVLogical) := do
  goOrAtom origExpr
where
  /--

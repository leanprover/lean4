...
def unifyEq? (mvarId : MVarId) (subst : FVarSubst) (caseName? : Option Name := none): MetaM (Option UnifyEqResult) := do
...
partial def unifyEqs? (numEqs : Nat) (mvarId : MVarId) (subst : FVarSubst) (caseName? : Option Name := none): MetaM (Option (MVarId × FVarSubst)) := do
  if numEqs == 0 then
    return some (mvarId, subst)
  else if let some { mvarId, subst, numNewEqs } ← unifyEq? mvarId subst caseName? then

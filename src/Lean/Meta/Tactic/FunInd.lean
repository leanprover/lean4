...
partial def foldAndCollect (oldIH newIH : FVarId) (isRecCall : Expr → Option Expr) (e : Expr) : M Expr := do
  ...
  let e' ← id do
    ...
    -- If we have a a function application that abstracts over `oldIH`, then is it very
    -- likely the result of `abstractNestedProofs`. For these we replace the argument
    -- `oldIh` with `fun x _h => f x`.
    -- This seems to work fine. We used to unfold these definitions, but that copies a lot of
    -- proofs and may expose function calls that look different than what the user expects.
    -- It is important, though, to `foldAndCollect` the _type_ of `e` here, else
    -- we may miss recursive calls, it seems.
    if e.getAppFn.isConst && e.getAppArgs.any (·.isFVarOf oldIH) then
      let eType ← inferType e
      let eType' ← foldAndCollect oldIH newIH isRecCall eType
      let args' ← e.getAppArgs.mapM fun a => do
        if a.isFVarOf oldIH then
          forallBoundedTelescope (← inferType a) (some 2) fun xs _ => do
            let aExpanded := mkAppN (mkFVar newIH) xs
            if let some a' := isRecCall (← inferType aExpanded) then
              mkLambdaFVars xs a'
            else
              throwError "Failed to eliminate {mkFVar oldIH} from:{indentExpr e}"
        else
          return ← foldAndCollect oldIH newIH isRecCall a

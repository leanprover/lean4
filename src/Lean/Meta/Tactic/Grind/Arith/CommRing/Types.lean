...
structure CommRing extends Ring where
  ...
  /-- `PowCharIdentity` instance, the synthesized `CommSemiring` instance, and characteristic `p` if available. -/
  powCharIdentityInst? : Option (Expr × Expr × Nat) := none
  ...
  /-- Number of variables for which `PowCharIdentity` equations have been pushed. -/
  powCharIdentityVarCount : Nat := 0
  /--

...
=           - 2*x*(x - 1)
(2*x + x*y)                  | s₂ := .step (2*x + x*y) c₁ 1 (-2) x s₁
=           - 2*1*(x - 1)
(x*y + 2)                    | s₃ := .step (x*y + 2) c₁ 1 (-2) 1 s₂
=           - 1*y*(x - 1)
(y + 2)                      | s₄ := .step (y+2) c₁ 1 (-1) y s₃
=           - 1*1*(y - 2)
4                            | s₅ := .step 4 c₂ 1 1 1 s₄
```
...
inductive PolyDerivation where
  ...
  | /--
    ...
    *2
    =   - 1*1*(2*x - 1)
    2*y + 2*z + 1        | s₂ := .step (2*y + 2*z + 1) c₁ 2 (-1) 1 s₁
    *3
    =   - 2*1*(3*y - 1)
    6*z + 5              | s₃ := .step (6*z + 5) c₂ 3 (-2) 1 s₂
    =   - 1*1*(6*z + 5)
    0                    | s₄ := .step (0) c₃ 1 (-1) 1 s₃
    ```

...
open GrindCnstr in
def grindPatternCnstrAtom : Parser :=
...
/-- A constraint can be a single atomic constraint or a disjunction of atomic constraints separated by `<|>` -/
def grindPatternCnstr : Parser :=
  sepBy1 grindPatternCnstrAtom " <|> "
...
Constraints can be combined using `<|>` to express "at least one must hold":
```
...
delayed evaluation which is incompatible with the immediate checking semantics of disjunctions.

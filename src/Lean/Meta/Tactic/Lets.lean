...
def addDecl (decl : LocalDecl) (isLet : Bool) : M Unit := do
  ...
  modify fun s => { s with
    ...
    valueMap := if cfg.merge then s.valueMap.insert (decl.value false) decl.fvarId else s.valueMap
  }
...
def flushDecls (fvar : FVarId) : M (Array LocalDecl') := do
  ...
  for ldecl in (← get).decls do
    if ldecl.decl.type.hasAnyFVar (fvarSet.contains ·) || (ldecl.decl.value false).hasAnyFVar (fvarSet.contains ·) then
...
def mkLetDecls (decls : Array LocalDecl') (e : Expr) : Expr :=
  decls.foldr (init := e) fun { decl, isLet } e =>
    Expr.letE decl.userName decl.type (decl.value false) (e.abstract #[decl.toExpr]) (nonDep := !isLet)
...
def initializeValueMap : M Unit := do
  ...
  lctx.forM fun decl => do
    if decl.isLet false && !decl.isImplementationDetail then
      let value ← instantiateMVars (decl.value false)

...
def rewrite (e : Expr) (s : DiscrTree SimpLemma) (discharge? : Expr → SimpM σ (Option Expr)) : SimpM σ Result := do
  let lemmas ← s.getMatch e
  if lemmas.isEmpty then
    trace[Meta.Tactic.simp]! "no lemmas found for rewriting {e}"
    return { expr := e }
  else
    ...
    return { expr := e }
where
  getLHS (kind : SimpLemmaKind) (xs : Array Expr) (type : Expr) : MetaM Expr :=
    match kind with
    | SimpLemmaKind.pos => return type
    | SimpLemmaKind.neg => inferType xs.back
  ...
  tryLemma? (lemma : SimpLemma) : SimpM σ (Option Result) :=
    withNewMCtxDepth do
      ...
      let lhs ← getLHS lemma.kind xs type
...
def preDefault (e : Expr) (discharge? : Expr → SimpM σ (Option Expr)) : SimpM σ Step := do
  ...
  return Step.visit (← rewrite e lemmas.pre discharge?)

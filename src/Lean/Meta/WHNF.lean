...
pair auto-generated by the `type_def` command for some virtual type (see `VirtualStructureInfo`).
...
instead consult the `type_def` registry. Like real projection-of-constructor reduction, this is
independent of `ctorName`/`projName`'s reducibility status, even though `type_def` marks both
    ...
    whnfEasyCases e fun e => do
      ...
      | .app f ..       =>
        ...
        if f'.isLambda && (cfg.beta || !f.isLambda) then
          ...
          -- `type_def`-generated projector never becomes a real `Expr.proj` node (it stays an

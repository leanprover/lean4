...
/--
Mark `mvarId` so that assignments to it must preserve its type up to
`TransparencyMode.instances`. See `MetavarContext.instanceTypedMVars`.
-/
...
/--
Return `true` if assignments to `mvarId` must preserve its type up to
`TransparencyMode.instances`. See `MetavarContext.instanceTypedMVars`.
-/

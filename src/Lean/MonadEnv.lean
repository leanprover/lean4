...
def addDecl [Monad m] [MonadEnv m] [MonadError m] [MonadOptions m] [MonadLog m] [AddMessageContext m] (maxHeartbeats: Nat) (decl : Declaration) : m Unit := do
...
def compileDecl [Monad m] [MonadEnv m] [MonadError m] [MonadOptions m] (maxHeartbeats: Nat) (decl : Declaration) : m Unit := do
...
def compileDecls [Monad m] [MonadEnv m] [MonadError m] [MonadOptions m] (maxHeartbeats: Nat) (decls : List Name) : m Unit := do

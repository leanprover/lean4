...
def symbolRarity (env : Environment) : NameMap Nat := Id.run do
  let mut map := {}
...
def mepo (initialRelevant : NameSet) (accept : ConstantInfo → CoreM Bool) (p : Float := 0.6) (c : Float := 2.4) : CoreM (Array (Name × Float)) := do
  let env ← getEnv
  let mut p := p
  let mut candidates := #[]
  ...
  for (n, ci) in env.constants do
    if ← accept ci then
      candidates := candidates.push (n, ci.getUsedConstantsAsSet)
  while candidates.size > 0 do
    let (newAccepted, candidates') := candidates.map (fun (n, c) => (n, c, unweightedScore relevant c)) |>.partition fun (_, _, s) => p ≤ s

...
builtin_initialize reducibilityCoreExt : PersistentEnvExtension (Name × ReducibilityStatus) (Name × ReducibilityStatus) (NameMap ReducibilityStatus) ←
  registerPersistentEnvExtension {
    ...
    -- change log and validated by birth arithmetic (see `Environment.synthChangeLog`)
    synthCovered    := true
    mkInitial       := pure {}
...
(`Environment.synthChangeLog`): an attribute that is part of `foo`'s own elaboration
...
private def addAttr (status : ReducibilityStatus) (declName : Name) (stx : Syntax) (attrKind : AttributeKind) : AttrM Unit := do
  ...
  -- query could have observed must be recorded; see `Environment.synthChangeLog`.
  modifyEnv fun env => env.logSynthChange declName

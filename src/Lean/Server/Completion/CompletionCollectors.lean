...
-/
private def trailingDotCompletion
    (entries : Array (Name × α)) (stx : Syntax) (caps : ClientCapabilities) (ctx : ContextInfo)
...
def optionCompletion
  ...
  ctx.runMetaM {} do
    ...
    return trailingDotCompletion decls.toArray stx[1] caps ctx fun name decl textEdit? => {
      label := name.toString

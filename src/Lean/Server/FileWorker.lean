...
This option can only be set on the command line, not in the lakefile or via `set_option`."
  ...
  private partial def reportSnapshots (ctx : WorkerContext) (doc : EditableDocumentCore)
    ...
    BaseIO.bindTask t fun _ => do
      let (_, st) ← goSeq [.pure <| toSnapshotTree doc.initSnap] |>.run {}
      ...
      return .pure ()
  where
    handle (node : SnapshotTree) : StateT ReportSnapshotsState BaseIO Unit := do
    ...
    handleFinished (t : SnapshotTask SnapshotTree) : StateT ReportSnapshotsState BaseIO (Array (SnapshotTask SnapshotTree)) := do

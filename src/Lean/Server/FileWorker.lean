...
section Initialization
  ...
  def buildHeaderEnv (m : DocumentMeta) (hOut : FS.Stream) (opts : Options) (headerStx : Syntax) : IO (Environment × MessageLog × SearchPath × Options) := do
    let (headerEnv, msgLog, srcSearchPath, opts) ←
      ...
      | .ok (srcSearchPath, fileOptions) =>
        ...
        pure (headerEnv, msgLog, srcSearchPath, opts)
      | .error e =>
        let msgs := MessageLog.empty.add { fileName := "<ignored>", pos := ⟨0, 0⟩, data := e.toString }
        pure (← mkEmptyEnvironment, msgs, ← initSrcSearchPath, opts)
  ...
  def compileHeader (m : DocumentMeta) (hOut : FS.Stream) (opts : Options) (hasWidgets : Bool)
    ...
    (headerStx, ·) <$> EIO.asTask do
      let (headerEnv, envMsgLog, srcSearchPath, opts) ← buildHeaderEnv m hOut opts headerStx

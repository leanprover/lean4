...
def runCommandElabM (snap : Snapshot) (doc : DocumentMeta) (c : CommandElabM α)
    (cancelTk? : Option IO.CancelToken := none) : EIO Exception α :=
...
def runCoreM (snap : Snapshot) (doc : DocumentMeta) (c : CoreM α)
    (cancelTk? : Option IO.CancelToken := none) : EIO Exception α :=
  snap.runCommandElabM doc (Command.liftCoreM c) cancelTk?

/-- Run a `TermElabM` computation using the data in the given snapshot.-/
def runTermElabM (snap : Snapshot) (doc : DocumentMeta) (c : TermElabM α)
    (cancelTk? : Option IO.CancelToken := none) : EIO Exception α :=
  snap.runCommandElabM doc (Command.liftTermElabM c) cancelTk?

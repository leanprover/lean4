...
def setStructureParents [Monad m] [MonadEnv m] [MonadError m] (structName : Name) (parentInfo : Array StructureParentInfo) : m Unit := do
  ...
  -- are invalidated by birth arithmetic; see `Environment.declChangeLog`.
  modifyEnv fun env => structureExt.addEntry (env.logDeclChange structName) { info with parentInfo }

...
@[inline, inherit_doc Raw.getKey?] def getEntry? (m : DHashMap α β) (a : α) : Option ((a : α) × β a) :=
  Raw₀.getEntry? ⟨m.1, m.2.size_buckets_pos⟩ a

@[inline, inherit_doc Raw.getKey] def getEntry (m : DHashMap α β) (a : α) (h : a ∈ m) : (a : α) × β a :=
  Raw₀.getEntry ⟨m.1, m.2.size_buckets_pos⟩ a h

@[inline, inherit_doc Raw.getKey!] def getEntry! [Inhabited ((a : α) × β a)] (m : DHashMap α β) (a : α) : (a : α) × β a :=
  Raw₀.getEntry! ⟨m.1, m.2.size_buckets_pos⟩ a

@[inline, inherit_doc Raw.getKeyD] def getEntryD (m : DHashMap α β) (a : α) (fallback : (a : α) × β a) : (a : α) × β a :=

...
-/
prelude
import Std.Data.DHashMap.Extensional

/-!
# Extensional dependent hash map lemmas

This file contains lemmas about `Std.Extensional.DHashMap`.
-/
...
namespace Std.Extensional.DHashMap
...
@[simp]
theorem emptyWithCapacity_eq {c} : (emptyWithCapacity c : DHashMap α β) = ∅ :=
  isEmpty_iff.mp Std.DHashMap.isEmpty_emptyWithCapacity

@[simp]
theorem not_insert_eq_empty {k : α} {v : β k} :
    ¬m.insert k v = ∅ :=
  m.inductionOn fun _ => isEmpty_eq_false_iff.mp Std.DHashMap.isEmpty_insert

theorem mem_iff_contains {a : α} : a ∈ m ↔ m.contains a :=
  Iff.rfl

theorem contains_congr {a b : α} (hab : a == b) : m.contains a = m.contains b :=
  m.inductionOn fun _ => Std.DHashMap.contains_congr hab

theorem mem_congr {a b : α} (hab : a == b) : a ∈ m ↔ b ∈ m :=
  m.inductionOn fun _ => Std.DHashMap.mem_congr hab

@[simp]
theorem contains_empty {a : α} : (∅ : Std.DHashMap α β).contains a = false :=
  Std.DHashMap.contains_empty

@[simp]
theorem not_mem_empty {a : α} : ¬a ∈ (∅ : Std.DHashMap α β) :=
  Std.DHashMap.not_mem_empty

theorem eq_empty_iff_forall_contains : m = ∅ ↔ ∀ a, m.contains a = false :=
  isEmpty_iff.symm.trans <| m.inductionOn fun _ => Std.DHashMap.isEmpty_iff_forall_contains

theorem eq_empty_iff_forall_not_mem : m = ∅ ↔ ∀ a, ¬a ∈ m :=
  isEmpty_iff.symm.trans <| m.inductionOn fun _ => Std.DHashMap.isEmpty_iff_forall_not_mem

@[simp] theorem insert_eq_insert {p : (a : α) × β a} : Insert.insert p m = m.insert p.1 p.2 := rfl

@[simp] theorem singleton_eq_insert {p : (a : α) × β a} :
    Singleton.singleton p = (∅ : Std.DHashMap α β).insert p.1 p.2 :=
  rfl

@[simp]
theorem contains_insert {k a : α} {v : β k} :
    (m.insert k v).contains a = (k == a || m.contains a) :=
  m.inductionOn fun _ => Std.DHashMap.contains_insert

@[simp]
theorem mem_insert {k a : α} {v : β k} : a ∈ m.insert k v ↔ k == a ∨ a ∈ m :=
  m.inductionOn fun _ => Std.DHashMap.mem_insert

theorem contains_of_contains_insert {k a : α} {v : β k} :
    (m.insert k v).contains a → (k == a) = false → m.contains a :=
  m.inductionOn fun _ => Std.DHashMap.contains_of_contains_insert

theorem mem_of_mem_insert {k a : α} {v : β k} : a ∈ m.insert k v → (k == a) = false → a ∈ m :=
  m.inductionOn fun _ => Std.DHashMap.mem_of_mem_insert
...
theorem erase_eq_empty_iff {k : α} : m.erase k = ∅ ↔ m = ∅ ∨ m.size = 1 ∧ k ∈ m := by
  ...
  simp only [Std.DHashMap.isEmpty_erase, Bool.or_eq_true, Bool.and_eq_true, beq_iff_eq]
  rfl

@[simp]
theorem contains_erase {k a : α} :
    (m.erase k).contains a = (!(k == a) && m.contains a) :=
  m.inductionOn fun _ => Std.DHashMap.contains_erase
...
theorem contains_of_contains_erase {k a : α} :
  ...
  m.inductionOn fun _ => Std.DHashMap.contains_of_contains_erase
...
theorem get?_erase [LawfulBEq α] {k a : α} :
  ...
  m.inductionOn fun _ => Std.DHashMap.get?_erase

@[simp]
theorem get?_erase_self [LawfulBEq α] {k : α} : (m.erase k).get? k = none :=
  m.inductionOn fun _ => Std.DHashMap.get?_erase_self
...
theorem get?_insert {k a : α} {v : β} :
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.get?_insert

@[simp]
theorem get?_insert_self {k : α} {v : β} :
    get? (m.insert k v) k = some v :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_insert_self

theorem contains_eq_isSome_get? {a : α} :
    m.contains a = (get? m a).isSome :=
  m.inductionOn fun _ => Std.DHashMap.Const.contains_eq_isSome_get?

theorem mem_iff_isSome_get? {a : α} : a ∈ m ↔ (get? m a).isSome :=
  m.inductionOn fun _ => Std.DHashMap.Const.mem_iff_isSome_get?

theorem get?_eq_none_of_contains_eq_false {a : α} :
    m.contains a = false → get? m a = none :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_eq_none_of_contains_eq_false
...
theorem get?_erase {k a : α} :
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.get?_erase

@[simp]
theorem get?_erase_self {k : α} : get? (m.erase k) k = none :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_erase_self

theorem get?_eq_get? [LawfulBEq α] {a : α} : get? m a = m.get? a :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_eq_get?

theorem get?_congr {a b : α} (hab : a == b) : get? m a = get? m b :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_congr hab
...
theorem get!_eq_default_of_contains_eq_false [LawfulBEq α] {a : α} [Inhabited (β a)] :
  ...
  m.inductionOn fun _ => Std.DHashMap.get!_eq_default_of_contains_eq_false

theorem get!_eq_default [LawfulBEq α] {a : α} [Inhabited (β a)] :
    ¬a ∈ m → m.get! a = default :=
  m.inductionOn fun _ => Std.DHashMap.get!_eq_default
...
theorem get?_eq_some_get!_of_contains [LawfulBEq α] {a : α} [Inhabited (β a)] :
  ...
  m.inductionOn fun _ => Std.DHashMap.get?_eq_some_get!_of_contains

theorem get?_eq_some_get! [LawfulBEq α] {a : α} [Inhabited (β a)] :
    a ∈ m → m.get? a = some (m.get! a) :=
  m.inductionOn fun _ => Std.DHashMap.get?_eq_some_get!

theorem get!_eq_get!_get? [LawfulBEq α] {a : α} [Inhabited (β a)] :
    m.get! a = (m.get? a).get! :=
  m.inductionOn fun _ => Std.DHashMap.get!_eq_get!_get?

theorem get_eq_get! [LawfulBEq α] {a : α} [Inhabited (β a)] {h} :
    m.get a h = m.get! a :=
  m.inductionOn (fun _ _ => Std.DHashMap.get_eq_get!) h
...
theorem get!_eq_default_of_contains_eq_false [Inhabited β] {a : α} :
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.get!_eq_default_of_contains_eq_false

theorem get!_eq_default [Inhabited β] {a : α} :
    ¬a ∈ m → get! m a = default :=
  m.inductionOn fun _ => Std.DHashMap.Const.get!_eq_default
...
theorem get?_eq_some_get!_of_contains [Inhabited β] {a : α} :
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.get?_eq_some_get!_of_contains

theorem get?_eq_some_get! [Inhabited β] {a : α} :
    a ∈ m → get? m a = some (get! m a) :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_eq_some_get!

theorem get!_eq_get!_get? [Inhabited β] {a : α} :
    get! m a = (get? m a).get! :=
  m.inductionOn fun _ => Std.DHashMap.Const.get!_eq_get!_get?
...
theorem get!_congr [Inhabited β] {a b : α} (hab : a == b) :
    get! m a = get! m b :=
  m.inductionOn (fun _ hab => Std.DHashMap.Const.get!_congr hab) hab
...
theorem getKey!_eq_default_of_contains_eq_false [Inhabited α]
  ...
  m.inductionOn fun _ => Std.DHashMap.getKey!_eq_default_of_contains_eq_false

theorem getKey!_eq_default [Inhabited α] {a : α} :
    ¬a ∈ m → m.getKey! a = default :=
  m.inductionOn fun _ => Std.DHashMap.getKey!_eq_default
...
theorem getKey?_eq_some_getKey!_of_contains [Inhabited α] {a : α} :
  ...
  m.inductionOn fun _ => Std.DHashMap.getKey?_eq_some_getKey!_of_contains

theorem getKey?_eq_some_getKey! [Inhabited α] {a : α} :
    a ∈ m → m.getKey? a = some (m.getKey! a) :=
  m.inductionOn fun _ => Std.DHashMap.getKey?_eq_some_getKey!

theorem getKey!_eq_get!_getKey? [Inhabited α] {a : α} :
    m.getKey! a = (m.getKey? a).get! :=
  m.inductionOn fun _ => Std.DHashMap.getKey!_eq_get!_getKey?

theorem getKey_eq_getKey! [Inhabited α] {a : α} {h} :
    m.getKey a h = m.getKey! a :=
  m.inductionOn (fun _ _ => Std.DHashMap.getKey_eq_getKey!) h
...
theorem getKey!_eq_of_contains [LawfulBEq α] [Inhabited α] {k : α} (h : m.contains k) :
    m.getKey! k = k :=
  m.inductionOn (fun _ h => Std.DHashMap.getKey!_eq_of_contains h) h

theorem getKey!_eq_of_mem [LawfulBEq α] [Inhabited α] {k : α} (h : k ∈ m) : m.getKey! k = k :=
  m.inductionOn (fun _ h => Std.DHashMap.getKey!_eq_of_mem h) h
...
theorem size_insertIfNew {k : α} {v : β k} :
  ...
  m.inductionOn fun _ => Std.DHashMap.size_insertIfNew

theorem size_le_size_insertIfNew {k : α} {v : β k} :
    m.size ≤ (m.insertIfNew k v).size :=
  m.inductionOn fun _ => Std.DHashMap.size_le_size_insertIfNew

theorem size_insertIfNew_le {k : α} {v : β k} :
    (m.insertIfNew k v).size ≤ m.size + 1 :=
  m.inductionOn fun _ => Std.DHashMap.size_insertIfNew_le
...
theorem get?_insertIfNew [LawfulBEq α] {k a : α} {v : β k} : (m.insertIfNew k v).get? a =
  ...
  m.inductionOn fun _ => Std.DHashMap.get?_insertIfNew
...
theorem get_insertIfNew [LawfulBEq α] {k a : α} {v : β k} {h₁} : (m.insertIfNew k v).get a h₁ =
  ...
  m.inductionOn (fun _ _ => Std.DHashMap.get_insertIfNew) h₁
...
theorem getD_insertIfNew [LawfulBEq α] {k a : α} {fallback : β a} {v : β k} :
  ...
  m.inductionOn fun _ => Std.DHashMap.getD_insertIfNew
...
theorem getD_insertIfNew {k a : α} {fallback v : β} :
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getD_insertIfNew

end Const

theorem getKey?_insertIfNew {k a : α} {v : β k} :
    getKey? (m.insertIfNew k v) a = if k == a ∧ ¬k ∈ m then some k else getKey? m a :=
  m.inductionOn fun _ => Std.DHashMap.getKey?_insertIfNew
...
theorem getKey_insertIfNew {k a : α} {v : β k} {h₁} :
  ...
  m.inductionOn (fun _ _ => Std.DHashMap.getKey_insertIfNew) h₁
...
theorem getKeyD_insertIfNew {k a fallback : α} {v : β k} :
  ...
  m.inductionOn fun _ => Std.DHashMap.getKeyD_insertIfNew

@[simp]
theorem getThenInsertIfNew?_fst [LawfulBEq α] {k : α} {v : β k} :
    (m.getThenInsertIfNew? k v).1 = m.get? k :=
  m.inductionOn fun _ => Std.DHashMap.getThenInsertIfNew?_fst

@[simp]
theorem getThenInsertIfNew?_snd [LawfulBEq α] {k : α} {v : β k} :
    (m.getThenInsertIfNew? k v).2 = m.insertIfNew k v :=
  m.inductionOn fun _ => congrArg Quotient.mk' Std.DHashMap.getThenInsertIfNew?_snd
...
theorem getThenInsertIfNew?_fst {k : α} {v : β} : (getThenInsertIfNew? m k v).1 = get? m k :=
  m.inductionOn fun _ => Std.DHashMap.Const.getThenInsertIfNew?_fst

@[simp]
theorem getThenInsertIfNew?_snd {k : α} {v : β} :
    (getThenInsertIfNew? m k v).2 = m.insertIfNew k v :=
  m.inductionOn fun _ => congrArg Quotient.mk' Std.DHashMap.Const.getThenInsertIfNew?_snd
...
private theorem insertMany_list_mk {m : Std.DHashMap α β} {l : List ((a : α) × β a)} :
    (DHashMap.insertMany (Quotient.mk _ m) l : DHashMap α β) = Quotient.mk _ (m.insertMany l) := by
  simp only [Quotient.mk]
  induction l generalizing m with
  | nil => rfl
  | cons x l ih =>
    rcases x with ⟨k, v⟩
    simp only [insertMany_cons, Std.DHashMap.insertMany_cons, insert,
...
theorem contains_insertMany_list
  ...
  exact Std.DHashMap.contains_insertMany_list
...
theorem mem_insertMany_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertMany_list_mk]
  exact Std.DHashMap.mem_insertMany_list
...
theorem mem_of_mem_insertMany_list
  ...
  exact Std.DHashMap.mem_of_mem_insertMany_list mem contains_eq_false
...
theorem get?_insertMany_list_of_contains_eq_false [LawfulBEq α]
  ...
  simp only [insertMany_list_mk]
  exact Std.DHashMap.get?_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem get?_insertMany_list_of_mem [LawfulBEq α]
  ...
  simp only [insertMany_list_mk]
  exact Std.DHashMap.get?_insertMany_list_of_mem k_beq distinct mem
...
theorem get_insertMany_list_of_contains_eq_false [LawfulBEq α]
  ...
  simp only [insertMany_list_mk]
  exact Std.DHashMap.get_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem get_insertMany_list_of_mem [LawfulBEq α]
  ...
  simp only [insertMany_list_mk]
  exact Std.DHashMap.get_insertMany_list_of_mem k_beq distinct mem
...
theorem getD_insertMany_list_of_contains_eq_false [LawfulBEq α]
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getD_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getD_insertMany_list_of_mem [LawfulBEq α]
  ...
  refine m.inductionOn (fun _ k_beq distinct mem => ?_) k_beq distinct mem
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getD_insertMany_list_of_mem k_beq distinct mem
...
theorem getKey?_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getKey?_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getKey?_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem => ?_) k_beq distinct mem
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getKey?_insertMany_list_of_mem k_beq distinct mem
...
theorem getKey_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false _ => ?_) contains_eq_false h
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getKey_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getKey_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem _ => ?_) k_beq distinct mem h
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getKey_insertMany_list_of_mem k_beq distinct mem
...
theorem getKeyD_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getKeyD_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getKeyD_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem => ?_) k_beq distinct mem
  simp only [insertMany_list_mk]
  exact Std.DHashMap.getKeyD_insertMany_list_of_mem k_beq distinct mem
...
theorem size_insertMany_list
  ...
  simp only [insertMany_list_mk]
  exact Std.DHashMap.size_insertMany_list distinct
...
theorem size_le_size_insertMany_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertMany_list_mk]
  exact Std.DHashMap.size_le_size_insertMany_list
...
theorem size_insertMany_list_le
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertMany_list_mk]
  exact Std.DHashMap.size_insertMany_list_le
...
theorem insertMany_list_eq_empty_iff {l : List ((a : α) × β a)} :
  ...
  exact Std.DHashMap.isEmpty_insertMany_list
...
private theorem insertMany_list_mk {m : Std.DHashMap α fun _ => β} {l : List (α × β)} :
    (insertMany (Quotient.mk _ m) l : DHashMap α fun _ => β) =
      Quotient.mk _ (Std.DHashMap.Const.insertMany m l) := by
  simp only [Quotient.mk]
  induction l generalizing m with
  | nil => rfl
  | cons x l ih =>
    rcases x with ⟨k, v⟩
    simp only [insertMany_cons, Std.DHashMap.Const.insertMany_cons, insert,
      Quotient.mk', Quotient.mk, Quotient.lift, ih]

@[elab_as_elim]
theorem insertMany_ind {motive : DHashMap α (fun _ => β) → Prop}
    (m : DHashMap α fun _ => β) (l : ρ)
    (init : motive m) (insert : ∀ m a b, motive m → motive (m.insert a b)) :
...
theorem contains_insertMany_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.contains_insertMany_list
...
theorem mem_insertMany_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.mem_insertMany_list
...
theorem mem_of_mem_insertMany_list
  ...
  refine m.inductionOn (fun _ mem contains_eq_false => ?_) mem contains_eq_false
  simp only [insertMany_list_mk] at mem
  exact Std.DHashMap.Const.mem_of_mem_insertMany_list mem contains_eq_false
...
theorem getKey?_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getKey?_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getKey?_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem => ?_) k_beq distinct mem
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getKey?_insertMany_list_of_mem k_beq distinct mem
...
theorem getKey_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false _ => ?_) contains_eq_false h
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getKey_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getKey_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem _ => ?_) k_beq distinct mem h
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getKey_insertMany_list_of_mem k_beq distinct mem
...
theorem getKeyD_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getKeyD_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getKeyD_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem => ?_) k_beq distinct mem
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getKeyD_insertMany_list_of_mem k_beq distinct mem
...
theorem size_insertMany_list
  ...
  refine m.inductionOn (fun _ distinct => ?_) distinct
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.size_insertMany_list distinct
...
theorem size_le_size_insertMany_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.size_le_size_insertMany_list
...
theorem size_insertMany_list_le
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.size_insertMany_list_le
...
theorem insertMany_list_eq_empty_iff {l : List (α × β)} :
  ...
  simp only [insertMany_list_mk, ← isEmpty_iff, ← List.isEmpty_iff,
    Bool.coe_iff_coe, ← Bool.and_eq_true]
  exact Std.DHashMap.Const.isEmpty_insertMany_list
...
theorem get?_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.get?_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem get?_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem => ?_) k_beq distinct mem
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.get?_insertMany_list_of_mem k_beq distinct mem
...
theorem get_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false _ => ?_) contains_eq_false h
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.get_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem get_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem _ => ?_) k_beq distinct mem h
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.get_insertMany_list_of_mem k_beq distinct mem
...
theorem getD_insertMany_list_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getD_insertMany_list_of_contains_eq_false contains_eq_false
...
theorem getD_insertMany_list_of_mem
  ...
  refine m.inductionOn (fun _ k_beq distinct mem => ?_) k_beq distinct mem
  simp only [insertMany_list_mk]
  exact Std.DHashMap.Const.getD_insertMany_list_of_mem k_beq distinct mem
...
private theorem insertManyIfNewUnit_list_mk {m : Std.DHashMap α fun _ => Unit} {l : List α} :
    (insertManyIfNewUnit (Quotient.mk _ m) l : DHashMap α fun _ => Unit) =
      Quotient.mk _ (Std.DHashMap.Const.insertManyIfNewUnit m l) := by
  simp only [Quotient.mk]
  induction l generalizing m with
  | nil => rfl
  | cons x l ih =>
    simp only [insertManyIfNewUnit_cons, Std.DHashMap.Const.insertManyIfNewUnit_cons, insertIfNew,
      Quotient.mk', Quotient.mk, Quotient.lift, ih]
...
theorem contains_insertManyIfNewUnit_list
  ...
  exact Std.DHashMap.Const.contains_insertManyIfNewUnit_list
...
theorem mem_insertManyIfNewUnit_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.mem_insertManyIfNewUnit_list
...
theorem mem_of_mem_insertManyIfNewUnit_list
  ...
  refine m.inductionOn (fun _ contains_eq_false => ?_) contains_eq_false
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.mem_of_mem_insertManyIfNewUnit_list contains_eq_false
...
theorem getKey?_insertManyIfNewUnit_list_of_not_mem_of_contains_eq_false
  ...
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKey?_insertManyIfNewUnit_list_of_not_mem_of_contains_eq_false not_mem contains_eq_false
...
theorem getKey?_insertManyIfNewUnit_list_of_not_mem_of_mem
  ...
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKey?_insertManyIfNewUnit_list_of_not_mem_of_mem k_beq not_mem distinct mem
...
theorem getKey?_insertManyIfNewUnit_list_of_mem
  ...
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKey?_insertManyIfNewUnit_list_of_mem h'
...
theorem getKey_insertManyIfNewUnit_list_of_not_mem_of_mem
  ...
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKey_insertManyIfNewUnit_list_of_not_mem_of_mem k_beq not_mem distinct mem
...
theorem getKey_insertManyIfNewUnit_list_of_mem
  ...
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKey_insertManyIfNewUnit_list_of_mem mem
...
theorem getKey!_insertManyIfNewUnit_list_of_mem
  ...
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKey!_insertManyIfNewUnit_list_of_mem mem
...
theorem getKeyD_insertManyIfNewUnit_list_of_not_mem_of_contains_eq_false
  ...
  refine m.inductionOn (fun _ not_mem contains_eq_false => ?_) not_mem contains_eq_false
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKeyD_insertManyIfNewUnit_list_of_not_mem_of_contains_eq_false not_mem contains_eq_false
...
theorem getKeyD_insertManyIfNewUnit_list_of_not_mem_of_mem
  ...
  refine m.inductionOn (fun _ k_beq not_mem distinct mem => ?_) k_beq not_mem distinct mem
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKeyD_insertManyIfNewUnit_list_of_not_mem_of_mem k_beq not_mem distinct mem
...
theorem getKeyD_insertManyIfNewUnit_list_of_mem
  ...
  refine m.inductionOn (fun _ mem => ?_) mem
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.getKeyD_insertManyIfNewUnit_list_of_mem mem
...
theorem size_insertManyIfNewUnit_list
  ...
  refine m.inductionOn (fun _ distinct => ?_) distinct
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.size_insertManyIfNewUnit_list distinct
...
theorem size_le_size_insertManyIfNewUnit_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.size_le_size_insertManyIfNewUnit_list
...
theorem size_insertManyIfNewUnit_list_le
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.size_insertManyIfNewUnit_list_le
...
theorem insertManyIfNewUnit_list_eq_empty_iff {l : List α} :
  ...
  simp only [insertManyIfNewUnit_list_mk, ← isEmpty_iff, ← List.isEmpty_iff,
    Bool.coe_iff_coe, ← Bool.and_eq_true]
  exact Std.DHashMap.Const.isEmpty_insertManyIfNewUnit_list
...
theorem get?_insertManyIfNewUnit_list
  ...
  refine m.inductionOn fun _ => ?_
  simp only [insertManyIfNewUnit_list_mk]
  exact Std.DHashMap.Const.get?_insertManyIfNewUnit_list
...
theorem ofList_singleton {k : α} {v : β k} :
    ofList [⟨k, v⟩] = (∅ : DHashMap α β).insert k v := rfl
...
private theorem ofList_eq_mk {l : List ((a : α) × β a)} :
    (ofList l : DHashMap α β) = Quotient.mk _ (Std.DHashMap.ofList l) := by
...
theorem contains_ofList
  ...
  exact Std.DHashMap.contains_ofList
...
theorem get?_ofList_of_contains_eq_false [LawfulBEq α]
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.get?_ofList_of_contains_eq_false contains_eq_false
...
theorem get?_ofList_of_mem [LawfulBEq α]
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.get?_ofList_of_mem k_beq distinct mem
...
theorem get_ofList_of_mem [LawfulBEq α]
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.get_ofList_of_mem k_beq distinct mem
...
theorem getD_ofList_of_contains_eq_false [LawfulBEq α]
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.getD_ofList_of_contains_eq_false contains_eq_false
...
theorem getD_ofList_of_mem [LawfulBEq α]
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.getD_ofList_of_mem k_beq distinct mem
...
theorem getKey?_ofList_of_contains_eq_false
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.getKey?_ofList_of_contains_eq_false contains_eq_false
...
theorem getKey?_ofList_of_mem
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.getKey?_ofList_of_mem k_beq distinct mem
...
theorem getKey_ofList_of_mem
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.getKey_ofList_of_mem k_beq distinct mem
...
theorem getKeyD_ofList_of_contains_eq_false
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.getKeyD_ofList_of_contains_eq_false contains_eq_false
...
theorem getKeyD_ofList_of_mem
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.getKeyD_ofList_of_mem k_beq distinct mem

theorem size_ofList
    {l : List ((a : α) × β a)} (distinct : l.Pairwise (fun a b => (a.1 == b.1) = false)) :
    (ofList l).size = l.length := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.size_ofList distinct

theorem size_ofList_le
    {l : List ((a : α) × β a)} :
    (ofList l).size ≤ l.length := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.size_ofList_le

@[simp]
theorem ofList_eq_empty_iff {l : List ((a : α) × β a)} :
    ofList l = ∅ ↔ l = [] := by
  simp only [ofList_eq_mk, ← isEmpty_iff, ← List.isEmpty_iff, Bool.coe_iff_coe]
  exact Std.DHashMap.isEmpty_ofList
...
private theorem ofList_eq_mk {l : List (α × β)} :
    (ofList l : DHashMap α fun _ => β) = Quotient.mk _ (Std.DHashMap.Const.ofList l) := by
  change insertMany (Quotient.mk _ _) _ = _
  rw [insertMany_list_mk]
  rfl
...
theorem contains_ofList
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.contains_ofList
...
theorem mem_ofList
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.mem_ofList
...
theorem get?_ofList_of_contains_eq_false
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.get?_ofList_of_contains_eq_false contains_eq_false
...
theorem get?_ofList_of_mem
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.get?_ofList_of_mem k_beq distinct mem
...
theorem get_ofList_of_mem
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.get_ofList_of_mem k_beq distinct mem
...
theorem getD_ofList_of_contains_eq_false
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.getD_ofList_of_contains_eq_false contains_eq_false
...
theorem getD_ofList_of_mem
  ...
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.getD_ofList_of_mem k_beq distinct mem
...
theorem getKey?_ofList_of_contains_eq_false
    ...
    (ofList l).getKey? k = none := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.getKey?_ofList_of_contains_eq_false contains_eq_false
...
theorem getKey?_ofList_of_mem
    ...
    (ofList l).getKey? k' = some k := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.getKey?_ofList_of_mem k_beq distinct mem
...
theorem getKey_ofList_of_mem
    ...
    (ofList l).getKey k' h = k := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.getKey_ofList_of_mem k_beq distinct mem
...
theorem getKeyD_ofList_of_contains_eq_false
    ...
    (ofList l).getKeyD k fallback = fallback := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.getKeyD_ofList_of_contains_eq_false contains_eq_false
...
theorem getKeyD_ofList_of_mem
    ...
    (ofList l).getKeyD k' fallback = k := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.getKeyD_ofList_of_mem k_beq distinct mem

theorem size_ofList
    {l : List (α × β)} (distinct : l.Pairwise (fun a b => (a.1 == b.1) = false)) :
    (ofList l).size = l.length := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.size_ofList distinct

theorem size_ofList_le
    {l : List (α × β)} :
    (ofList l).size ≤ l.length := by
  simp only [ofList_eq_mk]
  exact Std.DHashMap.Const.size_ofList_le

@[simp]
theorem ofList_eq_empty_iff {l : List (α × β)} :
    ofList l = ∅ ↔ l = [] := by
  simp only [ofList_eq_mk, ← isEmpty_iff, ← List.isEmpty_iff, Bool.coe_iff_coe]
  exact Std.DHashMap.Const.isEmpty_ofList
...
private theorem unitOfList_eq_mk {l : List α} :
    (unitOfList l : DHashMap α fun _ => Unit) = Quotient.mk _ (Std.DHashMap.Const.unitOfList l) := by
...
theorem contains_unitOfList
  ...
  exact Std.DHashMap.Const.contains_unitOfList
...
theorem mem_unitOfList
  ...
  simp only [unitOfList_eq_mk]
  exact Std.DHashMap.Const.mem_unitOfList

theorem getKey?_unitOfList_of_contains_eq_false
    {l : List α} {k : α} (contains_eq_false : l.contains k = false) :
    getKey? (unitOfList l) k = none := by
  simp only [unitOfList_eq_mk]
  exact Std.DHashMap.Const.getKey?_unitOfList_of_contains_eq_false contains_eq_false
...
theorem getKey?_unitOfList_of_mem
  ...
  simp only [unitOfList_eq_mk]
  exact Std.DHashMap.Const.getKey?_unitOfList_of_mem k_beq distinct mem
...
theorem getKey_unitOfList_of_mem
  ...
  simp only [unitOfList_eq_mk]
  exact Std.DHashMap.Const.getKey_unitOfList_of_mem k_beq distinct mem
...
theorem getKeyD_unitOfList_of_contains_eq_false
  ...
  simp only [unitOfList_eq_mk]
  exact Std.DHashMap.Const.getKeyD_unitOfList_of_contains_eq_false contains_eq_false
...
theorem getKeyD_unitOfList_of_mem
  ...
  simp only [unitOfList_eq_mk]
  exact Std.DHashMap.Const.getKeyD_unitOfList_of_mem k_beq distinct mem
...
theorem get?_unitOfList
  ...
  simp only [unitOfList_eq_mk]
  exact Std.DHashMap.Const.get?_unitOfList
...
theorem alter_eq_empty_iff_erase_eq_empty [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)} :
  ...
  simp only [Std.DHashMap.isEmpty_alter_eq_isEmpty_erase, Bool.and_eq_true, Option.isNone_iff_eq_none]
  rfl
...
theorem contains_alter_of_beq_eq_false [LawfulBEq α] {k k' : α} {f : Option (β k) → Option (β k)}
  ...
  m.inductionOn (fun _ h => Std.DHashMap.contains_alter_of_beq_eq_false h) h

theorem mem_alter_of_beq_eq_false [LawfulBEq α] {k k' : α} {f : Option (β k) → Option (β k)}
    (h : (k == k') = false) : k' ∈ m.alter k f ↔ k' ∈ m :=
  m.inductionOn (fun _ h => Std.DHashMap.mem_alter_of_beq_eq_false h) h
...
theorem size_alter_eq_self_of_not_mem [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)}
  ...
  m.inductionOn (fun _ h h' => Std.DHashMap.size_alter_eq_self_of_not_mem h h') h h'
...
theorem size_alter_eq_self_of_mem [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)}
  ...
  m.inductionOn (fun _ h h' => Std.DHashMap.size_alter_eq_self_of_mem h h') h h'

theorem size_alter_le_size [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)} :
    (m.alter k f).size ≤ m.size + 1 :=
  m.inductionOn fun _ => Std.DHashMap.size_alter_le_size

theorem size_le_size_alter [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)} :
    m.size - 1 ≤ (m.alter k f).size :=
  m.inductionOn fun _ => Std.DHashMap.size_le_size_alter
...
theorem get?_alter [LawfulBEq α] {k k' : α} {f : Option (β k) → Option (β k)} :
    (m.alter k f).get? k' =
      if h : k == k' then
        ...
        m.get? k' :=
  m.inductionOn fun _ => Std.DHashMap.get?_alter

@[simp]
theorem get?_alter_self [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)} :
    (m.alter k f).get? k = f (m.get? k) :=
  m.inductionOn fun _ => Std.DHashMap.get?_alter_self
...
theorem get_alter [LawfulBEq α] {k k' : α} {f : Option (β k) → Option (β k)}
    ...
    (m.alter k f).get k' h =
      if heq : k == k' then
        ...
        m.get k' h' :=
  m.inductionOn (fun _ _ => Std.DHashMap.get_alter) h
...
theorem get_alter_self [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)}
  ...
  m.inductionOn (fun _ _ => Std.DHashMap.get_alter_self) h
...
theorem getD_alter [LawfulBEq α] {k k' : α} {fallback : β k'} {f : Option (β k) → Option (β k)} :
    (m.alter k f).getD k' fallback =
      if heq : k == k' then
        ...
        m.getD k' fallback :=
  m.inductionOn fun _ => Std.DHashMap.getD_alter
...
theorem getD_alter_self [LawfulBEq α] {k : α} {fallback : β k} {f : Option (β k) → Option (β k)} :
  ...
  m.inductionOn fun _ => Std.DHashMap.getD_alter_self
...
theorem getKey?_alter [LawfulBEq α] {k k' : α} {f : Option (β k) → Option (β k)} :
    (m.alter k f).getKey? k' =
      if k == k' then
        ...
        m.getKey? k' :=
  m.inductionOn fun _ => Std.DHashMap.getKey?_alter
...
theorem getKey?_alter_self [LawfulBEq α] {k : α} {f : Option (β k) → Option (β k)} :
  ...
  m.inductionOn fun _ => Std.DHashMap.getKey?_alter_self
...
theorem getKeyD_alter [LawfulBEq α] {k k' fallback : α} {f : Option (β k) → Option (β k)} :
  ...
  m.inductionOn fun _ => Std.DHashMap.getKeyD_alter
...
theorem getKeyD_alter_self [LawfulBEq α] [Inhabited α] {k : α} {fallback : α}
  ...
  m.inductionOn fun _ => Std.DHashMap.getKeyD_alter_self
...
theorem alter_eq_empty_iff_erase_eq_empty {k : α} {f : Option β → Option β} :
  ...
  simp only [← isEmpty_iff]
  dsimp only [isEmpty, alter, Quotient.lift, Quotient.mk', Quotient.mk]
  simp only [Std.DHashMap.Const.isEmpty_alter_eq_isEmpty_erase, Bool.and_eq_true,
...
theorem size_alter_eq_self_of_not_mem {k : α} {f : Option β → Option β}
  ...
  m.inductionOn (fun _ h h' => Std.DHashMap.Const.size_alter_eq_self_of_not_mem h h') h h'

theorem size_alter_eq_self_of_mem {k : α} {f : Option β → Option β}
    (h : k ∈ m) (h' : (f (Const.get? m k)).isSome) : (Const.alter m k f).size = m.size :=
  m.inductionOn (fun _ h h' => Std.DHashMap.Const.size_alter_eq_self_of_mem h h') h h'

theorem size_alter_le_size {k : α} {f : Option β → Option β} :
    (Const.alter m k f).size ≤ m.size + 1 :=
  m.inductionOn fun _ => Std.DHashMap.Const.size_alter_le_size

theorem size_le_size_alter {k : α} {f : Option β → Option β} :
    m.size - 1 ≤ (Const.alter m k f).size :=
  m.inductionOn fun _ => Std.DHashMap.Const.size_le_size_alter
...
theorem get?_alter {k k' : α} {f : Option β → Option β} :
    Const.get? (Const.alter m k f) k' =
      if k == k' then
        ...
        Const.get? m k' :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_alter

@[simp]
theorem get?_alter_self {k : α} {f : Option β → Option β} :
    Const.get? (Const.alter m k f) k = f (Const.get? m k) :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_alter_self
...
theorem get_alter {k k' : α} {f : Option β → Option β}
    ...
    Const.get (Const.alter m k f) k' h =
      if heq : k == k' then
        ...
        haveI h' : k' ∈ m := mem_alter_of_beq_eq_false (Bool.not_eq_true _ ▸ heq) |>.mp h
        Const.get m k' h' :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.get_alter) h
...
theorem get_alter_self {k : α} {f : Option β → Option β}
  ...
  m.inductionOn (fun _ _ => Std.DHashMap.Const.get_alter_self) h
...
theorem getKey?_alter {k k' : α} {f : Option β → Option β} :
    (Const.alter m k f).getKey? k' =
      if k == k' then
        ...
        m.getKey? k' :=
  m.inductionOn fun _ => Std.DHashMap.Const.getKey?_alter
...
theorem getKey?_alter_self {k : α} {f : Option β → Option β} :
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getKey?_alter_self
...
theorem getKey_alter [Inhabited α] {k k' : α}
    ...
    (Const.alter m k f).getKey k' h =
      if heq : k == k' then
        ...
        haveI h' : k' ∈ m := mem_alter_of_beq_eq_false (Bool.not_eq_true _ ▸ heq) |>.mp h
        m.getKey k' h' :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.getKey_alter) h

@[simp]
theorem getKey_alter_self [Inhabited α] {k : α}
    {f : Option β → Option β} {h : k ∈ Const.alter m k f} :
    (Const.alter m k f).getKey k h = k :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.getKey_alter_self) h
...
theorem getKeyD_alter {k k' fallback : α} {f : Option β → Option β} :
    (Const.alter m k f).getKeyD k' fallback =
      if k == k' then
        ...
        m.getKeyD k' fallback :=
  m.inductionOn fun _ => Std.DHashMap.Const.getKeyD_alter

theorem getKeyD_alter_self [Inhabited α] {k fallback : α}
    {f : Option β → Option β} :
    (Const.alter m k f).getKeyD k fallback =
      if (f (Const.get? m k)).isSome then k else fallback :=
  m.inductionOn fun _ => Std.DHashMap.Const.getKeyD_alter_self
...
theorem get?_modify [LawfulBEq α] {k k' : α} {f : β k → β k} :
    (m.modify k f).get? k' = if h : k == k' then
      ...
      m.get? k' :=
  m.inductionOn fun _ => Std.DHashMap.get?_modify

@[simp]
theorem get?_modify_self [LawfulBEq α] {k : α} {f : β k → β k} :
    (m.modify k f).get? k = (m.get? k).map f :=
  m.inductionOn fun _ => Std.DHashMap.get?_modify_self
...
theorem get_modify [LawfulBEq α] {k k' : α} {f : β k → β k}
    ...
    (m.modify k f).get k' h =
      if heq : k == k' then
        ...
        m.get k' h' :=
  m.inductionOn (fun _ h => Std.DHashMap.get_modify h) h

@[simp]
theorem get_modify_self [LawfulBEq α] {k : α} {f : β k → β k} {h : k ∈ m.modify k f} :
    haveI h' : k ∈ m := mem_modify.mp h
    (m.modify k f).get k h = f (m.get k h') :=
  m.inductionOn (fun _ _ => Std.DHashMap.get_modify_self) h
...
theorem getKeyD_modify [LawfulBEq α] {k k' fallback : α} {f : β k → β k} :
    (m.modify k f).getKeyD k' fallback =
      if k == k' then
        ...
        m.getKeyD k' fallback :=
  m.inductionOn fun _ => Std.DHashMap.getKeyD_modify
...
theorem getKeyD_modify_self [LawfulBEq α] [Inhabited α] {k fallback : α} {f : β k → β k} :
  ...
  m.inductionOn fun _ => Std.DHashMap.getKeyD_modify_self
...
theorem modify_eq_empty_iff {k : α} {f : β → β} :
  ...
  simp only [← isEmpty_iff, Bool.coe_iff_coe]
  exact m.inductionOn fun _ => Std.DHashMap.Const.isEmpty_modify

@[simp]
theorem contains_modify {k k': α} {f : β → β} :
    (Const.modify m k f).contains k' = m.contains k' :=
  m.inductionOn fun _ => Std.DHashMap.Const.contains_modify

@[simp]
theorem mem_modify {k k': α} {f : β → β} :
    k' ∈ Const.modify m k f ↔ k' ∈ m :=
  m.inductionOn fun _ => Std.DHashMap.Const.mem_modify

@[simp]
theorem size_modify {k : α} {f : β → β} :
    (Const.modify m k f).size = m.size :=
  m.inductionOn fun _ => Std.DHashMap.Const.size_modify
...
theorem get?_modify {k k' : α} {f : β → β} :
    Const.get? (Const.modify m k f) k' = if k == k' then
      ...
      Const.get? m k' :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_modify

@[simp]
theorem get?_modify_self {k : α} {f : β → β} :
    Const.get? (Const.modify m k f) k = (Const.get? m k).map f :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_modify_self
...
theorem get_modify {k k' : α} {f : β → β}
    ...
    Const.get (Const.modify m k f) k' h =
      if heq : k == k' then
        ...
        haveI h' : k' ∈ m := mem_modify.mp h
        Const.get m k' h' :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.get_modify) h
...
theorem get_modify_self {k : α} {f : β → β}
  ...
  m.inductionOn (fun _ _ => Std.DHashMap.Const.get_modify_self) h
...
theorem getKey?_modify {k k' : α} {f : β → β} :
    (Const.modify m k f).getKey? k' =
      if k == k' then
        ...
        m.getKey? k' :=
  m.inductionOn fun _ => Std.DHashMap.Const.getKey?_modify

theorem getKey?_modify_self {k : α} {f : β → β} :
    (Const.modify m k f).getKey? k = if k ∈ m then some k else none :=
  m.inductionOn fun _ => Std.DHashMap.Const.getKey?_modify_self
...
theorem getKey_modify [Inhabited α] {k k' : α} {f : β → β}
    ...
    (Const.modify m k f).getKey k' h =
      if k == k' then
        ...
        haveI h' : k' ∈ m := mem_modify.mp h
        m.getKey k' h' :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.getKey_modify) h

@[simp]
theorem getKey_modify_self [Inhabited α] {k : α} {f : β → β}
    {h : k ∈ Const.modify m k f} : (Const.modify m k f).getKey k h = k :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.getKey_modify_self) h
...
theorem getKeyD_modify {k k' fallback : α} {f : β → β} :
    (Const.modify m k f).getKeyD k' fallback =
      if k == k' then
        ...
        m.getKeyD k' fallback :=
  m.inductionOn fun _ => Std.DHashMap.Const.getKeyD_modify
...
theorem getKeyD_modify_self [Inhabited α] {k fallback : α} {f : β → β} :
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getKeyD_modify_self
...
theorem get_filterMap [LawfulBEq α]
  ...
  m.inductionOn (fun _ _ => Std.DHashMap.get_filterMap) h'
...
theorem getD_filterMap [LawfulBEq α]
  ...
  m.inductionOn fun _ => Std.DHashMap.getD_filterMap
...
theorem getKey?_filterMap [LawfulBEq α]
  ...
  m.inductionOn fun _ => Std.DHashMap.getKey?_filterMap

@[simp]
theorem getKey_filterMap
    {f : (a : α) → β a → Option (γ a)} {k : α} {h'} :
    (m.filterMap f).getKey k h' = m.getKey k (mem_of_mem_filterMap h') :=
  m.inductionOn (fun _ _ => Std.DHashMap.getKey_filterMap) h'
...
theorem getKeyD_filterMap [LawfulBEq α]
  ...
  m.inductionOn fun _ => Std.DHashMap.getKeyD_filterMap
...
theorem filterMap_eq_empty_iff {f : α → β → Option γ} :
  ...
  isEmpty_iff.symm.trans <| m.inductionOn fun _ => Std.DHashMap.Const.isEmpty_filterMap_iff

theorem mem_filterMap
    {f : α → β → Option γ} {k : α} :
    k ∈ m.filterMap f ↔ ∃ h, (f (m.getKey k h) (Const.get m k h)).isSome :=
  m.inductionOn fun _ => Std.DHashMap.Const.mem_filterMap

theorem size_filterMap_eq_size_iff
    {f : α → β → Option γ} :
    (m.filterMap f).size = m.size ↔ ∀ k h, (f (m.getKey k h) (Const.get m k h)).isSome :=
  m.inductionOn fun _ => Std.DHashMap.Const.size_filterMap_eq_size_iff
...
theorem get?_filterMap
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.get?_filterMap
...
theorem get?_filterMap_of_getKey?_eq_some
  ...
  m.inductionOn (fun _ h => Std.DHashMap.Const.get?_filterMap_of_getKey?_eq_some h) h
...
theorem isSome_apply_of_mem_filterMap
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.isSome_apply_of_mem_filterMap
    ...
    Const.get (m.filterMap f) k h =
      (f (m.getKey k (mem_of_mem_filterMap h))
        (Const.get m k (mem_of_mem_filterMap h))).get
          (isSome_apply_of_mem_filterMap h) :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.get_filterMap) h
...
theorem getD_filterMap
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getD_filterMap
...
theorem getD_filterMap_of_getKey?_eq_some
  ...
  m.inductionOn (fun _ h => Std.DHashMap.Const.getD_filterMap_of_getKey?_eq_some h) h
...
theorem getKey?_filterMap
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getKey?_filterMap
...
theorem getKeyD_filterMap
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getKeyD_filterMap
...
theorem getD_filter [LawfulBEq α]
  ...
  m.inductionOn fun _ => Std.DHashMap.getD_filter
...
theorem getKey?_filter [LawfulBEq α]
  ...
  m.inductionOn fun _ => Std.DHashMap.getKey?_filter

theorem getKey?_filter_key
    {f : α → Bool} {k : α} :
    (m.filter fun k _ => f k).getKey? k = (m.getKey? k).filter f :=
  m.inductionOn fun _ => Std.DHashMap.getKey?_filter_key

@[simp]
theorem getKey_filter
    {f : (a : α) → β a → Bool} {k : α} {h'} :
    (m.filter f).getKey k h' = m.getKey k (mem_of_mem_filter h') :=
  m.inductionOn (fun _ _ => Std.DHashMap.getKey_filter) h'
...
theorem filter_eq_empty_iff {f : α → β → Bool} :
  ...
  isEmpty_iff.symm.trans <| m.inductionOn fun _ => Std.DHashMap.Const.isEmpty_filter_iff

theorem mem_filter
    {f : α → β → Bool} {k : α} :
    k ∈ m.filter f ↔ ∃ (h' : k ∈ m),
      f (m.getKey k h') (Const.get m k h') :=
  m.inductionOn fun _ => Std.DHashMap.Const.mem_filter

theorem size_filter_le_size
    {f : α → β → Bool} :
    (m.filter f).size ≤ m.size :=
  m.inductionOn fun _ => Std.DHashMap.Const.size_filter_le_size

theorem size_filter_eq_size_iff
    {f : α → β → Bool} :
    (m.filter f).size = m.size ↔ ∀ (a : α) (h : a ∈ m),
      f (m.getKey a h) (Const.get m a h) :=
  m.inductionOn fun _ => Std.DHashMap.Const.size_filter_eq_size_iff

theorem filter_eq_self_iff {f : α → β → Bool} :
    m.filter f = m ↔ ∀ k h, f (m.getKey k h) (Const.get m k h) :=
  m.inductionOn fun _ => Iff.trans ⟨Quotient.exact, Quotient.sound⟩ Std.DHashMap.Const.filter_equiv_self_iff

theorem get?_filter
    {f : α → β → Bool} {k : α} :
    Const.get? (m.filter f) k = (Const.get? m k).pfilter (fun x h' =>
      f (m.getKey k (mem_iff_isSome_get?.mpr (Option.isSome_of_eq_some h'))) x) :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_filter

theorem get?_filter_of_getKey?_eq_some
    {f : α → β → Bool} {k k' : α} :
    m.getKey? k = some k' →
      Const.get? (m.filter f) k = (Const.get? m k).filter (fun x => f k' x) :=
  m.inductionOn fun _ => Std.DHashMap.Const.get?_filter_of_getKey?_eq_some

@[simp]
theorem get_filter
    {f : α → β → Bool} {k : α} {h'} :
    Const.get (m.filter f) k h' = Const.get m k (mem_of_mem_filter h') :=
  m.inductionOn (fun _ _ => Std.DHashMap.Const.get_filter) h'
...
theorem getD_filter
    ...
    Const.getD (m.filter f) k fallback = ((Const.get? m k).pfilter (fun x h' =>
      f (m.getKey k (mem_iff_isSome_get?.mpr (Option.isSome_of_eq_some h'))) x)).getD fallback :=
  m.inductionOn fun _ => Std.DHashMap.Const.getD_filter
...
theorem getD_filter_of_getKey?_eq_some
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getD_filter_of_getKey?_eq_some
...
theorem getKey?_filter
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getKey?_filter
...
theorem getKeyD_filter
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getKeyD_filter
...
theorem map_id_fun : m.map (fun _ v => v) = m :=
  m.inductionOn fun _ => Quotient.sound Std.DHashMap.map_id_equiv

@[simp]
theorem map_map {f : (a : α) → β a → γ a} {g : (a : α) → γ a → δ a} :
    (m.map f).map g = m.map fun k v => g k (f k v) :=
  m.inductionOn fun _ => Quotient.sound Std.DHashMap.map_map_equiv

theorem filterMap_eq_map {f : (a : α) → β a → γ a} :
    (m.filterMap (fun k v => some (f k v))) = m.map f :=
  m.inductionOn fun _ => Quotient.sound Std.DHashMap.filterMap_equiv_map

@[simp]
theorem map_eq_empty_iff {f : (a : α) → β a → γ a} :
    m.map f = ∅ ↔ m = ∅ := by
  simp only [← isEmpty_iff, Bool.coe_iff_coe]
  exact m.inductionOn fun _ => Std.DHashMap.isEmpty_map

theorem contains_map
    {f : (a : α) → β a → γ a} {k : α} :
    (m.map f).contains k = m.contains k :=
  m.inductionOn fun _ => Std.DHashMap.contains_map

theorem contains_of_contains_map
    {f : (a : α) → β a → γ a} {k : α} :
    (m.map f).contains k = true → m.contains k = true :=
  m.inductionOn fun _ => Std.DHashMap.contains_of_contains_map

@[simp]
theorem mem_map
    {f : (a : α) → β a → γ a} {k : α} :
    k ∈ m.map f ↔ k ∈ m :=
  m.inductionOn fun _ => Std.DHashMap.mem_map

theorem mem_of_mem_map
    {f : (a : α) → β a → γ a} {k : α} :
    k ∈ m.map f → k ∈ m :=
  m.inductionOn fun _ => Std.DHashMap.mem_of_mem_map

@[simp]
theorem size_map
    {f : (a : α) → β a → γ a} :
    (m.map f).size = m.size :=
  m.inductionOn fun _ => Std.DHashMap.size_map

@[simp]
theorem get?_map [LawfulBEq α]
    {f : (a : α) → β a → γ a} {k : α} :
    (m.map f).get? k = (m.get? k).map (f k) :=
  m.inductionOn fun _ => Std.DHashMap.get?_map

@[simp]
theorem get_map [LawfulBEq α]
    {f : (a : α) → β a → γ a} {k : α} {h'} :
    (m.map f).get k h' = f k (m.get k (mem_of_mem_map h')) :=
  m.inductionOn (fun _ _ => Std.DHashMap.get_map) h'
...
theorem getD_map [LawfulBEq α]
  ...
  m.inductionOn fun _ => Std.DHashMap.getD_map

@[simp]
theorem getKey?_map
    {f : (a : α) → β a → γ a} {k : α} :
    (m.map f).getKey? k = m.getKey? k :=
  m.inductionOn fun _ => Std.DHashMap.getKey?_map

@[simp]
theorem getKey_map
    {f : (a : α) → β a → γ a} {k : α} {h'} :
    (m.map f).getKey k h' = m.getKey k (mem_of_mem_map h') :=
  m.inductionOn (fun _ _ => Std.DHashMap.getKey_map) h'
...
theorem getKeyD_map
  ...
  m.inductionOn fun _ => Std.DHashMap.getKeyD_map
...
theorem get?_map
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.get?_map

theorem get?_map_of_getKey?_eq_some
    {f : α → β → γ} {k k' : α} (h : m.getKey? k = some k') :
    Const.get? (m.map f) k = (Const.get? m k).map (f k') :=
  m.inductionOn (fun _ h => Std.DHashMap.Const.get?_map_of_getKey?_eq_some h) h
...
theorem get_map
  ...
  m.inductionOn (fun _ _ => Std.DHashMap.Const.get_map) h'
...
theorem getD_map
  ...
  m.inductionOn fun _ => Std.DHashMap.Const.getD_map
...
theorem getD_map_of_getKey?_eq_some [Inhabited γ]
  ...
  m.inductionOn (fun _ h => Std.DHashMap.Const.getD_map_of_getKey?_eq_some h) h

end Const

end map

end Std.Extensional.DHashMap

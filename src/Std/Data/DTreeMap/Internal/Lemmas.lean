...
/--
Restatement of `isSome_minKey?_eq_not_isEmpty` such that exactly matches the signature needed in
the statement of `minKey?_insert_le_minKey?`.
-/
theorem isSome_minKey?_of_isEmpty_eq_false [TransOrd α] (h : t.WF) :
    t.isEmpty = false → t.minKey?.isSome := by
  simp_to_model using List.isSome_minKey?_of_isEmpty_eq_false
...
theorem minKey?_bind_getKey? [TransOrd α] (h : t.WF) :
  ...
  -- TODO: Should the following be the signature of this lemma instead?

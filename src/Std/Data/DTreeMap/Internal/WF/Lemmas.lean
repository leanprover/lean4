...
theorem entryAtIdx?_eq_getElem? {t : Impl α β} (htb : t.Balanced) {i : Nat} :
  ...
  | inner n k v l r ih ih' =>
    rw [entryAtIdx?]
    simp only [toListModel_inner, compare, compareOfLessAndEq]
    have lsize := size_eq_length l htb.left
    have rsize := size_eq_length r htb.right
    by_cases h : i < l.size
    · simp only [h, ↓reduceIte, List.getElem?_append_left, ← lsize]
      exact ih htb.left
    by_cases h' : i = l.size
    · simp only [h', Nat.lt_irrefl, ↓reduceIte, ← lsize, Nat.le_refl, List.getElem?_append_right,
        Nat.sub_self, List.getElem?_cons_zero]
    have h'' := Nat.lt_of_le_of_ne (Nat.le_of_not_lt h) (Ne.symm h')
    simp only [h, h', ↓reduceIte, List.getElem?_append_right, ← lsize,
...
/-!
### `getEntryLE?` / `getKeyLE?` (all 48 variants)
-/
...
theorem getEntryLE?_eq_getEntryGE?_reverse [o : Ord α] [TransOrd α] {t : Impl α β} {k : α} :
  ...
  rw [getEntryLE?, @getEntryGE?.eq_def]
  generalize none = best
  induction t generalizing best with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [reverse, getEntryLE?.go, @getEntryGE?.go.eq_2]
    dsimp [Ord.opposite]
    rw [OrientedOrd.eq_swap (a := k')]
    split
    · simp only [ih, ‹compare _ _ = _›, Ordering.swap_lt]
    · simp only [‹compare _ _ = _›, Ordering.swap_eq]
    · simp only [ih', ‹compare _ _ = _›, Ordering.swap_gt]
...
theorem getEntryLT?_eq_getEntryGT?_reverse [o : Ord α] [TransOrd α] {t : Impl α β} {k : α} :
  ...
  rw [getEntryLT?, @getEntryGT?.eq_def]
  generalize none = best
  induction t generalizing best with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [reverse, getEntryLT?.go, @getEntryGT?.go.eq_2]
    dsimp [Ord.opposite]
    rw [OrientedOrd.eq_swap (a := k')]
    split
    · simp only [ih', ‹compare _ _ = _›, Ordering.swap_gt]
    · rename_i h
      rw [← Ordering.swap_eq_lt] at h
      simp only [ih]
...
theorem getEntryGE?_eq_some_getEntryGE [Ord α] [TransOrd α]
    {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
    t.getEntryGE? k = some (t.getEntryGE k hto he) := by
  rw [getEntryGE?]
  induction t with
  | leaf => simp [mem_iff_contains, contains] at he
  | inner n k' v l r ih ih' =>
    rw [getEntryGE?.go, getEntryGE]
    split <;> split <;> rename_i h h'
    all_goals try (rw [h] at h'; contradiction)
    · rw [getEntryGE?.go_eq, Option.or_some', getEntryGED_eq_getD_getEntryGE?, getEntryGE?]
    · rfl
    · apply ih'

theorem getEntryGT?_eq_some_getEntryGT [Ord α] [TransOrd α]
    {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
    t.getEntryGT? k = some (t.getEntryGT k hto he) := by
  rw [getEntryGT?]
  induction t with
  | leaf => simp [mem_iff_contains, contains] at he
  | inner n k' v l r ih ih' =>
    rw [getEntryGT?.go, getEntryGT]
    split <;> rename_i h
    · rw [dif_pos ‹_›]
      rw [getEntryGT?.go_eq, Option.or_some', getEntryGTD_eq_getD_getEntryGT?, getEntryGT?]
    · rw [dif_neg ‹_›]
      apply ih'

theorem getEntryLE?_eq_some_getEntryLE [Ord α] [TransOrd α]
    {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
    t.getEntryLE? k = some (t.getEntryLE k hto he) := by
  rw [getEntryLE?_eq_getEntryGE?_reverse, @getEntryGE?.eq_def]
  induction t with
  | leaf => simp [mem_iff_contains, contains] at he
  | inner n k' v l r ih ih' =>
    rw [reverse, @getEntryGE?.go.eq_2, getEntryLE]
    simp only [Ord.opposite, OrientedCmp.eq_swap (a := k')]
    symm; split <;> (rename_i h; simp only [h, Ordering.swap])
    · symm; rw [@getEntryGE?.go_eq, Option.or_some', getEntryLED_eq_getD_getEntryLE?,
        getEntryLE?_eq_getEntryGE?_reverse, @getEntryGE?.eq_def]
    · symm; apply ih

theorem getEntryLT?_eq_some_getEntryLT [Ord α] [TransOrd α]
    {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
    t.getEntryLT? k = some (t.getEntryLT k hto he) := by
  rw [getEntryLT?_eq_getEntryGT?_reverse, @getEntryGT?.eq_def]
  induction t with
  | leaf => simp [mem_iff_contains, contains] at he
  | inner n k' v l r ih ih' =>
    rw [reverse, @getEntryGT?.go.eq_2, getEntryLT]
    simp only [OrientedCmp.gt_iff_lt, Ord.opposite]
    split <;> rename_i h
    · rw [dif_pos ‹_›]
      rw [@getEntryGT?.go_eq, Option.or_some', getEntryLTD_eq_getD_getEntryLT?,
        getEntryLT?_eq_getEntryGT?_reverse, @getEntryGT?.eq_def]
    · rw [dif_neg ‹_›]
      apply ih

theorem getKeyGE?_eq_getEntryGE? [Ord α] {t : Impl α β} {k : α} :
    getKeyGE? k t = (getEntryGE? k t).map (·.1) := by
  rw [getKeyGE?, getEntryGE?]
  change getKeyGE?.go k ((none : Option ((a : α) × β a)).map (·.1)) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getKeyGE?.go, getEntryGE?.go]
    split
    · exact ih (some ⟨k', v⟩)
    · rfl
    · exact ih' base
...
theorem getKeyGT?_eq_getEntryGT? [Ord α] {t : Impl α β} {k : α} :
  ...
  rw [getKeyGT?, getEntryGT?]
  change getKeyGT?.go k ((none : Option ((a : α) × β a)).map (·.1)) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getKeyGT?.go, getEntryGT?.go]
    split
    · exact ih (some ⟨k', v⟩)
    · exact ih' base
...
theorem getKeyLE?_eq_getEntryLE? [Ord α] {t : Impl α β} {k : α} :
  ...
  rw [getKeyLE?, getEntryLE?]
  change getKeyLE?.go k ((none : Option ((a : α) × β a)).map (·.1)) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getKeyLE?.go, getEntryLE?.go]
    split
    · exact ih base
    · rfl
    · exact ih' (some ⟨k', v⟩)
...
theorem getKeyLT?_eq_getEntryLT? [Ord α] {t : Impl α β} {k : α} :
  ...
  rw [getKeyLT?, getEntryLT?]
  change getKeyLT?.go k ((none : Option ((a : α) × β a)).map (·.1)) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getKeyLT?.go, getEntryLT?.go]
    split
    · exact ih' (some ⟨k', v⟩)
    · exact ih base
...
theorem getKeyGE_eq_getEntryGE [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getKeyGE, getEntryGE]
    split
    · rw [getKeyGED_eq_getD_getKeyGE?, getEntryGED_eq_getD_getEntryGE?, getKeyGE?_eq_getEntryGE?]
      exact Option.getD_map (·.1) ⟨k', v⟩ (getEntryGE? k l)
    · rfl
    · apply ih'

theorem getKeyGT_eq_getEntryGT [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getKeyGT, getEntryGT]
    split
    · rw [getKeyGTD_eq_getD_getKeyGT?, getEntryGTD_eq_getD_getEntryGT?, getKeyGT?_eq_getEntryGT?]
      exact Option.getD_map (·.1) ⟨k', v⟩ (getEntryGT? k l)
    · apply ih'

theorem getKeyLE_eq_getEntryLE [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getKeyLE, getEntryLE]
    split
    · rw [getKeyLED_eq_getD_getKeyLE?, getEntryLED_eq_getD_getEntryLE?, getKeyLE?_eq_getEntryLE?]
      exact Option.getD_map (·.1) ⟨k', v⟩ (getEntryLE? k r)
    · rfl
    · apply ih

theorem getKeyLT_eq_getEntryLT [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getKeyLT, getEntryLT]
    split
    · rw [getKeyLTD_eq_getD_getKeyLT?, getEntryLTD_eq_getD_getEntryLT?, getKeyLT?_eq_getEntryLT?]
      exact Option.getD_map (·.1) ⟨k', v⟩ (getEntryLT? k r)
    · apply ih
...
theorem getEntryGE?_eq_map [Ord α] {t : Impl α β} {k : α} :
    getEntryGE? k t = (Impl.getEntryGE? k t).map (fun x => (x.1, x.2)) := by
  rw [getEntryGE?, Impl.getEntryGE?]
  change getEntryGE?.go k ((none : Option ((_ : α) × β)).map (fun x => (x.1, x.2))) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getEntryGE?.go, Impl.getEntryGE?.go]
    split
    · exact ih (some ⟨k', v⟩)
    · rfl
    · exact ih' base

theorem getEntryGT?_eq_map [Ord α] {t : Impl α β} {k : α} :
    getEntryGT? k t = (Impl.getEntryGT? k t).map (fun x => (x.1, x.2)) := by
  rw [getEntryGT?, Impl.getEntryGT?]
  change getEntryGT?.go k ((none : Option ((_ : α) × β)).map (fun x => (x.1, x.2))) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getEntryGT?.go, Impl.getEntryGT?.go]
    split
    · exact ih (some ⟨k', v⟩)
    · exact ih' base

theorem getEntryLE?_eq_map [Ord α] {t : Impl α β} {k : α} :
    getEntryLE? k t = (Impl.getEntryLE? k t).map (fun x => (x.1, x.2)) := by
  rw [getEntryLE?, Impl.getEntryLE?]
  change getEntryLE?.go k ((none : Option ((_ : α) × β)).map (fun x => (x.1, x.2))) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getEntryLE?.go, Impl.getEntryLE?.go]
    split
    · exact ih base
    · rfl
    · exact ih' (some ⟨k', v⟩)

theorem getEntryLT?_eq_map [Ord α] {t : Impl α β} {k : α} :
    getEntryLT? k t = (Impl.getEntryLT? k t).map (fun x => (x.1, x.2)) := by
  rw [getEntryLT?, Impl.getEntryLT?]
  change getEntryLT?.go k ((none : Option ((_ : α) × β)).map (fun x => (x.1, x.2))) _ = _
  generalize none = base
  induction t generalizing base with
  | leaf => rfl
  | inner n k' v l r ih ih' =>
    rw [getEntryLT?.go, Impl.getEntryLT?.go]
    split
    · exact ih' (some ⟨k', v⟩)
    · exact ih base

theorem getEntryGE!_eq_get!_getEntryGE? [Ord α] {t : Impl α β} {k : α} [Inhabited (α × β)] :
    getEntryGE! k t = (getEntryGE? k t).get! := rfl

theorem getEntryGT!_eq_get!_getEntryGT? [Ord α] {t : Impl α β} {k : α} [Inhabited (α × β)] :
    getEntryGT! k t = (getEntryGT? k t).get! := rfl

theorem getEntryLE!_eq_get!_getEntryLE? [Ord α] {t : Impl α β} {k : α} [Inhabited (α × β)] :
    getEntryLE! k t = (getEntryLE? k t).get! := rfl

theorem getEntryLT!_eq_get!_getEntryLT? [Ord α] {t : Impl α β} {k : α} [Inhabited (α × β)] :
    getEntryLT! k t = (getEntryLT? k t).get! := rfl

theorem getEntryGED_eq_getD_getEntryGE? [Ord α] {t : Impl α β} {k : α} {fallback : α × β} :
    getEntryGED k t fallback = (getEntryGE? k t).getD fallback := rfl

theorem getEntryGTD_eq_getD_getEntryGT? [Ord α] {t : Impl α β} {k : α} {fallback : α × β} :
    getEntryGTD k t fallback = (getEntryGT? k t).getD fallback := rfl

theorem getEntryLED_eq_getD_getEntryLE? [Ord α] {t : Impl α β} {k : α} {fallback : α × β} :
    getEntryLED k t fallback = (getEntryLE? k t).getD fallback := rfl

theorem getEntryLTD_eq_getD_getEntryLT? [Ord α] {t : Impl α β} {k : α} {fallback : α × β} :
    getEntryLTD k t fallback = (getEntryLT? k t).getD fallback := rfl

theorem getEntryGE_eq [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getEntryGE, Impl.getEntryGE]
    split
    · rw [getEntryGED_eq_getD_getEntryGE?, Impl.getEntryGED_eq_getD_getEntryGE?, getEntryGE?_eq_map]
      exact Option.getD_map (fun x => (x.1, x.2)) ⟨k', v⟩ (Impl.getEntryGE? k l)
    · rfl
    · exact ih' hto.right _

theorem getEntryGT_eq [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getEntryGT, Impl.getEntryGT]
    split
    · rw [getEntryGTD_eq_getD_getEntryGT?, Impl.getEntryGTD_eq_getD_getEntryGT?, getEntryGT?_eq_map]
      exact Option.getD_map (fun x => (x.1, x.2)) ⟨k', v⟩ (Impl.getEntryGT? k l)
    · exact ih' hto.right _

theorem getEntryLE_eq [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getEntryLE, Impl.getEntryLE]
    split
    · rw [getEntryLED_eq_getD_getEntryLE?, Impl.getEntryLED_eq_getD_getEntryLE?, getEntryLE?_eq_map]
      exact Option.getD_map (fun x => (x.1, x.2)) ⟨k', v⟩ (Impl.getEntryLE? k r)
    · rfl
    · exact ih hto.left _

theorem getEntryLT_eq [Ord α] [TransOrd α] {t : Impl α β} (hto : t.Ordered) {k : α} (he) :
  ...
  | inner n k' v l r ih ih' =>
    rw [getEntryLT, Impl.getEntryLT]
    split
    · rw [getEntryLTD_eq_getD_getEntryLT?, Impl.getEntryLTD_eq_getD_getEntryLT?, getEntryLT?_eq_map]
      exact Option.getD_map (fun x => (x.1, x.2)) ⟨k', v⟩ (Impl.getEntryLT? k r)
    · exact ih hto.left _

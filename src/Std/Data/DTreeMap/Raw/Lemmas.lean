...
@[simp]
theorem min?_emptyc :
    (∅ : Raw α β cmp).min? = none :=
  Impl.min?_empty

theorem min?_of_isEmpty [TransCmp cmp] (h : t.WF) (he : t.isEmpty) :
    t.min? = none :=
  sorry

@[simp]
theorem min?_eq_none_iff_isEmpty [TransCmp cmp] (h : t.WF) :
    t.min? = none ↔ t.isEmpty :=
  sorry

@[simp]
theorem min?_isSome_iff_isEmpty_eq_false [TransCmp cmp] (h : t.WF) :
    t.min?.isSome ↔ t.isEmpty = false :=
  sorry

theorem min?_insert [TransCmp cmp] (h : t.WF) {k v} :
    (t.insert k v).min? =
      some (match t.min? with
       | none => ⟨k, v⟩
       | some e => if cmp k e.1 |>.isLE then ⟨k, v⟩ else e) :=
  sorry

theorem isSome_min?_insert [TransCmp cmp] (h : t.WF) {k v} :
    (t.insert k v).min?.isSome :=
  sorry

theorem get_min?_insert_le_get_min? [TransCmp cmp] (h : t.WF) {k v} (he : t.isEmpty = false) :
    cmp ((t.insert k v).min?.get (isSome_min?_insert h)).1
      (t.min?.get ((min?_isSome_iff_isEmpty_eq_false h).mpr he)).1 |>.isLE :=
  sorry

theorem get_min?_insert_le_self [TransCmp cmp] (h : t.WF) {k v} :
    cmp ((t.insert k v).min?.get (isSome_min?_insert h)).1 k |>.isLE :=
  sorry

theorem contains_min?_fst [TransCmp cmp] (h : t.WF) :
    t.min?.all fun e => t.contains e.1 :=
  sorry

theorem isSome_min?_of_contains [TransCmp cmp] (h : t.WF) {k} (hc : t.contains k) :
    t.min?.isSome :=
  sorry

theorem isSome_min?_of_mem [TransCmp cmp] (h : t.WF) {k} (hc : k ∈ t) :
    t.min?.isSome :=
  sorry

theorem min?_fst_le_of_contains [TransCmp cmp] (h : t.WF) {k} (hc : t.contains k) :
    t.min?.all fun e => (cmp e.1 k).isLE :=
  sorry

theorem min?_fst_le_of_mem [TransCmp cmp] (h : t.WF) {k} (hc : k ∈ t) :
    t.min?.all fun e => (cmp e.1 k).isLE :=
  sorry

theorem min?_get_fst_le_of_contains [TransCmp cmp] (h : t.WF) {k} (hc : t.contains k) :
    cmp (t.min?.get (isSome_min?_of_contains h hc)).1 k |>.isLE :=
  sorry

theorem min?_get_fst_le_of_mem [TransCmp cmp] (h : t.WF) {k} (hc : k ∈ t) :
    cmp (t.min?.get (isSome_min?_of_contains h hc)).1 k |>.isLE :=
  sorry

@[simp]
theorem min?_fst_bind_getKey? [TransCmp cmp] (h : t.WF) :
    (t.min?.bind fun e => t.getKey? e.1) = t.min?.map fun e => e.1 :=
  sorry

theorem min?_erase_of_isNe_compare [TransCmp cmp] (h : t.WF) {k}
    (hc : t.min?.all fun e => (cmp e.1 k).isNe) :
    (t.erase k).min? = t.min? :=
  sorry
...
theorem min?_erase_fst_le [TransCmp cmp] (h : t.WF) {k} :
    (t.erase k).min?.attach.all fun ⟨e, he⟩ => cmp e.1
        (t.min?.get <| isSome_min?_of_isSome_min?_erase h <| he ▸ Option.isSome_some).1 |>.isLE :=
  sorry

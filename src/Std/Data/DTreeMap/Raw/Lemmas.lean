...
theorem union_inter_equiv_empty [TransCmp cmp] {t₃ : Raw α β cmp}
  ...
  have hu : (t₁ ∪ t₂).WF := ⟨Impl.WF.union! h₁.out h₂.out⟩
  change ((t₁ ∪ t₂) ∩ t₃) ~m empty ↔ (t₁ ∩ t₃) ~m empty ∧ (t₂ ∩ t₃) ~m empty
  simp only [equiv_empty_iff_isEmpty, isEmpty_inter_iff hu h₃, isEmpty_inter_iff h₁ h₃,
    isEmpty_inter_iff h₂ h₃, mem_union_iff h₁ h₂, or_imp, forall_and]
...
theorem inter_union_equiv_empty [TransCmp cmp] {t₃ : Raw α β cmp}
  ...
  have hu : (t₂ ∪ t₃).WF := ⟨Impl.WF.union! h₂.out h₃.out⟩
  change (t₁ ∩ (t₂ ∪ t₃)) ~m empty ↔ (t₁ ∩ t₂) ~m empty ∧ (t₁ ∩ t₃) ~m empty
  simp only [equiv_empty_iff_isEmpty, isEmpty_inter_iff h₁ hu, isEmpty_inter_iff h₁ h₂,
    isEmpty_inter_iff h₁ h₃, mem_union_iff h₂ h₃, not_or, imp_and, forall_and]

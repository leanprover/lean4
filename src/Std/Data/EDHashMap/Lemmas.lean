...
-/
prelude
import Std.Data.EDHashMap.Basic

/-!
# Extensional dependent hash map lemmas

This file contains lemmas about `Std.EDHashMap`.
-/
...
namespace Std.EDHashMap

variable {m : EDHashMap α β}
...
@[simp]
theorem emptyWithCapacity_eq {c} : (emptyWithCapacity c : EDHashMap α β) = ∅ :=
...
@[simp]
theorem size_empty : (∅ : EDHashMap α β).size = 0 := rfl
...
@[simp]
theorem erase_empty {k : α} : (∅ : EDHashMap α β).erase k = ∅ :=
...
@[simp]
theorem get?_empty [LawfulBEq α] {a : α} : (∅ : EDHashMap α β).get? a = none :=
  DHashMap.get?_empty
...
@[simp]
theorem get?_empty {a : α} : get? (∅ : EDHashMap α (fun _ => β)) a = none :=
...
@[simp]
theorem get!_empty [Inhabited β] {a : α} : get! (∅ : EDHashMap α (fun _ => β)) a = default :=
  DHashMap.Const.get!_empty
...
theorem getD_empty [LawfulBEq α] {a : α} {fallback : β a} :
    (∅ : EDHashMap α β).getD a fallback = fallback :=
  DHashMap.getD_empty
...
theorem getD_empty {a : α} {fallback : β} :
    getD (∅ : EDHashMap α (fun _ => β)) a fallback = fallback :=
...
@[simp]
theorem getKey?_empty {a : α} : (∅ : EDHashMap α β).getKey? a = none :=
...
theorem getKey!_empty [Inhabited α] {a : α} :
    (∅ : EDHashMap α β).getKey! a = default :=
  DHashMap.getKey!_empty
...
theorem getKeyD_empty {a fallback : α} :
    (∅ : EDHashMap α β).getKeyD a fallback = fallback :=
...
private theorem insertMany_list_mk {m : DHashMap α β} {l : List ((a : α) × β a)} :
    (EDHashMap.insertMany (Quotient.mk _ m) l : EDHashMap α β) = Quotient.mk _ (m.insertMany l) := by
...
private theorem insertMany_list_mk {m : DHashMap α fun _ => β} {l : List (α × β)} :
    (insertMany (Quotient.mk _ m) l : EDHashMap α fun _ => β) =
...
private theorem insertManyIfNewUnit_list_mk {m : DHashMap α fun _ => Unit} {l : List α} :
    (insertManyIfNewUnit (Quotient.mk _ m) l : EDHashMap α fun _ => Unit) =
...
theorem ofList_singleton {k : α} {v : β k} :
    ofList [⟨k, v⟩] = (∅ : EDHashMap α β).insert k v := rfl

theorem ofList_cons {k : α} {v : β k} {tl : List ((a : α) × β a)} :
    ofList (⟨k, v⟩ :: tl) = ((∅ : EDHashMap α β).insert k v).insertMany tl :=
  insertMany_cons

private theorem ofList_eq_mk {l : List ((a : α) × β a)} :
    (ofList l : EDHashMap α β) = Quotient.mk _ (DHashMap.ofList l) := by
...
theorem ofList_singleton {k : α} {v : β} :
    ofList [⟨k, v⟩] = (∅ : EDHashMap α (fun _ => β)).insert k v :=
  rfl

theorem ofList_cons {k : α} {v : β} {tl : List (α × β)} :
    ofList (⟨k, v⟩ :: tl) = insertMany ((∅ : EDHashMap α (fun _ => β)).insert k v) tl :=
  insertMany_cons

private theorem ofList_eq_mk {l : List (α × β)} :
    (ofList l : EDHashMap α fun _ => β) = Quotient.mk _ (DHashMap.Const.ofList l) := by
  change insertMany (Quotient.mk _ _) _ = _
  rw [insertMany_list_mk]
  rfl
...
theorem unitOfList_singleton {k : α} :
    unitOfList [k] = (∅ : EDHashMap α (fun _ => Unit)).insertIfNew k () :=
  rfl

theorem unitOfList_cons {hd : α} {tl : List α} :
    unitOfList (hd :: tl) =
      insertManyIfNewUnit ((∅ : EDHashMap α (fun _ => Unit)).insertIfNew hd ()) tl :=
  insertManyIfNewUnit_cons

private theorem unitOfList_eq_mk {l : List α} :
    (unitOfList l : EDHashMap α fun _ => Unit) = Quotient.mk _ (DHashMap.Const.unitOfList l) := by
...
variable {m₁ m₂ m₃ : Std.EDHashMap α β}
...
variable {β : Type v} {m₁ m₂ : EDHashMap α fun _ => β}
...
variable {β : Type v} {γ : Type w} {m : EDHashMap α (fun _ => β)}

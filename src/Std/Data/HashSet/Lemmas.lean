...
theorem union_inter_equiv_empty [EquivBEq α] [LawfulHashable α]
  ...
  simp only [equiv_empty_iff_isEmpty, isEmpty_inter_iff, mem_iff_contains, mem_union_iff,
    or_imp, forall_and]

theorem inter_union_equiv_empty [EquivBEq α] [LawfulHashable α]
    {m₁ m₂ m₃ : HashSet α} :
    (m₁ ∩ (m₂ ∪ m₃)) ~m ∅ ↔ (m₁ ∩ m₂) ~m ∅ ∧ (m₁ ∩ m₃) ~m ∅ := by
  simp only [equiv_empty_iff_isEmpty, isEmpty_inter_iff, mem_iff_contains, mem_union_iff, not_or,
    imp_and, forall_and]

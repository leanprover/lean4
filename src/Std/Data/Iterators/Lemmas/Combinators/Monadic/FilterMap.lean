...
theorem HItEquivM.InternalCombinators.filterMap [Monad m] [LawfulMonad m]
  ...
  case implies =>
    ...
    simp only [BundledIterM.step', BundledIterM.ofIterM, IterM.step', IterM.step_filterMapWithProof]
    ...
    · simp [HetT.bind, Pure.pure, HetT.pure]
      ...
      · rintro ⟨step', hp, rfl⟩
        ...
        case yieldNone it' out h' h'' =>
          simp [IterM.filterMapWithProof, IterM.InternalCombinators.filterMap] at h'
          ...
          rename_i itb outb
          cases hsb.2
          refine ⟨.skip (itb.filterMapWithProof f), .yieldNone hpb h'', ?_⟩
          simp
          apply Eq.symm
          apply Quot.sound

...
theorem get!_eq [TransCmp cmp] [Inhabited β] {k : α} (h : t₁ ~m t₂) :
    t₁.get! k = t₂.get! k :=
  h.1.constGet!_eq
...
theorem forIn_eq [TransCmp cmp] [Monad m] [LawfulMonad m] {f : α → β → δ → m (ForInStep δ)}
    {init : δ} (h : t₁ ~m t₂) :
...
theorem Const.equiv_iff_keys_unit_perm {t₁ t₂ : TreeMap α Unit cmp} :

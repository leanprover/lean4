...
theorem Except.of_wp_eq {ε α : Type u} {x prog : Except ε α} (h : prog = x) (P : Except ε α → Prop) :
  ...
  -- transparency).
  try simp only [wp, instWP._aux_1, ExceptT.run, Id.run, PredTrans.apply_pushExcept,
    PredTrans.apply_Pure_pure] at hspec
  try simp only [wp, ExceptT.run, Id.run, PredTrans.apply_pushExcept,
    PredTrans.apply_Pure_pure] at hspec
...
theorem Option.of_wp_eq {α : Type u} {x prog : Option α} (h : prog = x) (P : Option α → Prop) :
  ...
  try simp only [wp, instWP._aux_1, OptionT.run, Id.run, PredTrans.apply_pushOption,
    PredTrans.apply_Pure_pure] at hspec
  try simp only [wp, OptionT.run, Id.run, PredTrans.apply_pushOption,
    PredTrans.apply_Pure_pure] at hspec
  split at hspec <;> exact hspec True.intro

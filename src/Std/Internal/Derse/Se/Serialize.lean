...
instance {α β : Type v} [Serialize α] [Serialize β] [BEq α] [Hashable α] : Serialize (HashMap α β) where
  serialize xs := Serializer.serializeMap xs.iter

instance {α β : Type v} {cmp : α → α → Ordering} [Serialize α] [Serialize β] : Serialize (TreeMap α β cmp) where
  serialize xs := Serializer.serializeMap xs.iter

instance {α : Type v} {β : α → Type v} [Serialize α] [∀ k, Serialize (β k)] [BEq α] [Hashable α] : Serialize (DHashMap α β) where
  serialize xs := Serializer.serializeDMap xs.iter

instance {α : Type v} {β : α → Type v} {cmp : α → α → Ordering} [Serialize α] [∀ k, Serialize (β k)] : Serialize (DTreeMap α β cmp) where
  serialize xs := Serializer.serializeDMap xs.iter

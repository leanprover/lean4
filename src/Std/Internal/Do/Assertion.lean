...
`α`: an outParam function type `Fun` and an `evalsTo` embedding of the function graph into the
lattice, with a covering law that packs an arbitrary assertion under the graph join.
...
The `Prop` and `Pi` instances are `@[reducible]`, so `evalsTo` unfolds definitionally for any
finite nesting of state layers.
-/

/--
`Pred` interprets total nondeterministic functions into `α`. The function type `Fun` is an
`outParam` synthesized from `Pred` and `α`.
-/
class NondetFun (Pred : Type u) (α : Type v) (Fun : outParam (Type u)) [Assertion Pred] where
...
@[reducible] instance : NondetFun Prop α α where
  evalsTo f a := ⌜f = a⌝
  total f P :=
    le_iSup_of_le f <| by
      rw [ofProp_eq_top rfl, top_meet]
      exact PartialOrder.rel_refl

/-- State-dependent nondeterministic functions: a function for `σ → Pred` is a `σ`-indexed
function for `Pred`. -/
@[reducible] instance {σ : Type u} {Pred : Type u} {α : Type v} {Fun : Type u}
    [Assertion Pred] [inst : NondetFun Pred α Fun] :
    NondetFun (σ → Pred) α (σ → Fun) where
...
theorem NondetFun.le_of_total_le {Pred : Type u} {α : Type v} {Fun : Type u}
    [Assertion Pred] [inst : NondetFun Pred α Fun] (f : Fun) {P Q : Pred}

...
- If it is initial or finished this is a no-op.
- If it is running the execution of the timer is stopped and it is put into the stopped state.
  state. Note that if the last `IO.Promise` generated by `next` is unresolved and being waited

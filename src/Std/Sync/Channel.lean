...
instance [Inhabited α] : Std.IO.Stream IO (CloseableChannel α) (Option α) where
  next channel := channel.recvSelector

instance [Inhabited α] : Std.IO.Sink Async (CloseableChannel α) α where
  write receiver x := do
...
instance [Inhabited α] : Std.IO.Stream IO (Channel α) α where
  next channel := channel.recvSelector

instance [Inhabited α] : Std.IO.Sink Async (Channel α) α where
  write receiver x := do
    let task ← receiver.send x
    Async.ofTask task

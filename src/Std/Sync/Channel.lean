...
Additionally `Std.ClosableChannel` is provided in case closing the channel is of interest.
...
-/
def ClosableChannel (α : Type) : Type := ClosableChannel.Flavors α
...
Additionally `Std.ClosableChannel.Sync` can be closed if necessary, unlike `Std.Channel.Sync`.
This introduces the need to handle errors in some cases, thus it is usually easier to use
`Std.Channel` if applicable.
-/
def ClosableChannel.Sync (α : Type) : Type := ClosableChannel α

instance : Nonempty (ClosableChannel α) :=
  inferInstanceAs (Nonempty (ClosableChannel.Flavors α))

instance : Nonempty (ClosableChannel.Sync α) :=
  inferInstanceAs (Nonempty (ClosableChannel α))
...
- `capacity` is `some n` with `n > 0` it will use a buffer of size `n` and begin blocking once it
  is filled
-/
def new (capacity : Option Nat := none) : BaseIO (ClosableChannel α) := do
  match capacity with
  | none => return .unbounded (← ClosableChannel.Unbounded.new)
  | some 0 => return .zero (← ClosableChannel.Zero.new)
  | some (n + 1) => return .bounded (← ClosableChannel.Bounded.new (n + 1) (by omega))
...
-/
def trySend (ch : ClosableChannel α) (v : α) : BaseIO Bool :=
  match ch with
  | .unbounded ch => ClosableChannel.Unbounded.trySend ch v
  | .zero ch => ClosableChannel.Zero.trySend ch v
  | .bounded ch => ClosableChannel.Bounded.trySend ch v
...
could be completed.
-/
def send (ch : ClosableChannel α) (v : α) : BaseIO (Task (Except Error Unit)) :=
  match ch with
  | .unbounded ch => ClosableChannel.Unbounded.send ch v
  | .zero ch => ClosableChannel.Zero.send ch v
  | .bounded ch => ClosableChannel.Bounded.send ch v
...
- if there are already values waiting to be received they can still be received by subsequent `recv`
  calls
-/
def close (ch : ClosableChannel α) : EIO Error Unit :=
  match ch with
  | .unbounded ch => ClosableChannel.Unbounded.close ch
  | .zero ch => ClosableChannel.Zero.close ch
  | .bounded ch => ClosableChannel.Bounded.close ch

/--
Return `true` if the channel is closed.
-/
def isClosed (ch : ClosableChannel α) : BaseIO Bool :=
  match ch with
  | .unbounded ch => ClosableChannel.Unbounded.isClosed ch
  | .zero ch => ClosableChannel.Zero.isClosed ch
  | .bounded ch => ClosableChannel.Bounded.isClosed ch
...
-/
def tryRecv (ch : ClosableChannel α) : BaseIO (Option α) :=
  match ch with
  | .unbounded ch => ClosableChannel.Unbounded.tryRecv ch
  | .zero ch => ClosableChannel.Zero.tryRecv ch
  | .bounded ch => ClosableChannel.Bounded.tryRecv ch
...
completed.
-/
def recv (ch : ClosableChannel α) : BaseIO (Task (Option α)) :=
  match ch with
  | .unbounded ch => ClosableChannel.Unbounded.recv ch
  | .zero ch => ClosableChannel.Zero.recv ch
  | .bounded ch => ClosableChannel.Bounded.recv ch
...
-/
partial def forAsync (f : α → BaseIO Unit) (ch : ClosableChannel α)
...
-/
@[inline]
def sync (ch : ClosableChannel α) : ClosableChannel.Sync α := ch

namespace Sync

@[inherit_doc ClosableChannel.new, inline]
def new (capacity : Option Nat := none) : BaseIO (Sync α) := ClosableChannel.new capacity

@[inherit_doc ClosableChannel.trySend, inline]
def trySend (ch : Sync α) (v : α) : BaseIO Bool := ClosableChannel.trySend ch v
...
def send (ch : Sync α) (v : α) : EIO Error Unit := do
  EIO.ofExcept (← IO.wait (← ClosableChannel.send ch v))

@[inherit_doc ClosableChannel.close, inline]
def close (ch : Sync α) : EIO Error Unit := ClosableChannel.close ch

@[inherit_doc ClosableChannel.isClosed, inline]
def isClosed (ch : Sync α) : BaseIO Bool := ClosableChannel.isClosed ch

@[inherit_doc ClosableChannel.tryRecv, inline]
def tryRecv (ch : Sync α) : BaseIO (Option α) := ClosableChannel.tryRecv ch
...
def recv (ch : Sync α) : BaseIO (Option α) := do
  IO.wait (← ClosableChannel.recv ch)
...
structure Channel (α : Type) where
  private mk ::
    private inner : ClosableChannel α
deriving Nonempty
...
`Std.ClosableChannel.Sync` instead. Note that `Std.ClosableChannel` introduces a need for error
handling in some cases, thus `Std.ClosableChannel.Sync` is usually easier to use if applicable.
-/
...
def new (capacity : Option Nat := none) : BaseIO (Channel α) := do
  return ⟨← ClosableChannel.new capacity⟩

@[inherit_doc ClosableChannel.trySend, inline]
def trySend (ch : Channel α) (v : α) : BaseIO Bool :=
  ClosableChannel.trySend ch.inner v
...
def send (ch : Channel α) (v : α) : BaseIO (Task Unit) := do
  BaseIO.bindTask (sync := true) (← ClosableChannel.send ch.inner v)
    fun
      | .ok .. => return .pure ()
      | .error .. => unreachable!
...
def tryRecv (ch : Channel α) : BaseIO (Option α) :=
  ClosableChannel.tryRecv ch.inner

@[inherit_doc ClosableChannel.recv]
def recv [Inhabited α] (ch : Channel α) : BaseIO (Task α) := do
  BaseIO.bindTask (sync := true) (← ClosableChannel.recv ch.inner)
    fun
      | some val => return .pure val
      | none => unreachable!

@[inherit_doc ClosableChannel.forAsync]
...
@[inherit_doc ClosableChannel.sync, inline]

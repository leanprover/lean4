...
instance : BEq (BVExpr n) := ⟨go⟩
where
  go {n} : BVExpr n → BVExpr n → Bool := fun a b =>
    match ha : n, a with
    | _, .var idxA => match b with
      | .var idxB => idxA == idxB
      | _ => false
    | _, .const valA => match b with
      | .const valB => valA == valB
      | _ => false
    | _, @extract wA startA lenA exprA =>
      match b with
      | @extract wB startB lenB exprB =>
        if h : startA = startB ∧ lenA = lenB ∧ wA = wB then
          go exprA (h.right.right ▸ exprB)
        else false
      | _ => false
    | _, .bin lhsA opA rhsA =>
      match b with
      | .bin lhsB opB rhsB =>
        (go lhsA lhsB) && (opA == opB) && (go rhsA rhsB)
      | _ => false
    | _, .un opA operandA =>
      match b with
      | .un opB operandB => (opA == opB) && (go operandA operandB)
      | _ => false
    |_,@append la ra lhsa rhsa =>
      match n, ha ▸ b with
      | _, @append lb rb lhsb rhsb =>
        if h : la = lb ∧ ra = rb then
            (go lhsa (h.left ▸ lhsb)) && (go rhsa (h.right ▸ rhsb))
        else
          false
      | _, _ => false
    | _, @replicate wA nA exprA =>
      match n, ha ▸ b with
      | _, @replicate wB nB exprB =>
        if h : nA = nB ∧ wA = wB then
          go exprA (h.right ▸ exprB)
        else
          false
      | _, _ => false
    | _, @shiftLeft mA nA lhsA rhsA =>
      match n, ha ▸ b with
      | _, @shiftLeft mB nB lhsB rhsB =>
        if h : mA = mB ∧ nA = nB then
          (go lhsA (h.left ▸ lhsB)) && (go rhsA (h.right ▸ rhsB))
        else
          false
      | _, _ => false
    | _, @shiftRight mA nA lhsA rhsA =>
      match n, ha ▸ b with
      | _, @shiftRight mB nB lhsB rhsB =>
        if h : mA = mB ∧ nA = nB then
          (go lhsA (h.left ▸ lhsB)) && (go rhsA (h.right ▸ rhsB))
        else
          false
      | _, _ => false
    | _, @arithShiftRight mA nA lhsA rhsA =>
      match n, ha ▸ b with
      | _, @arithShiftRight mB nB lhsB rhsB =>
        if h : mA = mB ∧ nA = nB then
          (go lhsA (h.left ▸ lhsB)) && (go rhsA (h.right ▸ rhsB))
        else
          false

...
theorem propagateHints_spec (s : State) (assign : Assignment) (hints : Array Nat) :
  ...
  · next pref1 cur1 suff1 hfor1 b1 curAssign hintClause hclause hout pref cur suff hfor b st
      assignNow unit value hget hval u hunit hueq ih =>
    simp only [unit, st, assignNow] at hunit hget ⊢
    simp only [beq_iff_eq] at hval hueq
    subst hueq
    ...
    replace ih := ih.right
    simp only [hunit] at ih
    rcases ih with ⟨pol, hnone, hins, hall⟩ | hsame
    · have hpol : pol = cur.snd := by
        rw [hins, Assignment.get?_insert_of_eq rfl] at hget
        simp only [Option.some.injEq] at hget
        rw [hget, hval]
      subst hpol
      exact Or.inl ⟨cur.snd, hnone, hins, hall, Or.inl (by simp)⟩
    · exact Or.inr hsame
  · simp
  · next pref1 cur1 suff1 hfor1 b1 curAssign hintClause hclause hout pref cur suff hfor b st
      assignNow unit value hget hval hunit ih =>
    simp only [unit, st, assignNow] at hunit ⊢
    simp only [true_and, reduceCtorEq, false_and, and_false, exists_false, or_false] at ih ⊢
    replace ih := ih.right
    simp only [hunit] at ih
    exact Or.inr ih.left
  · simp
  · next pref1 cur1 suff1 hfor1 b1 curAssign hintClause hclause hout pref cur suff hfor b st
      assignNow unit value hget hval isUnit hisU ih =>
    ...
    · next u heq =>
      ...
      · rw [hins, Assignment.get?_insert_of_ne hne] at hget
        exact Or.inl ⟨pol, hnone, hins, hall, Or.inr hget⟩
      · exact Or.inr hsame
    · next heq =>
      simp only [heq] at ih ⊢
  ...
  · next pref1 cur1 suff1 hfor1 b1 curAssign hintClause hclause hout pref cur suff hfor b st
    ...
    obtain ⟨hsame, hall⟩ := ih
    rw [hsame]
    refine Or.inl ⟨cur.snd, ?_, rfl, fun lit hlit => Or.inr (hall lit hlit), Or.inl (by simp)⟩
    rw [← hsame]
    exact hget
  · simp
  · simp

...
macro (name := mleave) "mleave" : tactic =>
  `(tactic| (try simp only [
              ...
              $(mkIdent ``List.Cursor.prefix_begin):term,
              $(mkIdent ``List.Cursor.suffix_begin):term,
              $(mkIdent ``List.Cursor.prefix_end):term,
              $(mkIdent ``List.Cursor.suffix_end):term,
              $(mkIdent ``List.Cursor.cur_begin):term,

...
/--
{name}`Ordinal` represents a bounded value for weeks, which ranges between 1 and 53.
-/
...
/--
{name}`Offset` represents an offset in weeks.
-/
...
/--
{name}`OfMonth` represents the number of weeks within a month. It ensures that the week is within
the correct bounds—either 1 to 6, representing the possible weeks in a month.
-/
...
/--
Converts {name}`Week.Offset` into {name}`Millisecond.Offset`.
-/
@[inline]
...
/--
Converts {name}`Millisecond.Offset` into {name}`Week.Offset`.
-/
@[inline]
...
/--
Converts {name}`Week.Offset` into {name}`Nanosecond.Offset`.
-/
@[inline]
...
/--
Converts {name}`Nanosecond.Offset` into {name}`Week.Offset`.
-/
@[inline]
...
/--
Converts {name}`Week.Offset` into {name}`Second.Offset`.
-/
@[inline]
...
/--
Converts {name}`Second.Offset` into {name}`Week.Offset`.
-/
@[inline]
...
/--
Converts {name}`Week.Offset` into {name}`Minute.Offset`.
-/
@[inline]
...
/--
Converts {name}`Minute.Offset` into {name}`Week.Offset`.
-/
@[inline]
...
/--
Converts {name}`Week.Offset` into {name}`Hour.Offset`.
-/
@[inline]
...
/--
Converts {name}`Hour.Offset` into {name}`Week.Offset`.
-/
@[inline]
...
/--
Converts {name}`Week.Offset` into {name}`Day.Offset`.
-/
@[inline]
...
/--
Converts {name}`Day.Offset` into {name}`Week.Offset`.
-/
@[inline]

...
/--
{name}`Ordinal` represents a bounded value for hours, ranging from 0 to 23.
-/
...
/--
{lean}`Offset` represents an offset in hours, defined as an {lean}`Int`. This can be used to express durations
...
/--
Converts an {name}`Ordinal` into a relative hour in the range of 1 to 12.
-/
...
/--
Creates an {name}`Ordinal` from a natural number, ensuring the value is within the valid bounds for hours.
-/
@[inline]
...
/--
Creates an {name}`Ordinal` from a {name}`Fin` value.
-/
@[inline]
...
/--
Converts an {name}`Ordinal` to an {name}`Offset`, which represents the duration in hours as an
integer value.
-/
@[inline]

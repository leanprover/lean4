...
`EPostSlot ε Pred EPosts` locates the slot for exceptions of type `ε` inside exception
...
variable {ε : Type u} {ε' : Type u'} {Pred : Type v} {Pred' : Type v'} {EPosts : Type w}

/-- Locates the slot for exceptions of type `ε` inside exception postconditions of type
`EPosts`. The slot holds assertions of type `ε → Pred`. -/
class EPostSlot (ε : Type u) (Pred : outParam (Type v)) (EPosts : Type w) where
  /-- Fills the `ε` slot of `eposts` with `R`. Example: `set R (R₁, R₂) = (R₁, R)` for `ε` in
  the second slot. -/
  set (R : ε → Pred) (eposts : EPosts) : EPosts
...
instance instEPostSlotHead : EPostSlot ε Pred ((ε → Pred) × EPosts) where
...
instance (priority := low) instEPostSlotTail [EPostSlot ε Pred EPosts] :
    EPostSlot ε Pred ((ε' → Pred') × EPosts) where
  set R eposts := (eposts.1, EPostSlot.set R eposts.2)

/-- Unfolds `set` at a bare exception postcondition. -/
theorem EPostSlot.set_fun (R : ε → Pred) (eposts : ε → Pred) :
    EPostSlot.set R eposts = R := rfl

/-- Unfolds `set` at the head slot of a stack. -/
theorem EPostSlot.set_head (R : ε → Pred) (eposts : (ε → Pred) × EPosts) :
    EPostSlot.set R eposts = (R, eposts.2) := rfl

/-- Unfolds `set` at a slot in the tail of a stack. -/
theorem EPostSlot.set_tail [EPostSlot ε Pred EPosts] (R : ε → Pred)

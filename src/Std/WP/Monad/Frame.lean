...
@[instance_reducible] noncomputable def WPMonad.of_frameClosure {m : Type → Type} [Monad m]
    ...
    {R : Type} (op : R → P → P) [∀ r, PreservesSup (op r)]
    (opE : R → E → E) [∀ r, PreservesSup (opE r)] {comp : R → R → R} {e : R}
  ...
  toWP α := WP.of_frameClosure op opE (base.toWP α)
  pure_le_wp_pure x post E' := by
    show post x ⊑ (((base.toWP _).wpTrans (pure x)).frameClosure op opE).apply post E'
  ...
  bind_le_wp_bind x f post E' := by
    show (((base.toWP _).wpTrans x).frameClosure op opE).apply
          (fun a => (((base.toWP _).wpTrans (f a)).frameClosure op opE).apply post E') E'
        ⊑ (((base.toWP _).wpTrans (x >>= f)).frameClosure op opE).apply post E'
    refine (PredTrans.le_frameClosure_iff op _).mpr fun r => ?_

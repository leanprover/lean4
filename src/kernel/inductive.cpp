...
/* The equations a recursor's rules assert, one per rule, closed under every binder they mention.
   ...
   Each equation is the rule as iota will fire it, taken from the arities the recursor is declared
   with: the major premise sits at `nparams + nmotives + nminors + nindices`, and the fields the
   right-hand side is applied to are the ones iota takes off it.

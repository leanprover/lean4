    ...
    if (!d_t && !d_s) {
        ...
        */
        ...
        if (c < 0) {
            ...
            if (is_eqp(*d_t, *d_s) && d_t->get_hints().is_regular()) {
                // Optimization:
                // If both terms are the same definition (possibly with different but defeq universes),
                // we check if their universe levels and arguments are definitionally equal.
                ...
                if (!failed_before(t_n, s_n)) {
                    ...
                    } else if (is_constant(t_n) && is_constant(s_n)) {
                        // Handle the case where both are plain constants (not applications)
                        // with the same name but potentially different (but defeq) universes.
                        // This avoids expensive unfolding when e.g. comparing c.{max u v} with c.{max v u}.

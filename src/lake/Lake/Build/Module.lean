...
-/
def Module.recParseImports (self : Module) : FetchM (Job (Array Module)) :=
  withRegisterJob s!"{self.name}:imports" <| Job.async do
  let contents ← IO.FS.readFile self.leanFile
  let imports ← Lean.parseImports' contents self.leanFile.toString
...
: FetchM (Job (Array Module)) := do
  let imps ← imports.mapM fun imp => do
    ...
    return {module := imp, transImports, includeSelf : ImportEntry}

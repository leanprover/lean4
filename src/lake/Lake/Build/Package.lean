...
private def Package.recComputeDeps (self : Package) : IndexBuildM (Array Package) := do
...
private def Package.depsFacetConfig : PackageFacetConfig depsFacet :=
...
-/
private def Package.recBuildExtraDepTargets (self : Package) : IndexBuildM (BuildJob Unit) := do
...
private def Package.extraDepFacetConfig : PackageFacetConfig extraDepFacet :=
...
private def Package.fetchRelease (self : Package) : SchedulerM (BuildJob Unit) := Job.async do
...
private def Package.releaseFacetConfig : PackageFacetConfig releaseFacet :=

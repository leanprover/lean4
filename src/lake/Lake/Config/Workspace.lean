...
def addFacet {kind name} (cfg : FacetConfig kind name) (self : Workspace) : Workspace :=
...
def findFacet? (kind name : Name) (self : Workspace) : Option (FacetConfig kind name) :=

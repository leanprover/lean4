...
/-- Load the package's cached inputs from the local Lake artifact cache (if enabled). -/
protected def Package.loadInputs (pkg : Package) (cacheDir : FilePath) : LogIO Package := do
  if !pkg.enableArtifactCache || cacheDir.toString.isEmpty then
    return pkg
  else
    let inputs ← InputMap.load (pkg.inputsFile cacheDir)
    return {pkg with inputMap? := ← IO.mkRef inputs}

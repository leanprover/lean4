...
namespace Lake
open Lean System
open TypeName (typeName)

deriving instance TypeName for
  PackageConfig, LeanLibConfig, LeanExeConfig,
  PackageFacetDecl, LibraryFacetDecl, ModuleFacetDecl,
  ExternLibDecl, TargetDecl, ScriptFn

/-- Unsafe implementation of `evalConstCheck`. -/
unsafe def unsafeEvalConstCheck (α) [TypeName α] (const : Name) (env : Environment) (opts : Options) : Except String α :=
  match env.find? const with
  | none => throw s!"unknown constant '{const}'"
  | some info =>
    match info.type with
    | Expr.const c _ =>
      if c != typeName α then
        throwUnexpectedType
      else
...
where
  throwUnexpectedType : Except String α :=
    throw s!"unexpected type at '{const}', `{typeName α}` expected"
...
(α) [TypeName α] (const : Name) (env : Environment) (opts : Options) : Except String α
...
(env : Environment) (opts : Options) : Except String PackageConfig := do
  let declName ←
  ...
  evalConstCheck PackageConfig declName env opts
...
def Package.finalize (self : Package) (deps : Array Package) : LogIO Package := do
  ...
  let scripts ← mkTagMap env scriptAttr fun scriptName => do
    ...
    let fn ← IO.ofExcept <| evalConstCheck ScriptFn scriptName env opts
  ...
  let leanLibConfigs ← IO.ofExcept <| mkOrdTagMap env leanLibAttr fun name =>
    evalConstCheck LeanLibConfig name env opts
  let leanExeConfigs ← IO.ofExcept <| mkOrdTagMap env leanExeAttr fun name =>
    evalConstCheck LeanExeConfig name env opts
  let externLibConfigs ← mkTagMap env externLibAttr fun name =>
    match evalConstCheck ExternLibDecl name env opts with
    | .ok decl =>
  ...
  let opaqueTargetConfigs ← mkTagMap env targetAttr fun name =>
    match evalConstCheck TargetDecl name env opts with
    | .ok decl =>
      if h : decl.pkg = self.config.name ∧ decl.name = name then

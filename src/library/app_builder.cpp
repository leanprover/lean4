...
namespace lean {
app_builder_cache::key::key(name const & c, unsigned n):
    m_name(c), m_num_expl(n),
    m_hash(::lean::hash(c.hash(), n)) {
}

app_builder_cache::key::key(name const & c, list<bool> const & m):
...
bool app_builder_cache::key::check_invariant() const {
    lean_assert(empty(m_mask) || length(m_mask) == m_num_expl);
    return true;
}

app_builder_cache::app_builder_cache(environment const & env):
    m_env(env),
...
auto app_builder::get_entry(name const & c, unsigned nargs) -> optional<entry> {
    key k(c, nargs);
...
auto app_builder::get_entry(name const & c, unsigned mask_sz, bool const * mask) -> optional<entry> {
...
bool app_builder::check_all_assigned(entry const & e) {
...
expr app_builder::mk_eq(expr const & a, expr const & b) {
    expr A    = m_ctx.infer(a);
    level lvl = get_level(A);
    return ::lean::mk_app(mk_constant(get_eq_name(), {lvl}), A, a, b);
}

expr app_builder::mk_iff(expr const & a, expr const & b) {
    return ::lean::mk_app(mk_constant(get_iff_name()), a, b);
}

expr app_builder::mk_heq(expr const & a, expr const & b) {
    expr A    = m_ctx.infer(a);
...
expr app_builder::mk_eq_rec(expr const & motive, expr const & H1, expr const & H2) {
...
expr app_builder::mk_eq_drec(expr const & motive, expr const & H1, expr const & H2) {
    if (is_constant(get_app_fn(H2), get_eq_refl_name()))
        return H1;
    expr p       = m_ctx.relaxed_whnf(m_ctx.infer(H2));
    expr A, lhs, rhs;

...
struct ematch_fn {
        ...
        if (cg_lemma->m_hcongr_lemma) {
            ...
            for (unsigned i = 0; i < t_args.size(); i++) {
                ...
                if (pinfo.is_inst_implicit()) {
                    new_entries.emplace_back(get_eq_name(), DefEqOnly, p_args[i], t_args[i]);
                } else if (pinfo.is_subsingleton()) {
                    new_entries.emplace_back(get_eq_name(), MatchSS, p_args[i], t_args[i]);
                } else {
                    new_entries.emplace_back(get_eq_name(), Match, p_args[i], t_args[i]);
                }
                pinfos = &tail(*pinfos);
            }
    ...
    bool process_match(name const & R, expr const & p, expr const & t) {
        ...
        do {
            ...
            bool ok = false;
            if ((m_cc.is_congr_root(R, it) || m_cc.eq_class_heterogeneous(it)) && m_ctx->is_def_eq(it_fn, fn) &&
    ...
    bool process_continue(name const & R, expr const & p) {
        ...
        if (auto s = m_inst_ext.get_apps().find(head_index(f))) {
            s->for_each([&](expr const & t) {
                    if (m_cc.is_congr_root(R, t) || m_cc.eq_class_heterogeneous(t)) {
                        state new_state = m_state;

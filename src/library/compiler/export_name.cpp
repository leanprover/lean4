...
namespace lean {
struct export_name_attr_data : public attr_data {
    name m_id;
    export_name_attr_data(name const & id): m_id(id) {}
    export_name_attr_data() {}
...
typedef typed_attribute<export_name_attr_data> export_name_attr;

static export_name_attr const & get_export_name_attr() {
    return static_cast<export_name_attr const &>(get_system_attribute("export"));
}

optional<name> get_export_name_for(environment const & env, name const & n) {
    if (auto const & data = get_export_name_attr().get(env, n)) {
...
void initialize_export_name() {
    register_system_attribute(export_name_attr("export", "name to be used by code generators",
                                           [](environment const & env, io_state const &, name const & n, unsigned, bool persistent) {
                                               ...
                                               auto const & data = *get_export_name_attr().get(env, n);
                                               name it = data.m_id;

...
`BaseIO` functions return their result directly: `Unit` as a boxed scalar, `Bool` and `Status` as
`uint8_t`, `Int32` as `uint32_t` and `Int64` as `uint64_t`. Only the `IO` printing functions return
an `IO` result object.
*/
#include <lean/lean.h>

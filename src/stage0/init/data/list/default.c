...
// Imports: init.data.list.basic init.data.list.aux init.data.list.instances
...
lean_object* initialize_init_data_list_aux(lean_object*);
...
if (lean_io_result_is_error(w)) return w;
w = initialize_init_data_list_aux(w);
if (lean_io_result_is_error(w)) return w;

...
public: // todo : move public lower ; it is for debug only
    ...
    int choose_entering_column(unsigned number_of_benefitial_columns_to_go_over) { // at this moment m_y = cB * B(-1)
        ...
        unsigned offset_in_nb = lrand48() % this->m_non_basic_columns.size();
    ...
    bool get_current_x_is_feasible() const {
        return m_current_x_is_feasible;
    }

    bool get_current_x_is_infeasible() const {
        return !m_current_x_is_feasible;
    }
    ...
    void limit_theta_on_basis_column_for_inf_case_m_neg_fixed(unsigned j, const T & m, X & theta) {
        lean_assert(m < 0 && this->m_column_type[j] == fixed);
        limit_inf_on_bound_m_neg(m, this->m_x[j], this->m_upper_bound_values[j], theta);
    }

    void limit_theta_on_basis_column_for_inf_case_m_pos_fixed(unsigned j, const T & m, X & theta) {
        lean_assert(m > 0 && this->m_column_type[j] == fixed);
        limit_inf_on_bound_m_pos(m, this->m_x[j], this->m_upper_bound_values[j], theta);
    }
    ...
    X harris_eps_for_bound(const X & bound) const {
        return ( convert_struct<X, double>::convert(1) +  abs(bound)/10) * m_converted_harris_eps/3;
    }
    ...
    bool column_is_free(unsigned j) {
        return this->m_column_type[j] == free;
    }
    ...
    bool column_has_low_bound(unsigned j) {
        return this->m_column_type[j] != free_column;
    }

    bool bound_holds(unsigned entering) {
        if (numeric_traits<T>::precise()) return true;
        T v = this->m_x[entering];
        T eps = T(this->m_settings.harris_feasibility_tolerance);

        switch (this->m_column_type[entering]) {
        case low_bound:
            lean_assert(v >= -eps);
            break;
        case boxed:
            lean_assert(v >= -eps);
            lean_assert(v <= this->m_upper_bound_values[entering] + eps);
            break;
        default:
            break;
        }
    ...
    unsigned get_number_of_non_basic_column_to_try_for_enter() {
        ...
        return std::max(static_cast<unsigned>(lrand48() % ret), 1u);
    }
    ...
    void set_current_x_is_feasible() {
        m_current_x_is_feasible = calc_current_x_is_feasible();
    }
    ...
    lu<T, X> * factorization() {
        return this->m_factorization;
    }
...
#endif
        this->m_A.forget_last_columns(how_many_to_forget);
        this->m_n -= how_many_to_forget;
        this->m_non_basic_columns.clear();
        this->init_basis_heading();
    }

    void reduce_basis_by_one(unsigned col) {
        // at this moment this->m_basis has one column too many
        for (unsigned i = this->m_basis_heading[col]; i < this->m_m; i++) {
            unsigned t = this->m_basis[i] = this->m_basis[i + 1];
            this->m_basis_heading[t] = i;
        }
        this->m_basis_heading[col] = -1;
    }

    // k is the redundant row index
    void cross_out_redundant_row_and_remove_basis_column(unsigned k, unsigned col) {
        this->m_A.cross_out_row(k);
        this->m_m--;
        reduce_basis_by_one(col);
    }
    ...
    bool column_is_feasible(unsigned j){
        const X& x = this->m_x[j];
        switch (this->m_column_type[j]) {
        case fixed:
        case boxed:
    ...
    bool calc_current_x_is_feasible() {
        unsigned i = this->m_m;
        while (i--) {

...
template <>
std::string T_to_string(const int & t) {
    std::ostringstream strs;
    strs << t;
    std::string str = strs.str();
    return str;
...
template <>
std::string T_to_string(const unsigned & t) {
    std::ostringstream strs;
    strs << t;
    std::string str = strs.str();
    return str;
...
std::string T_to_string(const mpq & t) {
    std::ostringstream strs;
    strs << t.get_double();
    std::string str = strs.str();
    return str;
...
template <typename X>
X min_abs_in_vector(std::vector<X>& t){
    X r(zero_of_type<X>());
    for (auto & v : t) {
...
public:
    ...
    void apply_from_left(std::vector<T> & w , lp_settings & ) {
        apply_from_left(w);
    }
...
inline unsigned get_width_of_column(unsigned j, std::vector<std::vector<std::string>> & A) {
    unsigned r = 0;
...
#ifdef LEAN_DEBUG
inline void print_matrix_with_widths(std::vector<std::vector<std::string>> & A, std::vector<unsigned> & ws) {
    for (unsigned i = 0; i < A.size(); i++) {
...
inline void print_string_matrix(std::vector<std::vector<std::string>> & A) {
...
public:
    ...
    permutation_matrix(unsigned length, std::vector<unsigned> const & values): m_length(length), m_permutation(length), m_rev(length) {

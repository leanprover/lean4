...
LEAN_EXPORT lean_object* meta_initialize_Init_Data_Nat_Package(uint8_t builtin) {
lean_object * res;
...
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = meta_initialize_Init_Data_Nat_Package(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
return initialize_Init_Data_Nat_Package(builtin);
}
#ifdef __cplusplus
}
#endif

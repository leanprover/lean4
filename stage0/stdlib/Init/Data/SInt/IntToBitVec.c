...
LEAN_EXPORT lean_object* meta_initialize_Init_Data_SInt_IntToBitVec(uint8_t builtin) {
lean_object * res;
...
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = meta_initialize_Init_Data_SInt_IntToBitVec(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
return initialize_Init_Data_SInt_IntToBitVec(builtin);
}
#ifdef __cplusplus
}
#endif

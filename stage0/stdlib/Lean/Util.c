...
lean_object* initialize_Lean_Util_TermUnsafe(uint8_t builtin, lean_object*);
...
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = initialize_Lean_Util_TermUnsafe(builtin, lean_io_mk_world());
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);

...
LEAN_EXPORT lean_object* meta_initialize_Std_Internal_Order_PreservesSup(uint8_t builtin) {
lean_object * res;
...
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = meta_initialize_Std_Internal_Order_PreservesSup(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
return initialize_Std_Internal_Order_PreservesSup(builtin);
}
#ifdef __cplusplus
}
#endif

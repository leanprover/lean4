...
def maxMarkerRun (xs : Array (NOption α)) : Nat := Id.run do
  let mut longest := 0
  let mut current := 0
  for x in xs do
    match x with
...
@[noinline] unsafe def checkValueOnlyUpdates : IO Unit := do
  let m : DHashMap CollisionKey (fun _ => Nat) :=
    (DHashMap.emptyWithCapacity 1).insert (CollisionKey.mk 0) 10
  let keysBeforeModify := m.1.keyArray
  let m := DHashMap.modify m (CollisionKey.mk 0) (· + 1)
  check "modify retains the key array"
    (ptrAddrUnsafe keysBeforeModify == ptrAddrUnsafe m.1.keyArray)
  check "modify updates the value" (m.get? (CollisionKey.mk 0) == some 11)
  let keysBeforeAlter := m.1.keyArray
  let m := DHashMap.alter m (CollisionKey.mk 0) (fun value => value.map (· + 1))
  check "alter retains the key array"
    (ptrAddrUnsafe keysBeforeAlter == ptrAddrUnsafe m.1.keyArray)
  check "alter updates the value" (m.get? (CollisionKey.mk 0) == some 12)
  let keysBeforeMissingErase := m.1.keyArray
  let m := m.erase (CollisionKey.mk 1)
  check "missing erase retains the key array"
    (ptrAddrUnsafe keysBeforeMissingErase == ptrAddrUnsafe m.1.keyArray)

def checkSequentialHashes : IO Unit := do
  let count := 4096
  let m := (List.range count).foldl (init := ({} : HashMap Nat Nat))
    fun m i => m.insert i i
  check "sequential hashes do not form one primary cluster"
    (maxMarkerRun m.1.1.keyArray < 256)

def checkTombstoneCleanup : IO Unit := do
  let keys := (List.range 128).map CollisionKey.mk
  let mut m := keys.foldl (init := HashMap.emptyWithCapacity 128)
    fun m k => m.insert k k.id
  for k in keys.take 100 do
    m := m.erase k
  check "deletion compacts dense tombstones"
    (countKeys m.1.1.keyArray <= 2 * m.size)
  for k in keys.drop 100 do
    check s!"lookup survives tombstone compaction {k.id}" (m[k]? == some k.id)
  for k in keys.drop 100 do
    m := m.erase k
  check "empty table releases all retained key markers" (countKeys m.1.1.keyArray == 0)
...
public unsafe def main : IO Unit := do
  ...
  checkValueOnlyUpdates
  checkSequentialHashes
  checkTombstoneCleanup
  checkCapacityHint

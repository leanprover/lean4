...
example (f : Bool → Nat) : f (a ^^ b) = 0 → a = true → f (!b) = 0 := by grind (splits := 0)
example (f : Bool → Nat) : f (a ^^ b) = 0 → b = true → f (!a) = 0 := by grind (splits := 0)
example (f : Bool → Nat) : (a ^^ b) = c → c = false → f a = 0 → f b = 0 := by grind (splits := 0)
example (f : Bool → Nat) : (a ^^ b) = c → c = false → f b = 0 → f a = 0 := by grind (splits := 0)

-- xor-specific: equivalence/disequality, no `&&`/`||` analogue
example (f : Bool → Nat) : a = b → f (a ^^ b) = 0 → f false = 0 := by grind (splits := 0)
example (f : Bool → Nat) (a : Bool) : f (a ^^ a) = 0 → f false = 0 := by grind (splits := 0)
example (f : Bool → Nat) : a ≠ b → f (a ^^ b) = 0 → f true = 0 := by grind (splits := 0)
example (a b : Bool) : (a ^^ b) = c → c = true → a ≠ b := by grind (splits := 0)

...
trace: [grind.debug.proof] fun h h_1 h_2 h_3 h_4 h_5 h_6 h_7 h_8 =>
      id
        ...
        let p_1 := Poly.add (-1) 0 (Poly.num 1);
        let p_2 := Poly.add 1 0 (Poly.num 0);
        let p_3 := Poly.num 1;
        le_unsat ctx p_3 (eagerReduce (Eq.refl true)) (le_combine ctx p_1 p_2 p_3 (eagerReduce (Eq.refl true)) h_8 h_1))
-/
#guard_msgs in -- `cutsat` context should contain only `f 2`
open Lean Int.Internal.Linear in

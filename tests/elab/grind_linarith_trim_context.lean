...
trace: [grind.debug.proof] fun h h_1 h_2 h_3 h_4 h_5 h_6 h_7 h_8 =>
      id
        ...
        let e_1 := Expr.zero;
        let e_2 := (Expr.intMul 1 (Expr.var 1)).add (Expr.intMul 0 (Expr.var 0));
        ...
        let re_2 := ((CommRing.Expr.num 1).neg.mul (CommRing.Expr.var 0)).add (CommRing.Expr.num 1);
        let re_3 := CommRing.Expr.num 0;
        lt_unsat ctx
          (le_lt_combine ctx p_3 p_5 p_1 (eagerReduce (Eq.refl true))
            (le_le_combine ctx p_4 p_2 p_3 (eagerReduce (Eq.refl true))
              (le_norm ctx e_3 e_1 p_4 (eagerReduce (Eq.refl true))
                (CommRing.le_int_module rctx rp_2
                  (CommRing.le_norm rctx re_2 re_3 rp_2 (eagerReduce (Eq.refl true)) h_8)))
              (le_norm ctx e_2 e_1 p_2 (eagerReduce (Eq.refl true))
                (CommRing.le_int_module rctx rp_1
                  (CommRing.le_norm rctx re_1 re_3 rp_1 (eagerReduce (Eq.refl true)) h_1))))

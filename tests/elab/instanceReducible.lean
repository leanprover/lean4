...
/-- warning: Definition `unexposed` of class type must be marked with `@[reducible]` or `@[instance_reducible]` -/
#guard_msgs in
...
/-- warning: Definition `exposed` of class type must be marked with `@[reducible]` or `@[instance_reducible]` -/
#guard_msgs in
@[expose]

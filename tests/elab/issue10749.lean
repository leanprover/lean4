...
/--
info: @[instance_reducible] def test.match_1.{u_1} : (motive : List Nat → Sort u_1) →
...
/--
info: @[instance_reducible] def test2.match_1.{u_1} : (motive : List Nat → List Nat → Sort u_1) →
  (a b : List Nat) →
    ((x : List Nat) → motive [] x) →
      ((x : List Nat) → motive x []) →
        ((head : Nat) →
...
/--
info: @[instance_reducible] def test3.match_1.{u_1} : (motive : List Nat → Bool → Sort u_1) →
  (a : List Nat) →
    (b : Bool) →
...
/--
info: @[instance_reducible] def test4.match_1.{u_1} : (motive : Bool → Bool → Bool → Bool → Bool → Sort u_1) →
...
/--
info: @[instance_reducible] def testOld.match_1.{u_1} : (motive : List Nat → Sort u_1) →
  (a : List Nat) → ((x : List Nat) → motive x) → (Unit → motive []) → motive a :=

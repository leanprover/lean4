...
The last block covers `NestedGen.certify`, the entry point `environment::add_inductive` calls, which
reaches the generator through a kernel environment rather than the one being elaborated.
-/
...
/-!
An unsafe nested inductive still gets its recursors derived from the model, but nothing certifies
them: the bridges are definitions and the rule proofs are theorems, and neither may mention an
unsafe constant. Such a declaration is outside the kernel's guarantees to begin with.

Pinned because it is the case that has no certificate, so it is the one that breaks when the
...
occurrence and the declaration is rejected.

The message is poor: it names an internal constant of the scratch model and calls the failure an
uncertified computation rule, when what went wrong is the occurrence. Pinned as it stands so the
rejection itself is guarded; the wording wants improving, and this expectation with it.
-/
...
/-!
`SigmaNest` is reflexive, and it takes the model to see it. `is_reflexive` asks `is_pi` of each
constructor field, and the field here is `(fun I => I → SigmaNest) fst`, a redex that hides the
binder. The model has already reduced it to `fst → SigmaNest`, which really is a function into the
type, so the flag now comes from there rather than from the presentation the kernel was handed.

This is what decides which `below` and `brecOn` get generated for such a type, so pin it.
-/
run_meta do
  ...
  unless declared && derived do
    throwError "expected both the declaration and the model reflexive, got \

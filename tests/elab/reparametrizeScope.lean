...
/-!
Tests that `Induction.Reparametrize.reparametrize` rejects a result that refers to erased
declarations. Only the `fold` argument can smuggle such a reference past the dependency guards, so
the test uses a fold that reintroduces the replaced variable itself. The result is type correct in
the old context but not in the new one.
-/
...
elab "bad_fold" : tactic => withMainContext do
  ...
  let r? ← withLocalDeclD `y (mkConst ``Wrap) fun y => do
    ...
    Induction.Reparametrize.reparametrize (← getMainGoal) a.fvarId y.fvarId! yInner

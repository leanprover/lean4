...
-- A post-hoc reducibility change (here: overriding an imported declaration) bumps the change
-- counter and conservatively invalidates every earlier entry, including entries whose search
-- never consulted the declaration.
attribute [local implicit_reducible] Function.const

/-- trace: [Meta.synthInstance.cache] new: R Nat -/
#guard_msgs in
def q7 : Unit := let _ : R Nat := inferInstance; ()

/-- trace: [Meta.synthInstance.cache] cached: R Nat -/
#guard_msgs in
def q8 : Unit := let _ : R Nat := inferInstance; ()

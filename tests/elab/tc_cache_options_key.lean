...
options (e.g. pretty printing) are never recorded, and a plain by-name read of a non-inert
option on the search path panics (`Lean.OptionsRestriction`), so no access can go untracked.
-/

open Lean

-- Inert options are accessible as usual under the `tcResolution` restriction; other reads must
-- use the recording accessors, which read via `findUnrestricted?`.
/-- info: true -/
#guard_msgs in

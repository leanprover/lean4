...
(`Lean.getRecordedOption`), and an entry is reused exactly when its recorded lookups give the
same answers. Options the search never read do not partition the cache, result-irrelevant
options (e.g. pretty printing) are never recorded, and plainly acquiring the options on the
search path panics (`Core.Context.recordingDeps`), so no access can go untracked; the frameworks
whose reads cannot influence results (trace collection, limits) acquire them via
`getOptionsUnrestricted` at their accessors.
-/

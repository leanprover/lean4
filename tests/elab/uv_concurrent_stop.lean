...
exactly once. The signal is `sigwinch` because libuv accepts it on every platform; on Windows the
Unix-only signals map to `signum = 0`, which `uv_signal_start` rejects with `UV_EINVAL`.
-/

...
A user-defined docstring role saves a custom data element, and a renderer registered with
`@[doc_inline_md]` controls how that element renders to Markdown. The renderer is written against the
custom element type directly (`InlineMdRendererOf X`); the attribute generates the `Dynamic`-decoding
wrapper, so renderers never handle the `Dynamic`. The renderer runs with environment access, so it
can look up information at render time. Elements whose type has no registered renderer fall back to
rendering their fallback content. In the module system, roles and renderers must be `meta`.
-/
...
/--
Renders a declaration's docstring to Markdown, as the language server would.
-/
elab "#render_doc " name:ident : command => do
  ...
  runTermElabM fun _ => do
    ...
    match (← findSimpleDocString? env declName (currNamespace := currNamespace) (openDecls := openDecls)) with
    | some s => logInfo s
...
meta structure Unrendered where
deriving TypeName
...
meta structure Aliased where
  /-- The text to render. -/
  msg : String
deriving TypeName

/-- A reducible alias of the element type. -/
meta abbrev AliasedSyn := Aliased
...
@[doc_inline_md]
meta def aliasedRender : InlineMdRendererOf AliasedSyn := fun _go data _content => do
  return #[data.msg]
...
@[doc_block_md]
meta def bannerRender : BlockMdRendererOf Banner := fun _goI goB _data content => do
...
meta structure PlainBlock where
deriving TypeName
...
meta structure SlowBlock where
deriving TypeName
...
meta structure NoTypeName where
  x : Nat
...
#guard_msgs in
@[doc_inline_md]
meta def badRenderer : InlineMdRendererOf NoTypeName := fun _go _data _content => return #[]

...
# picks up from the lint target's import closure and runs once each, appending their entries to
# the linter-derived ones. `Checks.lean` defines the checks imported by every target here;
# `Extra.lean` and `Failing.lean` each register one further check visible only in their own
# target.
...
# --- Multiple lint targets: each check runs exactly once across all of them. ---
lake_out lint --code-quality Pkg Extra
collect_json
# `sizeMetric` is registered in both targets' closures but contributes its two entries once.
count_text 2 '"name":"sizeMetric"' produced.json
count_text 1 '"name":"tallyMetric"' produced.json
# The check registered only by `Extra` now runs, also exactly once.

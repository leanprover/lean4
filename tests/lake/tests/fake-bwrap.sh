...
# Lake spawns the sandbox through `env -i`, so this shim starts with no `PATH` and a bare `env`
# does not resolve. `/usr/bin/env` is the one location the shebang above already depends on.
ENV_BIN=/usr/bin/env
...
if [[ "$cleared" == 1 ]]; then
  exec "$ENV_BIN" -i "${env_args[@]}" "$@"
else
  exec "$ENV_BIN" "${env_args[@]}" "$@"
fi

...
@[simp] theorem sub_add_bmod_cancel {x y : BitVec w} :
  ...
  grind [Int.add_bmod_right]

private theorem lt_two_pow_of_le {x m n : Nat} (lt : x < 2 ^ m) (le : m ≤ n) : x < 2 ^ n :=
  Nat.lt_of_lt_of_le lt (Nat.pow_le_pow_right (by grind) le)
...
theorem getLsbD_ofBool (b : Bool) (i : Nat) : (ofBool b).getLsbD i = ((i = 0) && b) := by
  ...
  · simp only [ofBool, ofNat_eq_ofNat, cond_true, getLsbD_ofNat, Bool.and_true]
    grind [Nat.mod_succ, Nat.testBit_zero, getLsbD_ofNat]
...
grind_pattern BitVec.isLt => x.toNat
...
@[simp] theorem getMsbD_extractLsb' {start len : Nat} {x : BitVec w} {i : Nat} :
  ...
  simp only [getMsbD_eq_getLsbD]
  grind (splits := 12) [getLsbD_extractLsb', getLsbD_eq_getMsbD]
...
@[simp, grind =] theorem xor_self {x : BitVec w} : x ^^^ x = 0#w := by
  ext i
  simp
    ...
    · cases w : decide (i < v)
      · simp only [decide_eq_false_iff_not, Nat.not_lt] at w
        ...
        calc BitVec.toNat x < 2 ^ v := isLt _
          _ ≤ 2 ^ i := Nat.pow_le_pow_right (by grind) (by grind)
      · simp
...
@[simp] theorem setWidth_not {x : BitVec w} (_ : k ≤ w) :
  ...
  ext i h
  grind [BitVec.getElem_setWidth, BitVec.getLsbD_not, BitVec.getElem_not]
...
theorem extractLsb_not_of_lt {x : BitVec w} {hi lo : Nat} (hlo : lo ≤ hi) (hhi : hi < w) :
  ...
  ext k hk
  grind [BitVec.getElem_extract, BitVec.getLsbD_not, BitVec.getElem_not]
...
theorem shiftLeft_xor_distrib (x y : BitVec w) (n : Nat) :
    (x ^^^ y) <<< n = (x <<< n) ^^^ (y <<< n) := by
  ext i h
  grind [getElem_shiftLeft, getElem_xor]
...
theorem cons_append_append (x : BitVec w₁) (y : BitVec w₂) (z : BitVec w₃) (a : Bool) :
  ...
  ext i h
  simp only [cons, getElem_append, getElem_cast, getElem_ofBool, BitVec.cast_cast, getLsbD_append, getLsbD_cast, getLsbD_ofBool]
  by_cases h₀ : i < w₁ + w₂ + w₃
  · simp only [h₀, ↓reduceIte]
    by_cases h₁ : i < w₃
    · simp [h₁]

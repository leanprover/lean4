...
@[simp] theorem nil_eq {α} {xs : List α} : [] = xs ↔ xs = [] := by grind
...
@[simp] theorem length_eq_zero_iff : length l = 0 ↔ l = [] := by grind
...
-- @[simp] theorem ne_cons_self {a : α} {l : List α} : l ≠ a :: l := by grind
...
-- @[simp] theorem concat_ne_nil (a : α) (l : List α) : l ++ [a] ≠ [] := by grind
...
@[simp] theorem get_eq_getElem {l : List α} {i : Fin l.length} : l.get i = l[i.1]'i.2 := by grind
...
@[simp] theorem getElem?_nil {i : Nat} : ([] : List α)[i]? = none := by grind
...
@[simp] theorem getElem?_cons_succ {l : List α} : (a::l)[i+1]? = l[i]? := by grind
...
@[simp] theorem mem_singleton {a b : α} : a ∈ [b] ↔ a = b := by grind

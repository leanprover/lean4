...
-- This `grind` call fails nondeterministically.
-- Adjust whitespace to reproduce, it succeeds less than half the time.
theorem getLast?_dropLast {xs : List α} :
    xs.dropLast.getLast? = if xs.length ≤ 1 then none else xs[xs.length - 2]? := by
  grind only [List.getElem?_eq_none, List.getElem?_reverse, getLast?_eq_getElem?,

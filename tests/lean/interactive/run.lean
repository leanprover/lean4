  ...
  for enc in encodings.getD #[#[]] do
    ...
    Ipc.runWith (←IO.appPath) #["--server"] do
      let capabilities := {
        ...
        general? := some {
          positionEncodings := enc
        }
      }
      Ipc.writeRequest ⟨0, "initialize", { capabilities : InitializeParams }⟩
      let initRes ← Ipc.readResponseAs 0 InitializeResult
      let encoding := initRes.result.capabilities.positionEncoding
      correctEncoding enc encoding
      ...
      let fileMap : FileMap := FileMap.ofString text

...
theorem Nat.bit'_div_even (h : n % 2 = 0) : bit' false (n / 2) = n := by
  simp [bit']
  have := Nat.div_add_mod n 2
  simp [h] at this
  assumption

theorem Nat.bit'_div_odd (h : n % 2 ≠ 0) : bit' true (n / 2) = n := by
  have h : n % 2 = 1 := by
...
def Nat.binrec
  ...
  if h₁ : n = 0 then
    ...
    bit'_div_even h₂ ▸ ind false (n / 2) (fun _ => binrec motive base ind (n / 2))
  else
    bit'_div_odd h₂ ▸ ind true  (n / 2) (fun _ => binrec motive base ind (n / 2))
termination_by n

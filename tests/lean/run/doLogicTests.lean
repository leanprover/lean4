...
namespace List.Cursor

theorem mem_suffix_of_mem_suffix_tail {α} {l : List α} {c : Cursor l} {x : α}
    (h : 0 < c.suffix.length) (hmem : x ∈ c.tail.suffix) :
    x ∈ c.suffix := by
  sorry
grind_pattern mem_suffix_of_mem_suffix_tail => x ∈ c.tail.suffix

@[grind →]
theorem suffix_eq_current_cons_suffix_tail {α} {l : List α} {c : Cursor l}
    (h : 0 < c.suffix.length) :
    c.suffix = c.current :: c.tail.suffix := by
  sorry

@[simp, grind →]
theorem prefix_tail_eq_prefix_append_current {α} {l : List α} {c : Cursor l}
    (h : 0 < c.suffix.length) :
    c.tail.prefix = c.prefix ++ [c.current] := by
  sorry
...
theorem sum_loop_spec :
  ...
  unfold sum_loop
  mspec
  case inv => exact (⇓ (xs, r) => ⌜r + xs.suffix.length * 5 ≤ 25⌝)
...
theorem throwing_loop_spec :
  ...
  case post.success =>
    ...
    mspec
    mspec
    simp_all only [List.Cursor.suffix_end, List.sum_nil, Nat.add_zero, gt_iff_lt, SPred.entails_nil,
      SPred.down_pure, imp_false, not_true_eq_false]
    omega
  ...
  case step =>
    ...
    case isFalse => intro _; mintro _; mspec; intro _; simp_all +arith [List.Cursor.suffix_eq_current_cons_suffix_tail]
...
theorem beaking_loop_spec :
  ...
  case step =>
    ...
    case isFalse => intro _; mintro _; mspec; simp_all [List.Cursor.prefix_tail_eq_prefix_append_current]; omega
...
theorem fib_triple : ⦃⌜True⌝⦄ fib_impl n ⦃⇓ r => ⌜r = fib_spec n⌝⦄ := by
  ...
  case step => dsimp; intros; mintro _; simp_all [List.Cursor.prefix_tail_eq_prefix_append_current]
...
theorem fib_triple_cases : ⦃⌜True⌝⦄ fib_impl n ⦃⇓ r => ⌜r = fib_spec n⌝⦄ := by
  ...
  mspec
  mspec Spec.forIn_range (⇓ ⟨xs, a, b⟩ => ⌜a = fib_spec xs.prefix.length ∧ b = fib_spec (xs.prefix.length + 1)⌝) ?step
  case step => dsimp; intros; mintro _; mspec; mspec; simp_all [List.Cursor.prefix_tail_eq_prefix_append_current]
  simp_all [Nat.sub_one_add_one]
...
theorem fib_impl_vcs
    ...
    (ret : ⊢ₛ (Q 0).1 0)
    (loop_pre : ∀ n (hn : ¬n = 0), ⊢ₛ (I n hn).1 ⟨.begin _, 0, 1⟩)
    (loop_post : ∀ n (hn : ¬n = 0) r, (I n hn).1 ⟨.end _, r⟩ ⊢ₛ (Q n).1 r.2)
    (loop_step : ∀ n (hn : ¬n = 0) (c : [1:n].toList.Cursor) (h : 0 < c.suffix.length) r,
                  (I n hn).1 ⟨c, r⟩ ⊢ₛ (I n hn).1 ⟨c.tail h, r.2, r.1+r.2⟩)
...
theorem fib_triple_vcs : ⦃⌜True⌝⦄ fib_impl n ⦃⇓ r => ⌜r = fib_spec n⌝⦄ := by
  ...
  case loop_step => simp_all [List.Cursor.prefix_tail_eq_prefix_append_current]
...
theorem max_and_sum_spec (xs : Array Nat) :
  ...
  · rw [Nat.left_distrib]
    ...
    rename_i max _ _ _ h₂ h
    apply Nat.le_trans h
    apply Nat.mul_le_mul_right
    -- grind -- `grind` internal error, found non-normalized div/mod by 1
    omega
  · rw [Nat.left_distrib]
    grind
...
theorem fast_expo_correct (x n : Nat) : fast_expo x n = x^n := by
  ...
  all_goals simp_all
  case vc1 b _ _ _ _ _ _ _ _ ih =>
    obtain ⟨e, y, x'⟩ := b
    subst_vars
    grind
  case vc2 b _ _ _ _ _ _ _ _ ih _ =>
    obtain ⟨e, y, x'⟩ := b
    simp at *
    constructor
    · rw [← Nat.mul_assoc, ← Nat.pow_add_one, ← ih.1]
      ...
      rw [this]
    · grind
  case vc3 b _ _ _ _ _ _ _ _ ih _ =>
    obtain ⟨e, y, x'⟩ := b
    simp at *
    constructor

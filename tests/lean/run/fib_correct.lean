...
theorem fib_correct {n} : fib_impl n = fib_spec n := by
  ...
  simp only [fib_impl, Nat.zero_add, bind_pure_comp, map_pure, Std.Range.forIn_eq_forIn_range',
    Std.Range.size, Nat.add_one_sub_one, Nat.div_one, List.forIn_pure_yield_eq_foldl]
  match n with
  | 0 => simp [fib_spec]
  | n+1 =>

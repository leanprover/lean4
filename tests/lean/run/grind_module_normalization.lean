...
def IntModule.natSMul : SMul Nat R where
  smul := (· * ·)

attribute [local instance] IntModule.natSMul -- Check the everything still works if we use `•` notation.

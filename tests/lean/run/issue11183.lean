...
mutual
partial def currentlyEnabled [Monad M] (merge_var : extension) : M Bool := do
...
partial def get_xLPE [Monad M] (p : Privilege) : M Bool := do
  match p with
  | Machine => pure false
...
#print sig currentlyEnabled.match_1.splitter

...
/-!
We check that `Nat.gcd` is evaluated fast in the kernel
by timing a `rfl` proof that used to be slow.
-/
...
#fast example : Nat.gcd (115249 * 180811) (115249*181081) = 115249 := rfl
...
#fast example : Nat.gcd (p_29 * p_30 * p_31 * p_32 * p_33) 2^(2^20) = 1 := rfl
/- GCD with two prime factors on both sides, including one in common. -/
#fast example : Nat.gcd (m p_31 * m p_33) (m p_32 * m p_33) - m p_33 = 0 := rfl
...
#fast example : Nat.gcd (maxSmallNat - 92737) (maxSmallNat + 92737) = 185474 := rfl
#fast example : Nat.gcd (maxSmallNat / 649657) (maxSmallNat * 649657) = 14197294936951 := rfl

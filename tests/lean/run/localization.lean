...
-- This is not exactly fast, but it works well within the default maxHeartbeats,
-- with `backward.isDefEq.lazyWhnfCore false`
...
-- If we make some things irreducible it is fast:
set_option maxHeartbeats 1000 in

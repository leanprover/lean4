...
theorem parseAll1.eq_1 : ∀ {α : Type} (x : M1 α),
  parseAll1 x = do
    let __do_lift ← read
    let __do_lift_1 ← get
    have __do_jp : Unit → M1 (List α) := fun __r => do
      let val ← x
      let list ← parseAll1 x
      pure (val :: list)
    if String.Pos.Raw.atEnd __do_lift __do_lift_1 = true then pure [] else __do_jp PUnit.unit
-/
...
theorem parseAll2.eq_1 : ∀ {α : Type} (x : M2 α),
  parseAll2 x = do
    let __do_lift ← read
    let __do_lift_1 ← get
    have __do_jp : Unit → M2 (List α) := fun __r => do
      let val ← x
      let list ← parseAll2 x
      pure (val :: list)
    if String.Pos.Raw.atEnd __do_lift __do_lift_1 = true then pure [] else __do_jp PUnit.unit

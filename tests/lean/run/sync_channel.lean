...
def closeClose (ch : ClosableChannel Nat) : IO Unit := do
...
def trySend (ch : ClosableChannel Nat) (capacity : Option Nat) : IO Unit := do
  -- ready a receiver ahead of time
  let recvTask ← ch.recv
  assertBEq (← ch.trySend 37) true
  assertBEq (← IO.wait recvTask) (some 37)

  -- the unbounded ClosableChannel cannot go out of space so it is pointless to fill it up
...
def tryRecv (ch : ClosableChannel Nat) : IO Unit := do
  assertBEq (← ch.tryRecv) none
  let sendTask ← ch.send 37
  assertBEq (← ch.tryRecv) (some 37)
  assertBEq ((← IO.wait sendTask) matches .ok ()) true

def sendRecvClose (ch : ClosableChannel Nat) : IO Unit := do
  let sendTask ← ch.send 37
  assertBEq ((← EIO.toBaseIO ch.close) matches .ok ()) true
  let recvTask ← ch.recv
  assertBEq ((← IO.wait sendTask) matches .ok ()) true
...
def sendIt (ch : ClosableChannel Nat) (messages : List Nat) : BaseIO (Task (Option Unit)) := do
  match messages with
  | [] => return .pure <| some ()
  | msg :: messages =>
...
partial def recvIt (ch : ClosableChannel Nat) (messages : List Nat) : BaseIO (Task (List Nat)) := do
  BaseIO.bindTask (← ch.recv) fun
    | none => return .pure messages.reverse
    | some msg => recvIt ch (msg :: messages)

def sendLots (ch : ClosableChannel Nat) : IO Unit := do
...
def sendItSync (ch : ClosableChannel.Sync Nat) (messages : List Nat) : IO Unit := do
  for msg in messages do
    ch.send msg
  return ()

def recvItSync (ch : ClosableChannel.Sync Nat) : IO (List Nat) := do
  let mut messages := []
  for msg in ch do
    messages := msg :: messages
  return messages.reverse

def sendLotsSync (ch : ClosableChannel.Sync Nat) : IO Unit := do
  let messages := List.range 1000
...
partial def sendLotsMulti (ch : ClosableChannel Nat) : IO Unit := do
  let messages := List.range 1000
...
partial def sendLotsMultiSync (ch : ClosableChannel.Sync Nat) : IO Unit := do
  let messages := List.range 1000
...
def testIt (capacity : Option Nat) : IO Unit := do
  paired (← ClosableChannel.new capacity)
  syncPaired (← ClosableChannel.new capacity).sync

  closeClose (← ClosableChannel.new capacity)
  trySend (← ClosableChannel.new capacity) capacity
  tryRecv (← ClosableChannel.new capacity)
  sendRecvClose (← ClosableChannel.new capacity)

  sendLots (← ClosableChannel.new capacity)
  sendLotsSync (← ClosableChannel.new capacity).sync
  sendLotsMulti (← ClosableChannel.new capacity)
  sendLotsMultiSync (← ClosableChannel.new capacity).sync

...
/--
warning: declaration `MTree.map` uses `sorry`
---
trace: α : Type u_1
t : MTree α
x✝¹ : List (MTree α)
...
/--
warning: declaration `Ex1.t` uses `sorry`
---
...
/--
warning: declaration `Ex2.t` uses `sorry`
---
trace: L : List (String × Expression)
x : String × Expression
h✝ : x ∈ L
...
/-- warning: declaration `List.List.zipWith_unattach` uses `sorry` -/

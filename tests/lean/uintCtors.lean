...
#eval (3 : UInt64).val
#eval toString $ { val := { val := 3, isLt := (match USize.size, USize.size_eq with | _, Or.inl rfl => (by decide) | _, Or.inr rfl => (by decide)) } : USize }
#eval (3 : USize).val
...
#eval (4 : UInt64).val
#eval toString $ { val := { val := 4, isLt := (match USize.size, USize.size_eq with | _, Or.inl rfl => (by decide) | _, Or.inr rfl => (by decide)) } : USize }
#eval (4 : USize).val

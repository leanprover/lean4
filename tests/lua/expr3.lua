...
assert(mk_local("m", Bool):is_mlocal())
assert(mk_metavar("m", Bool):is_meta())
assert(mk_metavar("m", Bool)(a):is_meta())
assert(f(mk_metavar("m", Bool)):has_metavar())
assert(f(mk_local("l", Bool)):has_local())
assert(not f(mk_metavar("m", Bool)):has_local())
assert(not f(mk_local("l", Bool)):has_metavar())
...
assert(f1 == f)
assert(a1 == a)
local m1, t1 = mk_metavar("a", Bool):data()
assert(m1 == name("a"))
assert(t1 == Bool)
local m1, t1 = mk_local("a", Bool):data()
assert(m1 == name("a"))
assert(t1 == Bool)
local a1, t, b1, bi = mk_pi("a", Bool, Var(0), binder_info(true)):data()
assert(a1 == name("a"))
assert(t == Bool)
assert(b1 == Var(0))

...
if [[ "$($NODE -p 'Number(process.versions.node.split(".")[0])')" -lt 20 && -x /opt/homebrew/bin/node ]]; then
  NODE=/opt/homebrew/bin/node
fi
[[ "$($NODE -p 'Number(process.versions.node.split(".")[0])')" -ge 20 ]] || {
  echo "Node.js 20 or newer is required (set NODE to its executable)" >&2
  exit 1
}
...
(cd .. && lean -o browser_demo/PersistentTree.olean --wasm=browser_demo/_tmp_tree.o.wasm PersistentTree.lean)
...
leanc --target=wasm32-wasip1 \
  ...
  -o ui.wasm _tmp_ui_abi.o.wasm _tmp_tree.o.wasm "$OBJ"
wasm-validate --enable-exceptions ui.wasm
"$NODE" ui_abi_smoke.mjs
rm -f "$OBJ" _tmp_ui_abi.o.wasm _tmp_tree.o.wasm UiAbi.ir UiAbi.ir.sig UiAbi.olean UiAbi.ilean \
  UiAbi.olean.private UiAbi.olean.server PersistentTree.ir PersistentTree.ir.sig \
  PersistentTree.olean PersistentTree.ilean PersistentTree.olean.private PersistentTree.olean.server

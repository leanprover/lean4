...
#eval do
  ...
  IO.println s!"duplicate-unchanged={duplicate.versions.size == branch.versions.size && rootId (selectedVersion duplicate).root == rootId (selectedVersion branch).root}"

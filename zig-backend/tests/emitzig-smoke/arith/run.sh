...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/emitzig-smoke.arith.XXXXXX")"
TMP_INCLUDE_DIR="$TMP_DIR/include"
OUT_ZIG="$TMP_DIR/arith.zig"
EMITZIG_O="$TMP_DIR/arith.o"
DRIVER_O="$TMP_DIR/driver.o"
COMPAT_O="$TMP_DIR/compat.o"
OUT_BIN="$TMP_DIR/arith"
...
cd "$EMITZIG_DIR"
"$LAKE_BIN" exe emitzig "$SCRIPT_DIR/Arith.lean" -o "$OUT_ZIG"

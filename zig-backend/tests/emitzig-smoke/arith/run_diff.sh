...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/emitzig-diff.arith.XXXXXX")"
TMP_INCLUDE_DIR="$TMP_DIR/include"
EMITC_C="$TMP_DIR/arith.c"
EMITC_O="$TMP_DIR/arith-emitc.o"
EMITC_DRIVER_C="$TMP_DIR/arith-emitc-driver.c"
EMITC_DRIVER_O="$TMP_DIR/arith-emitc-driver.o"
EMITZIG_ZIG="$TMP_DIR/arith.zig"
EMITZIG_O="$TMP_DIR/arith-emitzig.o"
EMITZIG_DRIVER_O="$TMP_DIR/arith-emitzig-driver.o"
COMPAT_O="$TMP_DIR/compat.o"
EMITC_BIN="$TMP_DIR/arith-emitc"
EMITZIG_BIN="$TMP_DIR/arith-emitzig"
...
lean_object * initialize_Arith(uint8_t builtin);
lean_object * l___private_Arith_0__main(void);

static lean_object * run_main(int argc, char ** argv) {
  (void)argc;
  (void)argv;
  return l___private_Arith_0__main();
}
...
int main(int argc, char ** argv) {
  ...
  lean_initialize();
  res = initialize_Arith(1 /* builtin */);
...
cc -Wall -Wextra -pedantic -x c -std=c11 -c \
  -DEMITZIG_INIT_FN=initialize_Arith \
  -DEMITZIG_MAIN_FN=l___private_Arith_0__main \
  -I "$TMP_INCLUDE_DIR" \
  -I "$LEAN4_DIR/src/include" \
  -I "$LEAN4_DIR/build/release/stage1/include" \
  "$COMMON_DIR/driver.c" \
...
"$LEAN_BIN" -R "$SCRIPT_DIR" -c "$EMITC_C" "$SCRIPT_DIR/Arith.lean"
...
cd "$EMITZIG_DIR"
"$LAKE_BIN" exe emitzig "$SCRIPT_DIR/Arith.lean" -o "$EMITZIG_ZIG"

...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/emitzig-smoke.array_loop.XXXXXX")"
TMP_INCLUDE_DIR="$TMP_DIR/include"
OUT_ZIG="$TMP_DIR/array_loop.zig"
EMITZIG_O="$TMP_DIR/array_loop.o"
DRIVER_O="$TMP_DIR/driver.o"
COMPAT_O="$TMP_DIR/compat.o"
OUT_BIN="$TMP_DIR/array_loop"
...
cd "$EMITZIG_DIR"
"$LAKE_BIN" exe emitzig "$SCRIPT_DIR/ArrayLoop.lean" -o "$OUT_ZIG"

cc -Wall -Wextra -pedantic -x c -std=c11 -c \
  -DEMITZIG_INIT_FN=initialize_ArrayLoop \
  -DEMITZIG_MAIN_FN=l___private_ArrayLoop_0__main \
  -I "$TMP_INCLUDE_DIR" \

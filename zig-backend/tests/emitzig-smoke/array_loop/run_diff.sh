...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/emitzig-diff.array_loop.XXXXXX")"
TMP_INCLUDE_DIR="$TMP_DIR/include"
EMITC_C="$TMP_DIR/array_loop.c"
EMITC_O="$TMP_DIR/array_loop-emitc.o"
EMITC_DRIVER_C="$TMP_DIR/array_loop-emitc-driver.c"
EMITC_DRIVER_O="$TMP_DIR/array_loop-emitc-driver.o"
EMITZIG_ZIG="$TMP_DIR/array_loop.zig"
EMITZIG_O="$TMP_DIR/array_loop-emitzig.o"
EMITZIG_DRIVER_O="$TMP_DIR/array_loop-emitzig-driver.o"
COMPAT_O="$TMP_DIR/compat.o"
EMITC_BIN="$TMP_DIR/array_loop-emitc"
EMITZIG_BIN="$TMP_DIR/array_loop-emitzig"
...
lean_object * initialize_ArrayLoop(uint8_t builtin);
lean_object * l___private_ArrayLoop_0__main(void);

static lean_object * run_main(int argc, char ** argv) {
  (void)argc;
  (void)argv;
  return l___private_ArrayLoop_0__main();
}
...
int main(int argc, char ** argv) {
  ...
  lean_initialize();
  res = initialize_ArrayLoop(1 /* builtin */);
...
"$LEAN_BIN" -R "$SCRIPT_DIR" -c "$EMITC_C" "$SCRIPT_DIR/ArrayLoop.lean"
...
cd "$EMITZIG_DIR"
"$LAKE_BIN" exe emitzig "$SCRIPT_DIR/ArrayLoop.lean" -o "$EMITZIG_ZIG"

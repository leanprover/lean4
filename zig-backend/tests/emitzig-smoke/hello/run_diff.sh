...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/emitzig-diff.hello.XXXXXX")"
TMP_INCLUDE_DIR="$TMP_DIR/include"
EMITC_C="$TMP_DIR/hello.c"
EMITC_O="$TMP_DIR/hello-emitc.o"
EMITC_DRIVER_C="$TMP_DIR/hello-emitc-driver.c"
EMITC_DRIVER_O="$TMP_DIR/hello-emitc-driver.o"
EMITZIG_ZIG="$TMP_DIR/hello.zig"
EMITZIG_O="$TMP_DIR/hello-emitzig.o"
EMITZIG_DRIVER_O="$TMP_DIR/hello-emitzig-driver.o"
COMPAT_O="$TMP_DIR/compat.o"
EMITC_BIN="$TMP_DIR/hello-emitc"
EMITZIG_BIN="$TMP_DIR/hello-emitzig"
...
lean_object * initialize_Hello(uint8_t builtin);
lean_object * l___private_Hello_0__main(void);

static lean_object * run_main(int argc, char ** argv) {
  (void)argc;
  (void)argv;
  return l___private_Hello_0__main();
}
...
int main(int argc, char ** argv) {
  ...
  lean_initialize();
  res = initialize_Hello(1 /* builtin */);
...
"$LEAN_BIN" -R "$SCRIPT_DIR" -c "$EMITC_C" "$SCRIPT_DIR/Hello.lean"
...
cd "$EMITZIG_DIR"
"$LAKE_BIN" exe emitzig "$SCRIPT_DIR/Hello.lean" -o "$EMITZIG_ZIG"

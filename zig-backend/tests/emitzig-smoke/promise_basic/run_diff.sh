...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/emitzig-diff.promise_basic.XXXXXX")"
TMP_INCLUDE_DIR="$TMP_DIR/include"
EMITC_C="$TMP_DIR/promise_basic.c"
EMITC_O="$TMP_DIR/promise_basic-emitc.o"
EMITC_DRIVER_C="$TMP_DIR/promise_basic-emitc-driver.c"
EMITC_DRIVER_O="$TMP_DIR/promise_basic-emitc-driver.o"
EMITZIG_ZIG="$TMP_DIR/promise_basic.zig"
EMITZIG_O="$TMP_DIR/promise_basic-emitzig.o"
EMITZIG_DRIVER_O="$TMP_DIR/promise_basic-emitzig-driver.o"
TASK_SHIMS_O="$TMP_DIR/task_api_shims.o"
COMPAT_O="$TMP_DIR/compat.o"
EMITC_BIN="$TMP_DIR/promise_basic-emitc"
EMITZIG_BIN="$TMP_DIR/promise_basic-emitzig"
...
lean_object * initialize_PromiseBasic(uint8_t builtin);
lean_object * l___private_PromiseBasic_0__main(void);

static lean_object * run_main(int argc, char ** argv) {
  (void)argc;
  (void)argv;
  return l___private_PromiseBasic_0__main();
}
...
int main(int argc, char ** argv) {
  ...
  lean_initialize();
  res = initialize_PromiseBasic(1 /* builtin */);
...
"$LEAN_BIN" -R "$SCRIPT_DIR" -c "$EMITC_C" "$SCRIPT_DIR/PromiseBasic.lean"
...
cd "$EMITZIG_DIR"
"$LAKE_BIN" exe emitzig "$SCRIPT_DIR/PromiseBasic.lean" -o "$EMITZIG_ZIG"

python3 - "$EMITZIG_ZIG" <<'PY'
import pathlib
import re
import sys

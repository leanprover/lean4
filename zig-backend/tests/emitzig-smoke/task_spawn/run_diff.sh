...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/emitzig-diff.task_spawn.XXXXXX")"
TMP_INCLUDE_DIR="$TMP_DIR/include"
EMITC_C="$TMP_DIR/task_spawn.c"
EMITC_O="$TMP_DIR/task_spawn-emitc.o"
EMITC_DRIVER_C="$TMP_DIR/task_spawn-emitc-driver.c"
EMITC_DRIVER_O="$TMP_DIR/task_spawn-emitc-driver.o"
EMITZIG_ZIG="$TMP_DIR/task_spawn.zig"
EMITZIG_O="$TMP_DIR/task_spawn-emitzig.o"
EMITZIG_DRIVER_O="$TMP_DIR/task_spawn-emitzig-driver.o"
TASK_SHIMS_O="$TMP_DIR/task_api_shims.o"
COMPAT_O="$TMP_DIR/compat.o"
EMITC_BIN="$TMP_DIR/task_spawn-emitc"
EMITZIG_BIN="$TMP_DIR/task_spawn-emitzig"
...
lean_object * initialize_TaskSpawn(uint8_t builtin);
lean_object * l___private_TaskSpawn_0__main(void);

static lean_object * run_main(int argc, char ** argv) {
  (void)argc;
  (void)argv;
  return l___private_TaskSpawn_0__main();
}
...
int main(int argc, char ** argv) {
  ...
  lean_initialize();
  res = initialize_TaskSpawn(1 /* builtin */);
...
cc -Wall -Wextra -pedantic -x c -std=c11 -c \
  -DEMITZIG_INIT_FN=initialize_TaskSpawn \
  -DEMITZIG_MAIN_FN=l___private_TaskSpawn_0__main \
  -I "$TMP_INCLUDE_DIR" \
  -I "$LEAN4_DIR/src/include" \
  -I "$LEAN4_DIR/build/release/stage1/include" \
  "$COMMON_DIR/driver.c" \
...
"$LEAN_BIN" -R "$SCRIPT_DIR" -c "$EMITC_C" "$SCRIPT_DIR/TaskSpawn.lean"
...
cd "$EMITZIG_DIR"
"$LAKE_BIN" exe emitzig "$SCRIPT_DIR/TaskSpawn.lean" -o "$EMITZIG_ZIG"
